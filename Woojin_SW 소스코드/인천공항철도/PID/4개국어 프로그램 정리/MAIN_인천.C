...
#define MODULE_8_256	0x500 //8*240  (1920)    8*160    0x500 
#define MODULE_16_256	0xA00 //16*240 (3840)    16*160   0xA00 
#define MODULE_24_240           24*240
#define MODULE_32_256	0x1400	//32*240 (7680)    32*160   0x1400 
#define MODULE_40_256 	0x1900	//40*240 (9600)    40*160   0x1900
#define MODULE_48_256	0x1E00	//48*240 (11520)   48*160   0x1E00
                              
#define FONT_24_256		0xF00 	//24*240	(5760)    24*160   0xF00
#define FONT_48_256		0x1E00	//24*240*2(11520)  24*160*2 0x1E00
#define FONT_72_256		0x2D00	//24*240*3(17280)  24*160*3 0x2D00

#define COLOR_GREEN 0x01
#define COLOR_RED   0x02
#define COLOR_AMBER 0x03
...
void DOT_PRINT(); 	//인천 신공항 DOT PRINT
/******************************
	SDR 관련 변수
******************************/
	int COUNT=FALSE;
...
typedef struct {
	...
	BYTE SIZE;	//문자 화면길이
	BYTE COUNT;	//문장 반복 회수
	BYTE LINE_POSI;	//문장 표시행
	BYTE INTERVERL ;//문장 줄 간격
	BYTE STATE;	//스크롤 모드
	BYTE DELAY;	//스크롤 지연 속도
	BYTE STOP;	//스크롤 정지 시간
	BYTE SP_A;
	BYTE SP_B;
}FONT_VALUE;
...
typedef struct{
	...
	BYTE SIZE;	//폰트크기
	BYTE TEMPER;//글자 속성
	BYTE POSI_REM;	//문자 배경색
	BYTE nChar_SizeUp;	//문자 확대
	BYTE nCountry_Code;	//국가 코드
} FONT_VALUE_CHANGE;
FONT_VALUE_CHANGE ESC={0x02,0x01,0x01,0x00,0x00,0x00,0x00};
/***********************************************************
	함수 정의
************************************************************/
...
void FONT_POSITION(BYTE *Posi)
{
     ...
     DCC.SIZE=160;//(DCC.SIZE)*12;	// 항상 160 이어야 하무로 160으로 고정
     	...
     	if(DCC.STATE==0x03){L_BUFF= ((L_BUFF>=(DCC.SIZE/2))&&(L_BUFF<DCC.SIZE)) ? (L_BUFF+(DCC.SIZE/2)) : L_BUFF;}
          ...
          if((Posi[Font_DATA_Cont]&0xff)==0x1b)
          {
               ...
               if((Posi[Font_DATA_Cont]&0xff)==0x01) 
			   {
				   ...
				   else if((Posi[Font_DATA_Cont]&0xff) == 0x02)	{ESC.COLOR = 0x01; }
				   else if((Posi[Font_DATA_Cont]&0xff) == 0x03)	{ESC.COLOR = 0x03; }
				   else {ESC.COLOR = 0x01; }
			   }
			   else if((Posi[Font_DATA_Cont]&0xff)==0x02) {Font_DATA_Cont++; ESC.SHAPE =(Posi[Font_DATA_Cont++]&0xff) >= 0x02 ? 0 : (Posi[Font_DATA_Cont-1]&0xff);}   //글자체
			   else if((Posi[Font_DATA_Cont]&0xff)==0x03) {Font_DATA_Cont++; ESC.SIZE  =(Posi[Font_DATA_Cont++]&0xff) >= 0x04 ? 0 : (Posi[Font_DATA_Cont-1]&0xff);}   //size
			   ...
			   else if((Posi[Font_DATA_Cont]&0xff)==0x07) {Font_DATA_Cont++; ESC.nCountry_Code =(Posi[Font_DATA_Cont++]&0xff) >= 0x03 ? 0 : (Posi[Font_DATA_Cont-1]&0xff);}   //국가 코드
			   else {Font_DATA_Cont+=2;} //코드값이 잘못 들어 오면 건너 뛴다.06_04_24

			   if(WORD_L(ESC.nCountry_Code) == 0x00)
			   {
				   if(WORD_L(ESC.SHAPE) == 0x01){ESC.SHAPE = 0x100000;}
				   else if(WORD_L(ESC.SHAPE) == 0x00){ESC.SHAPE = 0x00;}
				   else {ESC.SHAPE = 0;}

				   if(WORD_L(ESC.SIZE) == 0x01) { ESC.SIZE = 0;}
				   else if(WORD_L(ESC.SIZE) == 0x02) { ESC.SIZE = 0x4B000;}
				   else {ESC.SIZE = 0;}
			   }
          ...
          else if((Posi[Font_DATA_Cont]&0xff)<0x80 ||((Posi[Font_DATA_Cont]&0xff)>=0xA0 
			   && (Posi[Font_DATA_Cont]&0xff)<=0xDF)&& WORD_L(ESC.nCountry_Code) == 0x01)   
          {
			   SCAN.Font_24_T_F=(ESC.SIZE==0) ? 0 : 1;
		  ...
		  else
		  {
				...
				else if(WORD_L(ESC.nCountry_Code) == 0x01)//일본어
				{
				   if(SCAN.Font_24_T_F==0)		//16폰트
				   {
						...
						{
							Jump=((Posi[Font_DATA_Cont++]&0xff)-0x81)*6016+((Posi[Font_DATA_Cont++]&0xff)-0x40)*32+3008+ESC.SIZE+ESC.SHAPE;
						}
						else if((Posi[Font_DATA_Cont]&0xff)>=0x88&&(Posi[Font_DATA_Cont]&0xff)<0xA0) 
						{
							Jump=((Posi[Font_DATA_Cont++]&0xff)-0x88)*6016+((Posi[Font_DATA_Cont++]&0xff)-0x40)*32+27072+ESC.SIZE+ESC.SHAPE;
						}
						else if((Posi[Font_DATA_Cont]&0xff)>=0xE0) 		      
						{
							Jump=((Posi[Font_DATA_Cont++]&0xff)-0xE0)*6016+((Posi[Font_DATA_Cont++]&0xff)-0x40)*32+171456+ESC.SIZE+ESC.SHAPE;
						}
				   ...
				   else				//24폰트
				   {
						if((Posi[Font_DATA_Cont]&0xff)>=0x81&&(Posi[Font_DATA_Cont]&0xff)<0x88)      
						{
							Jump=((Posi[Font_DATA_Cont++]&0xff)-0x81)*13536+((Posi[Font_DATA_Cont++]&0xff)-0x40)*72+6768+ESC.SIZE+ESC.SHAPE;
						}
						else if((Posi[Font_DATA_Cont]&0xff)>=0x88&&(Posi[Font_DATA_Cont]&0xff)<0xA0) 
						{
							Jump=((Posi[Font_DATA_Cont++]&0xff)-0x88)*13536+((Posi[Font_DATA_Cont++]&0xff)-0x40)*72+60912+ESC.SIZE+ESC.SHAPE;
						}
						else if((Posi[Font_DATA_Cont]&0xff)>=0xE0) 		       
						{
							Jump=((Posi[Font_DATA_Cont++]&0xff)-0xE0)*13536+((Posi[Font_DATA_Cont++]&0xff)-0x40)*72+385776+ESC.SIZE+ESC.SHAPE;
						}
	...
	if(SCAN.Move_Point <= 5)
	{
     ...
     if(SCAN.Font_Pr_P > DCC.SIZE)
     {
          for(i=23;i>0;i--)
          {
               ...
               memcpy(&pVd[(i*DCC.SIZE)+MODULE_24_240],&pVd_D[Font_Save_posi],DCC.SIZE);
          }
     }
}
/**********************************************************
...
void DOT_PRINT()
{
	...
	{
		LDM_ADDR(i);
		DMA0_SRC = (0x100000 +(i*(DCC.SIZE)));	//DMA 시작 주소
		DMA0_DST = LDM_UD_DATA_ADDR;															//DMA 도착 번지
		DMA0_CNT = 240;																	//DMA 전송 길이	
		DMA0_GCR = 0x3c13; // source 증가, destination 정지, 최우선 순위		//레지스트값 설정
		LdmLatchClk(0);

		DMA0_SRC = (0x100000 +MODULE_24_240+(i*(DCC.SIZE)));	//DMA 시작 주소
		DMA0_DST = LDM_UD_DATA_ADDR;															//DMA 도착 번지
		DMA0_CNT = 240;																	//DMA 전송 길이	
		DMA0_GCR = 0x3c13; // source 증가, destination 정지, 최우선 순위		//레지스트값 설정
		LdmLatchClk(1);
...
void HEX_SWITCH(void)
{
	...
	for(i=0;i<2;i++)
	{
		SW_RCV= (i==0) ? (gHexaSw>>4)&0x0f : (gHexaSw&0x0f);
...
void c_int09(void)		// Timer 0
{
	...
	static int iLineOffset = 1;
 	...
 	DMA0_SRC = (0x100000 +(iLineOffset%16)*(DCC.SIZE));	//DMA 시작 주소
...
void c_int10(void)		// Timer 1
{
	...
	WDI_CS;
		//상태 DATA 를 요청하였을 겨우 지금 까지의 Count를 같이 전송
	 glDebugCnt++;
}
