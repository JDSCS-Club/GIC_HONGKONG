...
#define MODULE_8_256	0x780	//0x500 //8*240  (1920)    8*160    0x500 
#define MODULE_16_256	0xF00	//0xA00 //16*240 (3840)    16*160   0xA00 
#define MODULE_24_240   0x1680  //        24*240
#define MODULE_32_256	0x1E00  //0x1400	//32*240 (7680)    32*160   0x1400 
#define MODULE_40_256 	0x2580  //0x1900	//40*240 (9600)    40*160   0x1900
#define MODULE_48_256	0x2D00  //0x1E00	//48*240 (11520)   48*160   0x1E00
	                                
#define FONT_24_256		0x1680  //0xF00 	//24*240	(5760)    24*160   0xF00
#define FONT_48_256		0x2D00  //0x1E00	//24*240*2(11520)  24*160*2 0x1E00
#define FONT_72_256		0x4380  //0x2D00	//24*240*3(17280)  24*160*3 0x2D00
...
typedef struct {
USHORT SCC_CODE;	//문장 내용
USHORT SCC_KIND;	//문장 종별
USHORT LEN_HI;	//문장 길이
USHORT LEN_LOW;	//문장 길이

USHORT TXT;	//문자 종별		
USHORT SIZE;	//문자 화면길이
USHORT COUNT;	//문장 반복 회수
USHORT LINE_POSI;	//문장 표시행
USHORT INTERVERL ;//문장 줄 간격
USHORT STATE;	//스크롤 모드
USHORT DELAY;	//스크롤 지연 속도
USHORT STOP;	//스크롤 정지 시간
USHORT SP_A;
USHORT SP_B;
}FONT_VALUE;
FONT_VALUE DCC={0x01,0x01,0x00,0x00,0x01,240,0,6,16,0,4,10,0,0};
...
USHORT SIZE;	//폰트크기
USHORT TEMPER;//글자 속성
USHORT POSI_REM;	//문자 배경색
USHORT nChar_SizeUp;	//문자 확대
USHORT nCountry_Code;	//국가 코드
} FONT_VALUE_CHANGE;
FONT_VALUE_CHANGE ESC={0x02,0x01,0x01,0x01,0x01,0x00,0x00,0x00,0x00};
/***********************************************************
함수 정의
************************************************************/
...
void FONT_POSITION(BYTE *Posi)
{
	...
	for(Font_DATA_Cont=14;Font_DATA_Cont<now_font_Len; )
	{
		...
		if((Posi[Font_DATA_Cont]&0xff)==0x1B)
		{
			...
			else if((Posi[Font_DATA_Cont]&0xff)==0x03) {Font_DATA_Cont++; ESC.D_SIZE  =(Posi[Font_DATA_Cont++]&0xff) >= 0x04 ? 0 : (Posi[(Font_DATA_Cont-1)]&0xff);}   //size
	...
	if(DCC.STATE==0x00)
	{
		DOT_VIDEO_PIRNT();SCREEN_PRINT();Dot_Video=1;Left_Dot_Video=0;Up_Dot_Video=0;STATIC_Left_Dot_Video=0;
	}
...
void Imag_Screen_door(BYTE *Imag)
{
	...
	for(i=Font_16_24;i<24;i++)
	{
		...
		if(SCAN.Font_24_T_F==0)
		{
			for(k=0;k<4;k++)
			{
				for(j=7;j>=0;j--) pVd[Font_Save_posi++]=(((Imag[(Lint_sort*2)+(k*32)]&0xff)>>j)&0x01)==0x01 ? ((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? 0x00 : ESC.COLOR) :((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? ESC.COLOR : 0x00);
				for(j=7;j>=0;j--) pVd[Font_Save_posi++]=(((Imag[((Lint_sort*2)+1+(k*32))]&0xff)>>j)&0x01)==0x01 ? ((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? 0x00 : ESC.COLOR) :((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? ESC.COLOR : 0x00);
			}
		}
		else
		{
