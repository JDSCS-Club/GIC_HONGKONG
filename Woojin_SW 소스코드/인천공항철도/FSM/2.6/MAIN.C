...
void START_DISTANCE_VALUE()
{
	...
	for(j=0;j<18;j++)
	{
		if((Station[Station_cnt]&0xff)==(Station_All[j]&0xff))
		{
			if(!BACK_BIT)
			{
				if((Distance_Point<Station_Led[(j*2)]&0xffff)||(ROAD==0))
				{
					Distance_Point= Station_Led[(j*2)]&0xffff;
				}
			...
			{
				if((Back_Distance<(192-Station_Led[(j*2)+1]&0xffff))||(ROAD==0))
				{
					Back_Distance=(192-Station_Led[(j*2)+1]&0xffff);
				}
