...
extern UCHAR m_SCCPISTXBuff_Ach[50];
...
void DP_DataPro_Main()
{
	...
	if(WORD_L(nTimeFlag.nAutoPlay_Flag))	//200ms 마다 동작 한다. 자동 운전 모드.
	{
		...
		else
		{
			...
			if((WORD_L(nTmsRxData.nDataDefin) == 0x31) || (WORD_L(nTmsRxData.nDataDefin) == 0x33)) // 마서트 인 곳에서만 전송 한다.
			{
				if(WORD_L(SCC2_INIT_A.TxOK) && WORD_L(SCC2_INIT_B.TxOK)) //통신 전송이 끝나면 다시 전송을 한다.
				{
...
void DP_SelfTest_B(UCHAR *pDataBuf,UCHAR *pRxDataBuf,UCHAR sUnitCnt)
{
	...
	else
	{
		...
		Delay_SCC2_SendTo(sSelf_Tx_Buf,20,SCC_B_CHANNEL,50);
...
void DP_Door_Sequence()
{
	...
	if(nDi_Check.nDoor.nFlag && nTr_St_Info.nDoOp_StChFlag) //출입문이 열렸을 경우 처리. nTr_St_Info.nDoOp_StChFlag 출입문이 열리고 통신이 이루어 진후 처리 한다. //120820
	{
	...
	else if(!nDi_Check.nDoor.nFlag)//&& (nIndex_Flag.nDistanRe != nIndex_Flag.nDistanNew))
	{
		...
		if(DWORD_MASKING(nTmsRxData.nDist) >= 50 )	//TMS거리 값이 있어야 된다.
		{
			...
			{
				if(WORD_L(nTr_St_Info.nRxCode_NoNe_Flag))
				{
					DP_Door_CLOSE();
				}
			}
...
void DP_Door_OPEN()
{
	...
	if(WORD_L(nTr_St_Info.nTcmsAutoDriverFlag) &&(WORD_L(nTr_St_Info.nDoOp_StCh) || WORD_L(nTr_St_Info.nAutoManuFlag)))
	{
...
void DP_IM_PR_TEXT_READ(UCHAR nTextNum,UCHAR nIM_PR_Code,UCHAR nUnitCode)
{
	...
	Delay_SCC2_SendTo(sTX_DATABUF,(sTextLen+10),SCC_A_CHANNEL,5);
		...
		{	
			if(!strncmp((char *)&nTempBuf[i+1],(char *)"[NextSta]",9))
			{
				...
				else 	
				{
					...
					memset(&pDData[DestOffst],0x20,20);
			...
			else if(!strncmp((char *)&nTempBuf[i+1],(char *)"[A)NextSta]",11))
			{
				...
				{	
					...
					memcpy(&pDData[DestOffst+((20-j)/2)],nTemp,j);
...
void DP_SCC1_A_RX_PRO(UCHAR *pRxData)
{
	...
	nTmsRxData.nDataDefin = pTmsRxData->nStCode;
	...
	if((WORD_L(nTmsRxData.nDataDefin) >= 0x31) && (WORD_L(nTmsRxData.nDataDefin) <= 0x34))
	{
		...
		if(WORD_L(nTr_St_Info.nAutoManuFlag)){} //수동 설정으로 전환 하였을 경우
		else
		{
			//현재역, 다음역이 "0" 이면 자동 운전을 해제 한다.
			if((!WORD_L(pTmsRxData->nNowCode) || !WORD_L(pTmsRxData->nNextCode)) ||  !DWORD_L(sTrNUM))
			{
			...
			else
			{
				...
				if(WORD_L(pTmsRxData->nNowCode) == WORD_L(pTmsRxData->nNextCode))
				{//2011_11_10 
			...
			if(WORD_L(nNvsram_Variable.nMatchCode[WORD_L(pTmsRxData->nDestCode)]) != WORD_L(nTr_St_Info.nOnlyDestSendFlag))
			{
				if(WORD_L(nNvsram_Variable.nMatchCode[WORD_L(pTmsRxData->nDestCode)]) != 0xFF)
				{
					 nTr_St_Info.nTcmsAutoDriverFlag = TRUE;//자동 운행 시작
					 //2011_11_16 nOnlyDestSendFlag
					nTr_St_Info.nOnlyDestSendFlag =  WORD_L(nNvsram_Variable.nMatchCode[WORD_L(pTmsRxData->nDestCode)]);
					nSccSendFlag.nDeSTCodeSet = 2;
		...
		//출입문이 열려 있을때문 검사 한다.
		if(nDi_Check.nDoor.nFlag && !WORD_L(nTr_St_Info.nAutoManuFlag)) //수동 설정이 되어 있으면 역코드 수신 상태를 확인 하지 않는다. 
		{
			
			if(WORD_L(StationCode_Definite(WORD_L(pTmsRxData->nNowCode),WORD_L(pTmsRxData->nNextCode))) >= 2 ) //현재역. 다음역이 정상 수신 되면 
			{

				nTmsRxData.nDoOpenIf = ((pTmsRxData->BIT.nDoor2<<1) | (pTmsRxData->BIT.nDoor1))&0x03; //120820 출입문 정보는 도어 OPEN 일 경우 TCMS 정보로 계속 갱신 한다.
				//열번이 변경 되었을 경우
				...
				if(nTr_St_Info.nStopPatNum && //열차 번호가 유효 하다면...
					(WORD_L(nSelect_Button.nGPTranNumInput.nNowSt_Re) != WORD_L(pTmsRxData->nNowCode) ||
					WORD_L(nSelect_Button.nGPTranNumInput.nNextSt_Re) != WORD_L(pTmsRxData->nNextCode)))
				{
					d_CarNum1 = nSelect_Button.nGPTranNumInput.nNowSt_Re = WORD_L(pTmsRxData->nNowCode);
					d_CarNumRem1 = nSelect_Button.nGPTranNumInput.nNextSt_Re = WORD_L(pTmsRxData->nNextCode);
					...
					if(WORD_L(nSelect_Button.nGPTranNumInput.nNowSt_Re) != WORD_L(nSelect_Button.nGPTranNumInput.nNextSt_Re))
					{
						...
						nTmsRxData.nNow = WORD_L(nNvsram_Variable.nMatchCode[WORD_L(pTmsRxData->nNowCode)]);
						nTmsRxData.nNext = WORD_L(nNvsram_Variable.nMatchCode[WORD_L(pTmsRxData->nNextCode)]);
			...
			else
			{
				...
				sprintf(szBuf2,"-Door Open St-St Code RX NG-NG : Now[%02X]H : Next[%02X]H ",WORD_L(pTmsRxData->nNowCode),WORD_L(pTmsRxData->nNextCode));
				strcat(szBuf,szBuf2);
				sdebugFlag = TRUE;
			}
...
void DP_SEND_PRO()
{
	...
	if(WORD_L(nTr_St_Info.nSelf_Test_Flag))	//자기 진단 진행중일때 전송한다.
	{
		if(!nSelf_Test.nSELF_ICR_SELECT)
		{
			memcpy(nSelf_Test.nSELF_ADDDATA_BUF,DISPLAY_UNIT_ADD,DISPLAY_UNIT);
			if(nSelf_Test.nSELF_All_Cnt < SELF_TEST_ALLCNT){
				DP_SelfTest(nSelf_Test.nSELF_ADDDATA_BUF,nSelf_Test.nSELF_RXDATA_Buf,DISPLAY_UNIT_A);	// 직통
			}
			
			if(nSelf_Test.nSELF_All_Cnt_B < SELF_TEST_ALLCNT){
				DP_SelfTest_B(&nSelf_Test.nSELF_ADDDATA_BUF[DISPLAY_UNIT_A],&nSelf_Test.nSELF_RXDATA_Buf[DISPLAY_UNIT_A],DISPLAY_UNIT_B);	// 직통
			}
		}
		else if(nSelf_Test.nSELF_ICR_SELECT)
		{
			memcpy(nSelf_Test.nSELF_ADDDATA_BUF,DISPLAY_UNIT_ADD_NOR_PIB,DISPLAY_STANDARD_UNIT_A);
			memcpy(&nSelf_Test.nSELF_ADDDATA_BUF[DISPLAY_STANDARD_UNIT_A],DISPLAY_UNIT_ADD_NOR_RND,DISPLAY_STANDARD_UNIT_B);
			if(nSelf_Test.nSELF_All_Cnt < SELF_TEST_ALLCNT){
				DP_SelfTest(nSelf_Test.nSELF_ADDDATA_BUF,nSelf_Test.nSELF_RXDATA_Buf,DISPLAY_STANDARD_UNIT_A);	// 일반
			}
		...
		if(WORD_L(nSelf_Test.nSELF_All_Cnt) >=SELF_TEST_ALLCNT && nSelf_Test.nSELF_All_Cnt_B >= SELF_TEST_ALLCNT)	//전책 반복 횟수가 3회를 넘으면 종료.
		{
			...
			DP_TCMSSD_Format(TCMSSD,&m_SCCPISTXBuff_Ach[3]); //TCMS 전송 DATA 를 갱신 한다.
	...
	else if(WORD_L(nSccSendFlag.nDeSTCodeSet) || WORD_L(nSccSendFlag.nTranNum)) // 정면 행선 표시기 출력
	{
		...
		Delay_SCC2_SendTo(sDestTxBuf,20,SCC_B_CHANNEL,5);
	...
	else if(WORD_L(nTr_St_Info.nTcmsAutoDriverFlag) || WORD_L(nTr_St_Info.nSimulationFlag)) //주행 및 시뮬레이션 기능 동작시 조작 가능한 조건들.
	{
		...
		else if(WORD_L(nSccSendFlag.nRNDTXFlag) && 	!WORD_L(nTr_St_Info.nClean_Flag))//노선 표시기 전송.  //무표시 조작시 전송 금지.
		{
			...
			if((WORD_L(nTr_St_Info.nNow.nCode)+1) && (WORD_L(nTr_St_Info.nNext.nCode)+1) && (WORD_L(nTr_St_Info.nDest.nCode)+1))
			{
				...
				Delay_SCC2_SendTo(sRnd_Tx_Buf,20,SCC_B_CHANNEL,5);
	...
	else if(!WORD_L(nTr_St_Info.nTcmsAutoDriverFlag) && !WORD_L(nTr_St_Info.nSimulationFlag))//자동 운전도 아니고 시뮬레이션도 아닐때 패턴 TEST 실시 한다.
	{
		...
		if(nSccSendFlag.nPIBPaten) // 차내 패턴 TEST
		{
			...
			Delay_SCC2_SendTo(sSimu_Tx_Buf,20,SCC_A_CHANNEL,5);
			nSccSendFlag.nPIBPaten = 0;
		}
		else if(nSccSendFlag.nS_FDIPaten) // 정면,열번,노선 패턴 TEST
		{
			...
			Delay_SCC2_SendTo(sSimu_Tx_Buf,20,SCC_B_CHANNEL,5);
...
void DP_SD_Format_Change(UCHAR *pDData)
{
	...
	//TC1
	TCMSSD[0].DR.BIT.B0 = 0;//IDC
	TCMSSD[0].DR.BIT.B1 = WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[50] : pDData[26] ;//TNSU1
	TCMSSD[0].DR.BIT.B2 = WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[48] : pDData[24] ;//DUS-F
	...
	//CAR1
	TCMSSD[1].DR.BIT.B0 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[0] : pDData[0] ; 	//IDU1
	TCMSSD[1].DR.BIT.B1 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[6] : pDData[6] ;   //IDU2
	TCMSSD[1].DR.BIT.B2 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[12] : 0 ;	//IDU3
	TCMSSD[1].DR.BIT.B3 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[18] : 0 ;	//IDU4
	TCMSSD[1].DR.BIT.B4 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[24] : pDData[12];	//FSM1   - CNSU1
	TCMSSD[1].DR.BIT.B5 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[30] : 0 ;	//FSM2
	TCMSSD[1].DR.BIT.B6 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[36] : 0 ;	//FSM3
	TCMSSD[1].DR.BIT.B7 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[42] : pDData[18];	//FSM4   - CNSU2

	//CAR2
	TCMSSD[2].DR.BIT.B0 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[1] : pDData[1] ; 	//IDU1
	TCMSSD[2].DR.BIT.B1 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[7] : pDData[7] ;   //IDU2
	TCMSSD[2].DR.BIT.B2 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[13] : 0 ;	//IDU3
	TCMSSD[2].DR.BIT.B3 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[19] : 0 ;	//IDU4
	TCMSSD[2].DR.BIT.B4 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[25] : pDData[13];	//FSM1   - CNSU1
	TCMSSD[2].DR.BIT.B5 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[31] : 0 ;	//FSM2
	TCMSSD[2].DR.BIT.B6 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[37] : 0 ;	//FSM3
	TCMSSD[2].DR.BIT.B7 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[43] : pDData[19];	//FSM4   - CNSU2

	//CAR3
	TCMSSD[3].DR.BIT.B0 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT)? pDData[2] : pDData[2] ; 	//IDU1
	TCMSSD[3].DR.BIT.B1 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT)? pDData[8] : pDData[8] ;   //IDU2
	TCMSSD[3].DR.BIT.B2 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT)? pDData[14] : 0 ;	//IDU3
	TCMSSD[3].DR.BIT.B3 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT)? pDData[20] : 0 ;	//IDU4
	TCMSSD[3].DR.BIT.B4 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT)? pDData[26] : pDData[14];	//FSM1   - CNSU1
	TCMSSD[3].DR.BIT.B5 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT)? pDData[32] : 0 ;	//FSM2
	TCMSSD[3].DR.BIT.B6 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT)? pDData[38] : 0 ;	//FSM3
	TCMSSD[3].DR.BIT.B7 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT)? pDData[44] : pDData[20];	//FSM4   - CNSU2

	//CAR4
	TCMSSD[4].DR.BIT.B0 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[3] : pDData[3] ; 	//IDU1
	TCMSSD[4].DR.BIT.B1 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[9] : pDData[9] ;   //IDU2
	TCMSSD[4].DR.BIT.B2 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[15] : 0 ;	//IDU3
	TCMSSD[4].DR.BIT.B3 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[21] : 0 ;	//IDU4
	TCMSSD[4].DR.BIT.B4 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[27] : pDData[15];	//FSM1   - CNSU1
	TCMSSD[4].DR.BIT.B5 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[33] : 0 ;	//FSM2
	TCMSSD[4].DR.BIT.B6 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[39] : 0 ;	//FSM3
	TCMSSD[4].DR.BIT.B7 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[45] : pDData[21];	//FSM4   - CNSU2

	//CAR5
	TCMSSD[5].DR.BIT.B0 =  0; 			//IDU1
	TCMSSD[5].DR.BIT.B1 =  0; 		    //IDU2
	TCMSSD[5].DR.BIT.B2 =  0;			//IDU3
	TCMSSD[5].DR.BIT.B3 =  0;			//IDU4
	TCMSSD[5].DR.BIT.B4 =  0;			//FSM1   - CNSU1
	TCMSSD[5].DR.BIT.B5 =  0;			//FSM2
	TCMSSD[5].DR.BIT.B6 =  0;			//FSM3
	TCMSSD[5].DR.BIT.B7 =  0;			//FSM4   - CNSU2

	//CAR6
	TCMSSD[6].DR.BIT.B0 =  0;			//IDU1	
	TCMSSD[6].DR.BIT.B1 =  0;			//IDU2
	TCMSSD[6].DR.BIT.B2 =  0;			//IDU3
	TCMSSD[6].DR.BIT.B3 =  0;			//IDU4
	TCMSSD[6].DR.BIT.B4 =  0;			//FSM1   - CNSU1
	TCMSSD[6].DR.BIT.B5 =  0;			//FSM2
	TCMSSD[6].DR.BIT.B6 =  0;			//FSM3
	TCMSSD[6].DR.BIT.B7 =  0;			//FSM4   - CNSU2

	//CAR7
	TCMSSD[7].DR.BIT.B0 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[4] : pDData[4] ; 	//IDU1
	TCMSSD[7].DR.BIT.B1 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[10] : pDData[10] ;   //IDU2
	TCMSSD[7].DR.BIT.B2 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[16] : 0 ;	//IDU3
	TCMSSD[7].DR.BIT.B3 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[22] : 0 ;	//IDU4
	TCMSSD[7].DR.BIT.B4 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[28] : pDData[16];	//FSM1   - CNSU1
	TCMSSD[7].DR.BIT.B5 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[34] : 0 ;	//FSM2
	TCMSSD[7].DR.BIT.B6 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[40] : 0 ;	//FSM3
	TCMSSD[7].DR.BIT.B7 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[46] : pDData[22];	//FSM4   - CNSU2

	//CAR8
	TCMSSD[8].DR.BIT.B0 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[5] : pDData[5] ; 	//IDU1
	TCMSSD[8].DR.BIT.B1 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[11] : 0 ;   //IDU2
	TCMSSD[8].DR.BIT.B2 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[17] : 0 ;	//IDU3
	TCMSSD[8].DR.BIT.B3 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[23] : 0 ;	//IDU4
	TCMSSD[8].DR.BIT.B4 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[29] : pDData[17];	//FSM1   - CNSU1
	TCMSSD[8].DR.BIT.B5 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[35] : 0 ;	//FSM2
	TCMSSD[8].DR.BIT.B6 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[41] : 0 ;	//FSM3
	TCMSSD[8].DR.BIT.B7 =   WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[47] : 0;	//FSM4   - CNSU2

	//TC2
	TCMSSD[9].DR.BIT.B0 =  0; 			//IDC	
	TCMSSD[9].DR.BIT.B1 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[51] : pDData[27] ;	//TNSU1
	TCMSSD[9].DR.BIT.B2 =  WORD_L(nSelf_Test.nSELF_ICR_SELECT) ? pDData[49] : pDData[25] ;	//DUS-F1	
	TCMSSD[9].DR.BIT.B3 =  0;	
	TCMSSD[9].DR.BIT.B4 =  0;	
	TCMSSD[9].DR.BIT.B5 =  0;	
	TCMSSD[9].DR.BIT.B6 =  0;	
	TCMSSD[9].DR.BIT.B7 =  0;	
...
void DP_TCMSSD_Format(TMS_SEND_DATA *Type,UCHAR *pDData)		
{
	...
	pDData[0] = 0x30;
	for(i=0;i<10;i++) pDData[(i+1)] = Type[i].DR.BYTE;
	pDData[11] = ROM_VER;
...
void DP_Timer_100ms_DataPro()
{
	...
	if(!(nTimeDataPro_100Cnt%10) && (!nTr_St_Info.nSelf_Test_Flag) && WORD_L(nSelf_Test.nSELF_ICR_SELECT))	//1초
	{
