...
extern UCHAR m_CDMA_TimeBuf[10];
...
void Display_Init()
{
	...
	memset(&nPault.n14Fa.nFaultRE,0x00,16);

	memset(&nScreenInf,0x00,sizeof(nScreenInf));

	nScreenInf.mkiGi_Mode = 0x00; //2012_04_23 분선 모드로 고정 한다.
...
void Display_Debug()
{
	...
	sTxProBuf[sTxCnt++] = WORD_L(m_CDMA_TimeBuf[0]); //년
	sTxProBuf[sTxCnt++] = WORD_L(m_CDMA_TimeBuf[1]); //년

	sTxProBuf[sTxCnt++] = WORD_H(m_CDMA_TimeBuf[2]); //월
	sTxProBuf[sTxCnt++] = WORD_L(m_CDMA_TimeBuf[2]); //월

	sTxProBuf[sTxCnt++] = WORD_H(m_CDMA_TimeBuf[3]); //일
	sTxProBuf[sTxCnt++] = WORD_L(m_CDMA_TimeBuf[3]); //일

	sTxProBuf[sTxCnt++] = WORD_H(m_CDMA_TimeBuf[4]); //시
	sTxProBuf[sTxCnt++] = WORD_L(m_CDMA_TimeBuf[4]); //시

	sTxProBuf[sTxCnt++] = WORD_H(m_CDMA_TimeBuf[5]); //분
	sTxProBuf[sTxCnt++] = WORD_L(m_CDMA_TimeBuf[5]); //분

	sTxProBuf[sTxCnt++] = WORD_H(m_CDMA_TimeBuf[6]); //초
	sTxProBuf[sTxCnt++] = WORD_L(m_CDMA_TimeBuf[6]); //초
	...
	ModBusMulti_WORD(0x00,335,sTxCnt/2,(UCHAR *)sTxProBuf,XR16L784_3CHL); // 화면 시간을 설정 한다.
...
UCHAR nTemp_Check_Flag = 0; // 온도 설정 인식이 정상 적인 온도 범위에 있는지 확인 한다.
...
void Display_Driver1_Data()
{
	...
	else
	{
		...
		//TC 9
		sTempDataBuf[18] = MAKE_WORD( pTisMemo->nTC_9.nAd04W1.nData.nWord.nTEMP2[0],pTisMemo->nTC_9.nAd04W1.nData.nWord.nTEMP2[1]);
		sTempDataBuf[19] = MAKE_WORD( pTisMemo->nTC_9.nAd04W1.nData.nWord.nTEMP1[0],pTisMemo->nTC_9.nAd04W1.nData.nWord.nTEMP1[1]);

		

		if(nTemp_Check_Flag >= 5){Display_Start();} // 13_03_19 - 온도 값이 잘 못 인식 되면 기본 값을 다시 읽는다.

		sTempStVal =(WORD *)&nTemp_StVla;
		sTempVal = (WORD *)&nTemp_Val.nT_0; //off_Set 값.
		...
		for(i = 0; i < 20;i++)
		{
			...
			if(WORD_L(sTempData) > 50) {nTemp_Check_Flag++;} // 온도가 특정 범위를 벗어 나면 카운터를 증가 한다. 13_03_11
			
			////////////////////  온도 OFF SET 값을 조절 한다.//////////////////////
			//sTempData = (int )Display_Tem_Vel(sTempDataBuf[i]);
			/* 13_03_19 73~75편성은 온도 OFF SET 삭제 */
			
			if(WORD_L(nTemp_Val.nTempOriginal)){ } //OFF SET 말고 진짜 값을 확인 한다.
			else
			{

				if(sTempData < 0) //130227_Q1
				{
					sTempData = 65535 + (sTempVal[i] + sTempData);
				}
				else
				{
		...
		for(i = 0; i < 6;i++)
		{
			...
			sTempData = sTempData+(sTempData+((sTempData/10)*2)); //2011년 06월28일 1.2로 수정.
			...
			*/
			...
			if(nCurrent.nPow_Step_4){ //파워링이 4단이면 OFF SET 값을 적용한다. 2010_05_20
				...
				sTempData = (sTempData<= 60) ? 0 : (sTempData+sTempVal[i]);
			}
			else{
				sTempData = (sTempData<= 60) ? 0 : sTempData;
			}
		...
		for(i = 0; i < 6;i++)
		{
			...
			sTempData = ((sTempData2/3276)*10+(sTempData2%3276)/327)*80;

			sTempData = sTempData+((sTempData/100)*2);  //2010년 04월20일

				/*증폭회로 수정 전.
				sTempData = ((sTempData2/3276)*10+(sTempData2%3276)/327)*40;

				sTempData = sTempData+((sTempData/10)*4);  //2010년 04월20일
				*/

			sTempData = (sTempData <= 500) ? 0 : sTempData;
...
UCHAR Display_Common_Data_Pro(BIT_DATAPRo *pData,UCHAR sSTPoint)
{
	...
	pData[sCnt].nData.nBit.nBit8 = (nTemp_Check_Flag >= 5) ? 1 :  0; //온도 설정. 
	sCnt++;
...
UCHAR Display_Common_Data_ProAn(BIT_DATAPRo *pData,UCHAR sSTPoint)
{
	...
	sTempData = ((sTempData/3276)*10+(sTempData%3276)/327)*80;
	sTempData = sTempData+((sTempData/100)*2);  //2010년 04월20일
	...
	//{
		pData[sCnt].nData.nBit.nBit1 = sTcDiList->nXd16A1.nData.nBit.nATS_R0;
		pData[sCnt].nData.nBit.nBit2 = sTcDiList->nXd16A1.nData.nBit.nATS_R1;
		pData[sCnt].nData.nBit.nBit3 = sTcDiList->nXd16A1.nData.nBit.nATS_YY;
		pData[sCnt].nData.nBit.nBit4 = sTcDiList->nXd16A1.nData.nBit.nATS_Y;
		pData[sCnt].nData.nBit.nBit5 = sTcDiList->nXd16A1.nData.nBit.nATS_G;
		pData[sCnt].nData.nBit.nBit6 = sTcDiList->nXd16W4.nData.nBit.nATS_EM;

		sATS_Data = pData[sCnt].nData.nBYTE;

		for(i = 0; i<8;i++)//신호가 2개이상 입력되어 화면정보가 이상동작 하는것을 막는다.
		{
			if((sATS_Data>>i)&0x01) break;
		}

		pData[sCnt].nData.nBYTE = sATS_Buf_Add[i];
	...
	{
		pData[sCnt].nData.nBYTE = 21;	//마스터 선택시 기지 - 본선 선택 화면이 출력 된다. 
	}
	else
	{
		mkiGi_Mode_Cnt++;
	...
	else
	{
		...
		case 2:
			...
			else if(nItemCnt == 1)
			{
				...
				Display_AnData(&sAnDataBuf_BackUp[0],&sAnDataBuf[0],1,0,40,0); //가선 전압
				Display_AnData(&sAnDataBuf_BackUp[2],&sAnDataBuf[2],1,0,10,0); //모터전류
				Display_AnData(&sAnDataBuf_BackUp[4],&sAnDataBuf[4],2,0,0,0);  //제동요구

				Display_AnData(&sAnDataBuf_BackUp[8],&sAnDataBuf[8],1,1,0,0);  // 서부레벨

				//Display_AnData(&sAnDataBuf_BackUp[10],&sAnDataBuf[10],1,0,0,0); 
				Display_AnData_Sl(&sAnDataBuf_BackUp[10],&sAnDataBuf[10],1);    //SL
				Display_AnData(&sAnDataBuf_BackUp[12],&sAnDataBuf[12],2,0,0,m_Temp_Conver); //온도

				Display_AnData(&sAnDataBuf_BackUp[16],&sAnDataBuf[16],4,0,0,0);  //발제 제한

				//Display_AnData(sAnDataBuf_BackUp,sAnDataBuf,12);

				Display_AnData(m_CloseLb4Buf,pTisMemo->nM1_1.nAd04W3.nData.nWord.nClose_LB4An,1,0,0,0);  //CLOSE LB4
...
void Display_AnData(UCHAR *pBackUp,UCHAR *pSouData,UCHAR Cnt,UCHAR nBcBrake,UCHAR sChangData,UCHAR sTempData)
{
	...
	for(i = 0;i < Cnt;i++)
	{
		...
		if(sChangData && !sTempData)//가선 전압 //모터 전류
		{
			if(sChangData == 40) //가선 전압
			{
				sAnDataVal = sAnDataVal*80;
				sAnDataVal = sAnDataVal+((sAnDataVal/100)*2);

			}
			else if(sChangData == 10) //모터 전류
			{
				sAnDataVal = sAnDataVal*10;
				sAnDataVal = sAnDataVal+(sAnDataVal+((sAnDataVal/10)*2));
		...
		else if(!sChangData && sTempData) //온도
		{
			sAnDataVal = MAKE_WORD(pSouData[(i*2)],pSouData[(i*2)+1]);
...
void Display_Fault()
{
	...
	if(s43aData && sTcDiList->nXd16W2.nData.nBit.nBPR)
	{
		...
		{	
			...
			if(nPault.s14Fa_TimeCnt.nFaultCnt >= 60)  //2010년 04월20일
			{
	...
	else {nPault.n19Fa.nFaultRE = 0; nPault.n19Fa.nFaultCnt = 0; sCnt = sCnt +2; }
	...
	else{nPault.n21Fa.nFaultRE = 0;  nPault.n21Fa.nFaultCnt = 0; sCnt = sCnt +2; }
	...
	else {nPault.n24Fa.nFaultRE = 0; nPault.n24Fa.nFaultCnt = 0; sCnt = sCnt +2; }
	////////////SIV기동 정지////////////////////////
	...
	else {nPault.n26Fa.nFaultRE = 0; nPault.n26Fa.nFaultCnt = 0; sCnt = sCnt +2; }
	////////////AISUS,FSIL////////////////////////
	...
	else{nPault.n29Fa.nFaultRE = 0;  nPault.n29Fa.nFaultCnt = 0; sCnt = sCnt +2; }
