...
//  4) 13_03_11  - 온도 값 이상 현시 확인 부분 /  / CDMA 시간 실시간 전송
////////////////////////////////////////////////////////////////////////////////////////////////
...
int main()
{
	...
	while(1)
	{
		...
		if(nTx_Data_Flag_4Ch && !m_nXr16l784TxPos1 && !m_nXr16l784TxPos3&& !m_nXr16l784TxPos5 && !m_nXr16l784TxPos4) //250ms 마다 실행 한다.
		{
			...
			if(!(m_TimeTxBuf_Cnt %10) && (WORD_L(nCdma_time_Flag.nFlag) == TRUE)) // 0호차만 CDMA 함수를 시작 한다.
			{
...
void ModBus_Cdma_Time()
{
	...
	else
	{
		...
		if(WORD_L(nCdma_time_Flag.nDisplay_time_Set) <= 50)
		{
		...
		else//nScreenInf.nDATE CDMA 모듈이 있는 곳에서 시간 설정이 되었다면, 메인 통신에 시간 정보를 상시 적으로 전송 한다. 13_03_11
		{
			m_CDMA_TimeBuf[0] = (WORD_H(nCdma_time_Flag.nYY) == 0) ? m_CDMA_TimeBuf[0] : WORD_H(nCdma_time_Flag.nYY);
			m_CDMA_TimeBuf[1] = (WORD_L(nCdma_time_Flag.nYY) == 0) ? m_CDMA_TimeBuf[1] : WORD_L(nCdma_time_Flag.nYY);

			m_CDMA_TimeBuf[2] = WORD_L(nScreenInf.nDATE[1]); //월
			m_CDMA_TimeBuf[3] = WORD_L(nScreenInf.nDATE[2]); //일

			m_CDMA_TimeBuf[4] = WORD_L(nScreenInf.nDATE[3]); //시
			m_CDMA_TimeBuf[5] = WORD_L(nScreenInf.nDATE[4]); //분

			m_CDMA_TimeBuf[6] = WORD_L(nScreenInf.nDATE[5]); //초
			m_CDMA_TimeBuf[7] = 0x11;
			...
			for(i=0;i<nRxLen;i++)
			{
				...
				default:
					...
					else if(nRxPos_3Ch >= 27)
					{
						...
						nScreenInf.nDATE[0] = BCD2BIN(m_ucXr161784RxBackup[8]);//년
						nScreenInf.nDATE[1] = BCD2BIN(m_ucXr161784RxBackup[10]);//월
						nScreenInf.nDATE[2] = BCD2BIN(m_ucXr161784RxBackup[12]);//일
						nScreenInf.nDATE[3] = BCD2BIN(m_ucXr161784RxBackup[14]);//시
						nScreenInf.nDATE[4] = BCD2BIN(m_ucXr161784RxBackup[16]);//분
						nScreenInf.nDATE[5] = BCD2BIN(m_ucXr161784RxBackup[18]);//초
						...
						if(nScreenInf.nMesgNum == 100){ //기지 본선 모드 선택 ----> 기지 모드
							...
							nTemp_Val.nTempOriginal = FALSE;
						}
						else if(nScreenInf.nMesgNum ==220){ //전류 OFF SET 설정 값. 2010_05_20
							nCurrent.nCurFlag = 5;
						}
						else if(nScreenInf.nMesgNum ==230)
						{
							nTemp_Val.nTempOriginal = TRUE;
