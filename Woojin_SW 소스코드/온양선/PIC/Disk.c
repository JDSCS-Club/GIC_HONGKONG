...
PNVSRAM_VARIABLE nNvsram_Variable = (NVSRAM_VARIABLE *) NVSRAM_SRAM_1;
PSTATION_NAME StationName = (STATION_NAME *) NVSRAM_SRAM_2;
PSTATION_NAME_ALL StationNameAll = (STATION_NAME_ALL *) NVSRAM_SRAM_3;
DISPLAY_INDEX_NAME nDisplayIndex;	//표시장치 데이타를 가지고 있다.
//PDISPLAY_INDEX_NAME nDisplayIndex = (DISPLAY_INDEX_NAME *) NVSRAM_SRAM_4;
...
int FileCompare1()
{
	...
	{
		if(!strncmp((char *)FileList[i].FileName,(char *)"PATTERN ",8) && 
		   !strncmp((char *)FileList[i].ExtName,(char *)"BIN",3)){j=i;break;}
	}
	return j;
}	
...
int FileCompare2()
{
	...
	for(i=0;i<nFileEntryCnt;i++)
	{
		...
		if(!strncmp((char *)FileList[i].FileName,(char *)"IDC_SW",6) && 
		    !strncmp((char *)FileList[i].ExtName,(char *)"BIN",3)){j=i;break;}
...
int RND_SWFind()
{
	...
	for(i=0;i<nFileEntryCnt;i++)
	{
		if(!strncmp((char *)FileList[i].FileName,(char *)"SDD_SW",6) && 
		   !strncmp((char *)FileList[i].ExtName,(char *)"BIN",3)){j=i;break;}
	}
	return j;
}
...
int FDD_SWFind()
{
	...
	for(i=0;i<nFileEntryCnt;i++)
	{
		if(!strncmp((char *)FileList[i].FileName,(char *)"FDD_SW",6) && 
		   !strncmp((char *)FileList[i].ExtName,(char *)"BIN",3)){j=i;break;}
	}
	return j;
}
...
{	  
	...
	if(WORD_L(sKindUnit) == 1){nTemp1 = FileCompare2();}
...
UCHAR Nvsram_IDC_UpLoad_CK(UCHAR *pData,UCHAR sKIndCode)
{
	...
	d_Idc_Up_Check_Flag++;
	if(Seek_File_UNIT(sKIndCode) && DWORD_L(glTotalClusterCnt))	// Match File 확인 
	{
		PreSector = 0;	//같은 곳을 읽어도 가능하도록 Flag 초기화 한다.
		sPoBlk_Ck+= m_FileSize-28;
...
UCHAR Nvsram_EditerData()
{
	...
	memset((UCHAR *)ADD_NVSRAM_STATION_NAME,0x00,(m_FileSize+512));
	memset((UCHAR *)NVSRAM_SRAM_1,0x00,20000); // 변수 초기화 
...
UCHAR NVSRAM_Display_Schedule(UCHAR Now, UCHAR Next, UCHAR Info)
{
	...
	d_ScheduleCnt = sDis_G_Cnt = MAKE_WORD(pSt_Pt_G_I.nHeader.nSt_P_G_Cnt_b[0],pSt_Pt_G_I.nHeader.nSt_P_G_Cnt_b[1]); //정차 그룹 ITEM 개수

	for(i=0;i<sDis_G_Cnt;i++)
	{
		sDis_G_Index_Now = MAKE_WORD(pSt_Pt_G_I.nSt_Pt_G_I_Flage[i].nSt_Info_flag.nSt_Index_b[0],pSt_Pt_G_I.nSt_Pt_G_I_Flage[i].nSt_Info_flag.nSt_Index_b[1]);
		sDis_G_Index_Nex = MAKE_WORD(pSt_Pt_G_I.nSt_Pt_G_I_Flage[i].nSt_Info_flag.nArr_Index_b[0],pSt_Pt_G_I.nSt_Pt_G_I_Flage[i].nSt_Info_flag.nArr_Index_b[1]);

		if(WORD_L(StationNameAll->StaName[sDis_G_Index_Now].StationCode) == WORD_L(Now) && 
		   WORD_L(StationNameAll->StaName[sDis_G_Index_Nex].StationCode) == WORD_L(Next))
...
UCHAR NVSRAM_Find_Schedule_Cnt(UCHAR *pData,UCHAR Display_Cnt,DWORD StartDistance,DWORD EndDistance)
{
	...
	d_dddCheck = Display_Cnt;

	memset(d_DisBuf,0x00,sizeof(d_DisBuf));
	
	for(i=0;i<Display_Cnt;i++)
	{	
		//d_DisBuf[i] = DWORD_L(nDisplayIndex.nDisplay_str[i].nDis_Point);
		
		if((WORD_L(nDisplayIndex.nDisplay_str[i].Type)==0)||(WORD_L(nDisplayIndex.nDisplay_str[i].Type)==1)||(WORD_L(nDisplayIndex.nDisplay_str[i].Type)==3)) //Point-9_29 시발역에서 거리값이 0일경우 계속같은문장 전송하는문재 해결
		{
			d_dddCnt++;
			if(!DWORD_L(EndDistance) && !DWORD_L(nDisplayIndex.nDisplay_str[i].nDis_Point))
			{
				pData[nCnt++] = i;	
			}
			else if((DWORD_L(StartDistance) < DWORD_L(nDisplayIndex.nDisplay_str[i].nDis_Point))
				  &&(DWORD_L(nDisplayIndex.nDisplay_str[i].nDis_Point) <= DWORD_L(EndDistance)))
			{
				pData[nCnt++] = i;
			}
...
WORD NVSRAM_Get_StopP(UCHAR *pData)
{
	...
	for(i=0;i<sCnt;i++)
	{
		pData[i] = MAKE_WORD(pSt_Pt_G_I.nSt_Pt_G_I_Flage[i].nSt_Info_flag.nSt_Index_b[0],
			                                             pSt_Pt_G_I.nSt_Pt_G_I_Flage[i].nSt_Info_flag.nSt_Index_b[1]);

		if(i == (sCnt-1))
		{
			pData[(i+1)] = MAKE_WORD(pSt_Pt_G_I.nSt_Pt_G_I_Flage[i].nSt_Info_flag.nArr_Index_b[0],
...
WORD NVSRAM_Rd_StationName(WORD nCnt,UCHAR *pData)// nCnt개의 역명을 버퍼에 저장한다.
{
	...
	{
		if(WORD_L(StationNameAll->StaName[pData[i]].StationCode) == WORD_L(pData[i]))
		{
			for(j=0;j<15;j++)
			{
				StationName->StaName[i].EngName[j] = WORD_L(StationNameAll->StaName[pData[i]].EngName[j]);
			}
			for(j=0;j<30;j++)
			{
				StationName->StaName[i].IranName[j] = WORD_L(StationNameAll->StaName[pData[i]].IranName[j]);
			}
		}	
		else 
		{
			memset(StationName->StaName[i].EngName,0,15);
			memset(StationName->StaName[i].IranName,0,30);
		}	
	}
}
...
WORD NVSRAM_Rd_StationName_Total() 
{
	...
	for(i=0;i<sCnt;i++)
	{
		nData = MAKE_WORD(pSt_NameAll->pBody[i].nCode_b[0],pSt_NameAll->pBody[i].nCode_b[1]);
		...
		StationNameAll->StaName[i].StationCode = WORD_L(nCodeData);
		for(j = 0;j<15;j++)
		{
			StationNameAll->StaName[i].EngName[j] =  WORD_L(pSt_NameAll->pBody[i].nKorName_b[j]);	
		}

		for(j = 0;j<30;j++)
		{
			StationNameAll->StaName[i].IranName[j] = WORD_L(pSt_NameAll->pBody[i].nEngName_b[j]);	
		}
		//memcpy(StationNameAll->StaName[i].EngName, pSt_NameAll->pBody[i].nKorName_b ,15);
		//memcpy(StationNameAll->StaName[i].IranName ,pSt_NameAll->pBody[i].nEngName_b ,30);
...
UCHAR StationCode_Definite(UCHAR Now,UCHAR Next)
{
	...
	for(Code=0;Code<256;Code++)
	{
		...
		{
			if(WORD_L(nNvsram_Variable->nMatchCode[Now])==WORD_L(nNvsram_Variable->nMatchCode[Code]) && (WORD_L(nNvsram_Variable->nMatchCode[Now]) != 0xFF)){Re_Data++;}
			if(WORD_L(nNvsram_Variable->nMatchCode[Next])==WORD_L(nNvsram_Variable->nMatchCode[Code])&& (WORD_L(nNvsram_Variable->nMatchCode[Next]) != 0xFF)){Re_Data++;}
		}
	}
...
UCHAR NVSRAM_Find_RootAllNo(WORD nLen,UCHAR *pData)
{
	...
	/*pDest_D->pBody = (DEST_INFO_B *)calloc(sDestAllCntNum, sizeof(DEST_INFO_B));  // 동적 배열 할당 소스*/
	...
	memset(d_DestBuf,0x00,20);
	for(i=0;i<sDestAllCntNum;i++)
	{
		pData[i] = MAKE_WORD(pDest_D->pBody[i].nName_Index_b[0],pDest_D->pBody[i].nName_Index_b[1]);
	}
...
WORD NVSRAM_Rd_StationName_Code(UCHAR Code,UCHAR *pData,UCHAR *pGpData) 
{
	...
	memset(pData,0,15);							// 차내 버퍼 영역 초기화 
	memset(pGpData,0,15);							// Gp 버터 영역 초기화

	//if(Code && WORD_L(Code) == WORD_L(StationNameAll[Code].StationCode))
	if(WORD_L(Code) == WORD_L(StationNameAll->StaName[Code].StationCode))
	{
		for(i=0;i<15;i++)
		{ 
			if(WORD_L(StationNameAll->StaName[Code].EngName[i]) != 0x00) 
			{
				pData[k++] =WORD_L(StationNameAll->StaName[Code].EngName[i]);
		...
		for(i=0;i<15;i++)
		{
			if(WORD_L(StationNameAll->StaName[Code].EngName[i]) != 0x00) 
			{
				pGpData[n++] =WORD_L(StationNameAll->StaName[Code].EngName[i]);
...
WORD NVSRAM_Comp_TrainNo(UCHAR sData) // No : 열차번호
{
	...
	sCnt = MAKE_WORD(WORD_L(pDia_g->nCnt.nNum_b[0]),WORD_L(pDia_g->nCnt.nNum_b[1]));

	nNvsram_Variable->nStop_Patten_Num = 0;

	for(i=0;i<sCnt;i++){
		sDiaCodeNum = MAKE_WORD(WORD_L(pDia_g->pBody[i].nCode_b[0]),WORD_L(pDia_g->pBody[i].nCode_b[1]));
		if((sData/1000) == (sDiaCodeNum/1000)){
			sAdd_Point_2 = MAKE_DWORD(WORD_L(pDia_g->pBody[i].nAddress_b[0]),WORD_L(pDia_g->pBody[i].nAddress_b[1]),
			                          WORD_L(pDia_g->pBody[i].nAddress_b[2]),WORD_L(pDia_g->pBody[i].nAddress_b[3]));
			pDia_g_i = (DIA_G_ITEM *)NVSRAM_ADD(sAdd_Point_2); //DIA 그룹 ITEM
			sCnt_2 = MAKE_WORD(WORD_L(pDia_g_i->nCnt.nNum_b[0]),WORD_L(pDia_g_i->nCnt.nNum_b[1]));
			for(l =0;l<sCnt_2;l++){
				if(MAKE_WORD(WORD_L(pDia_g_i->pBody[l].nCode_b[0]),WORD_L(pDia_g_i->pBody[l].nCode_b[1])) == DWORD_L(sData)) //열차 번호 비교.
				{
					nDisplayIndex.nStart_Time_H = WORD_L(pDia_g_i->pBody[l].nSt_Time_H);
					nDisplayIndex.nStart_Time_M= WORD_L(pDia_g_i->pBody[l].nSt_Time_M);
					nDisplayIndex.nStart_Time_S= WORD_L(pDia_g_i->pBody[l].nSt_Time_S);
					
					nStart_Address_Point.nDisp_G_Address = MAKE_DWORD(pDia_g_i->pBody[l].nDispG_Addres_b[0],pDia_g_i->pBody[l].nDispG_Addres_b[1],
						                                              pDia_g_i->pBody[l].nDispG_Addres_b[2],pDia_g_i->pBody[l].nDispG_Addres_b[3]);

					d_abcd = nStart_Address_Point.nSt_Pt_G_Address = MAKE_DWORD(pDia_g_i->pBody[l].nStopPtG_Addres_b[0],pDia_g_i->pBody[l].nStopPtG_Addres_b[1],
																	            pDia_g_i->pBody[l].nStopPtG_Addres_b[2],pDia_g_i->pBody[l].nStopPtG_Addres_b[3]);
...
UCHAR NVSRAM_Find_StationRoot(WORD ScreenNo,WORD nLen) 	// No 역명 읽기 
{
	...
	for(i=ScreenNo*10;i<ScreenNo*10+nLen;i++)			// i = 1이상일것 
	{
		...
		if(WORD_L(Data) != 0xFF)
		{
			...
			if(WORD_L(Data) == WORD_L(StationNameAll->StaName[Data].StationCode) )
			{
				
				l=0;
				memcpy(nTemp,StationNameAll->StaName[Data].EngName,15);
