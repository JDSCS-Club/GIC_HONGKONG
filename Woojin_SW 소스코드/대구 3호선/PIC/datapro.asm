...
;******************************************************************************
;	c:\lang\tms320c3x\511\bin\ac30.exe datapro.c C:\Users\JDS_RE~1\AppData\Local\Temp\datapro.if 
	.file	"datapro.c"
	.file	"string.h"
...
; 1203 |         //14_06_30 ÃâÀÔ¹® Á¤º¸¸¦ ÆíÁý µ¥ÀÌÅ¸·Î ÅëÀÏ ÇÑ´Ù.              
; 1204 |         //if(WORD_L(nTmsRxData.nDoOpenIf)&0x03){  //TCMS Åë½ÅÀ¸·Î ¼ö½ÅÇ
;     | Ñ ÃâÀÔ¹® Á¤º¸°¡ Á¤»óÀÏ °æ¿ì  //120820                                  
; 1205 |         //      nDisplayIndex.nDoor = nTmsRxData.nDoOpenIf; } //ÃâÀÔ¹®
;     | Á¤º¸¸¦ TCMS Åë½Å ¼ö½Å µ¥ÀÌÅ¸·Î º¯°æ ÇÑ´Ù.                              
;----------------------------------------------------------------------
        ldiu      255,r0
...
;*      Branch Occurs to ir0 
	...
	.sect	".cinit"
	.field  	1,32
	.field  	_d_MC1_Flag+0,32
	.field  	1,32		; _d_MC1_Flag @ 0

	.sect	".text"

	.global	_d_MC1_Flag
	.bss	_d_MC1_Flag,1
	.sym	_d_MC1_Flag,_d_MC1_Flag,12,2,32

	.sect	".cinit"
	.field  	1,32
	.field  	_d_MC2_Flag+0,32
	.field  	1,32		; _d_MC2_Flag @ 0

	.sect	".text"

	.global	_d_MC2_Flag
	.bss	_d_MC2_Flag,1
	.sym	_d_MC2_Flag,_d_MC2_Flag,12,2,32
	.sect	 ".text"
...
_DP_SCC1_A_RX_PRO:
	...
	.line	1
;----------------------------------------------------------------------
; 1985 | void DP_SCC1_A_RX_PRO(UCHAR *pRxData)                                  
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      621,sp
...
;----------------------------------------------------------------------
; 1988 | char szBuf[512];                                                       
; 1989 | char szBuf2[100];                                                      
;----------------------------------------------------------------------
	.line	6
;----------------------------------------------------------------------
; 1990 | int sdebugFlag = 0;                                                    
;----------------------------------------------------------------------
        ldiu      613,ir0
        ldiu      0,r0
        sti       r0,*+fp(ir0)
...
;----------------------------------------------------------------------
; 1994 | UINT sDist = 0;                                                        
; 1996 | UCHAR sNowSt; // ÇöÀç¿ª                                                
; 1997 | UCHAR sNextSt; //´ÙÀ½¿ª                                                
; 1998 | UCHAR sDestSt; // Çà¼±¿ª                                               
;----------------------------------------------------------------------
        ldiu      617,ir0
        sti       r0,*+fp(ir0)
	.line	16
;----------------------------------------------------------------------
; 2000 | PTMS_RX_DATA pTmsRxData = (TMS_RX_DATA *) pRxData;                     
;----------------------------------------------------------------------
        ldiu      621,ir0
        ldiu      *-fp(2),r0
        sti       r0,*+fp(ir0)
	.line	19
;----------------------------------------------------------------------
; 2003 | sTrNUM = BCD_BIN(MAKE_WORD(pTmsRxData->nCarTrainNum[0],pTmsRxData->nCar
;     | TrainNum[1]));                                                         
;----------------------------------------------------------------------
        ldiu      621,ir1
        ldiu      *+fp(ir1),ar0
	...
	.line	21
;----------------------------------------------------------------------
; 2005 | sNowSt = MAKE_WORD(pTmsRxData->nNowCode[0],pTmsRxData->nNowCode[1]);   
;----------------------------------------------------------------------
        ldiu      621,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      621,ir1
	...
	.line	22
;----------------------------------------------------------------------
; 2006 | sNextSt = MAKE_WORD(pTmsRxData->nNextCode[0],pTmsRxData->nNextCode[1]);
;----------------------------------------------------------------------
        ldiu      621,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      *+ar0(30),r0
	...
	.line	23
;----------------------------------------------------------------------
; 2007 | sDestSt = MAKE_WORD(pTmsRxData->nDestCode[0],pTmsRxData->nDestCode[1]);
;----------------------------------------------------------------------
        ldiu      621,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      *+ar0(24),r0
...
;----------------------------------------------------------------------
	...
	.line	27
;----------------------------------------------------------------------
; 2011 | pTmsTxData->nComandCode = 0x20;                                        
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      32,r0
        sti       r0,*+ar0(1)
	.line	29
;----------------------------------------------------------------------
; 2013 | pTmsTxData->nNowCode[0] = pTmsRxData->nNowCode[0];                     
;----------------------------------------------------------------------
        ldiu      621,ir0
        ldiu      *+fp(ir0),ar0
	...
	.line	30
;----------------------------------------------------------------------
; 2014 | pTmsTxData->nNowCode[1] = pTmsRxData->nNowCode[1];                     
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar1(27),r0
	...
	.line	32
;----------------------------------------------------------------------
; 2016 | pTmsTxData->nNextCode[0] = pTmsRxData->nNextCode[0];                   
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar1(30),r0
	...
	.line	33
;----------------------------------------------------------------------
; 2017 | pTmsTxData->nNextCode[1] = pTmsRxData->nNextCode[1];                   
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar1(31),r0
	...
	.line	35
;----------------------------------------------------------------------
; 2019 | pTmsTxData->nDestCode[0] = pTmsRxData->nDestCode[0];                   
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar1(24),r0
	...
	.line	36
;----------------------------------------------------------------------
; 2020 | pTmsTxData->nDestCode[1] = pTmsRxData->nDestCode[1];                   
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar1(25),r0
	...
	.line	38
;----------------------------------------------------------------------
; 2022 | pTmsTxData->nCarTrainNum[0] = pTmsRxData->nCarTrainNum[0];             
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar1(22),r0
	...
	.line	39
;----------------------------------------------------------------------
; 2023 | pTmsTxData->nCarTrainNum[1] = pTmsRxData->nCarTrainNum[1];             
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar1(23),r0
	...
	.line	41
;----------------------------------------------------------------------
; 2025 | pTmsTxData->nDistance[0] = pTmsRxData->nDistance[0];                   
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar1(28),r0
	...
	.line	42
;----------------------------------------------------------------------
; 2026 | pTmsTxData->nDistance[1] = pTmsRxData->nDistance[1];                   
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar1(29),r0
	...
	.line	44
;----------------------------------------------------------------------
; 2028 | d_TcmsRxInfo_Door = nTr_St_Info.nTcmsRxDoorFlag = (pTmsRxData->nTrain_I
...
;----------------------------------------------------------------------
	...
	.line	65
;----------------------------------------------------------------------
; 2049 | nSelf_Test.nSELF_ICR_SELECT = 1;                                       
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nSelf_Test+185
	.line	69
;----------------------------------------------------------------------
; 2053 | if(nDi_Check.nHcr.nFlag)                                               
;----------------------------------------------------------------------
        ldi       @_nDi_Check+0,r0
        beq       L548
;*      Branch Occurs to L548 
	.line	71
;----------------------------------------------------------------------
; 2055 | nTmsRxData.nDataDefin = 0x31;//pTmsRxData->nPiscFault.MC1_PiscF;       
;----------------------------------------------------------------------
        ldiu      49,r0
        sti       r0,@_nTmsRxData+0
	.line	73
;----------------------------------------------------------------------
; 2057 | pTmsTxData->nS_Master_Self_union.nS_Master_Self_BIT.MasterBit = 1;     
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      128,r0
        or        *+ar0(21),r0
	...
	.line	75
;----------------------------------------------------------------------
; 2059 | if(!WORD_L(nTr_St_Info.nPowerOnFalg))                                  
;----------------------------------------------------------------------
        ldiu      255,r0
...
;----------------------------------------------------------------------
	...
	.line	78
;----------------------------------------------------------------------
; 2062 | nTr_St_Info.nSelf_Test_Flag = TRUE; //Àü¿øÀÌ ONµÇ¸é ÀÚ±â Áø´ÜÀ» ½Ç½Ã ÇÑ
;     | ´Ù.                                                                    
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nTr_St_Info+368
...
L548:        
	.line	85
;----------------------------------------------------------------------
; 2069 | else if(!nDi_Check.nHcr.nFlag && (pTmsRxData->nS_PiscFault.MC1_PiscF ||
;     |  pTmsRxData->nS_PiscFault.MC2_PiscF))                                  
; 2071 |         //if(d_MC1_Flag && pTmsRxData->nS_PiscFault.MC2_PiscF)         
; 2072 |         //if(d_MC2_Flag && pTmsRxData->nS_PiscFault.MC1_PiscF)         
; 2073 |         //{                                                            
;----------------------------------------------------------------------
        ldi       @_nDi_Check+0,r0
        bne       L553
...
L551:        
	.line	91
;----------------------------------------------------------------------
; 2075 | nTmsRxData.nDataDefin = 0x33; // ¹é¾÷ µ¿ÀÛ½Ã µ¿ÀÛ ÄÚµå                 
;----------------------------------------------------------------------
        ldiu      51,r0
        sti       r0,@_nTmsRxData+0
	.line	93
;----------------------------------------------------------------------
; 2077 | pTmsTxData->nS_Master_Self_union.nS_Master_Self_BIT.MasterBit = 1;     
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      128,r0
        or        *+ar0(21),r0
	...
	.line	95
;----------------------------------------------------------------------
; 2079 | if(!WORD_L(nTr_St_Info.nPowerOnFalg))                                  
;----------------------------------------------------------------------
        ldiu      255,r0
        tstb      @_nTr_St_Info+378,r0
        bne       L554
...
;----------------------------------------------------------------------
	...
	.line	98
;----------------------------------------------------------------------
; 2082 | nTr_St_Info.nSelf_Test_Flag = TRUE; //Àü¿øÀÌ ONµÇ¸é ÀÚ±â Áø´ÜÀ» ½Ç½Ã ÇÑ
;     | ´Ù.                                                                    
; 2085 | //}                                                                    
; 2088 | else                                                                   
;----------------------------------------------------------------------
...
L553:        
	.line	106
;----------------------------------------------------------------------
; 2090 | nTmsRxData.nDataDefin = 0x34;  //¸¶½ºÅÍ°¡ ¾Æ´Ñ °÷Àº ´ë±â »óÅÂ·Î ÀÖ´Â´Ù.
; 2092 | //pTmsTxData->nS_Master_Self_union.UCHAR = 0x01;                       
;----------------------------------------------------------------------
        ldiu      52,r0
        sti       r0,@_nTmsRxData+0
	.line	110
;----------------------------------------------------------------------
; 2094 | pTmsTxData->nS_Master_Self_union.nS_Master_Self_BIT.selfBit = 1; // ÀÓ¼
...
;----------------------------------------------------------------------
	...
	.line	112
;----------------------------------------------------------------------
; 2096 | pTmsTxData->nS_Master_Self_union.nS_Master_Self_BIT.MasterBit = 0;     
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar0(21),r0
        andn      128,r0
...
L554:        
	.line	117
;----------------------------------------------------------------------
; 2101 | nTmsRxData.nTrainKind = pTmsRxData->nTrainKind; // Â÷·® Á¾º° ÄÚµå Çö½Ã 
;----------------------------------------------------------------------
        ldiu      621,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      *+ar0(21),r0
	...
	.line	119
;----------------------------------------------------------------------
; 2103 | pTmsTxData->nTrainKind = pTmsRxData->nTrainKind;                       
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar0
        ldiu      @_pTmsTxData+0,ar1
        ldiu      *+ar0(21),r0
	...
	.line	123
;----------------------------------------------------------------------
; 2107 | if((WORD_L(nTmsRxData.nDataDefin) >= 0x31) && (WORD_L(nTmsRxData.nDataD
;     | efin) <= 0x34))                                                        
;----------------------------------------------------------------------
        ldiu      255,r0
        and       @_nTmsRxData+0,r0
...
;*      Branch Occurs to L615 
	.line	125
;----------------------------------------------------------------------
; 2109 | nTr_St_Info.nMasterOffCnt = 0;                                         
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nTr_St_Info+376
	.line	127
;----------------------------------------------------------------------
; 2111 | if((WORD_L(nTmsRxData.nDataDefin) == 0x31) || (WORD_L(nTmsRxData.nDataD
;     | efin) == 0x33))                                                        
;----------------------------------------------------------------------
        ldiu      255,r0
        and       @_nTmsRxData+0,r0
...
L558:        
	.line	129
;----------------------------------------------------------------------
; 2113 | nTr_St_Info.nMasterFlag = TRUE;                                        
; 2117 | //Master¿¡¼­ ÀÚ±â Áø´Ü ½ÃÇè ±â´É È°¼ºÈ­ ÇÏ´Â ºÎºÐ.                     
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nTr_St_Info+372
	.line	134
;----------------------------------------------------------------------
; 2118 | if(pTmsRxData->nTest10_11.TEST_BIT.B0_TR)                              
;----------------------------------------------------------------------
        ldiu      621,ir0
        ldiu      *+fp(ir0),ar0
        tstb      *+ar0(11),r0
...
;*      Branch Occurs to L565 
	.line	136
;----------------------------------------------------------------------
; 2120 | pTmsTxData->nTest07_08.TEST_BIT.B0_TR = pTmsRxData->nTest10_11.TEST_BIT
;     | .B0_TR;                                                                
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      1,r1
	...
	.line	138
;----------------------------------------------------------------------
; 2122 | if(pTmsRxData->nTest10_11.TEST_BIT.B7_SR && pTmsRxData->nTest10_11.nTex
;     | tCode)                                                                 
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar0
        ldiu      128,r0
...
;*      Branch Occurs to L569 
	.line	141
;----------------------------------------------------------------------
; 2125 | pTmsTxData->nTest07_08.TEST_BIT.B7_SR = pTmsRxData->nTest10_11.TEST_BIT
;     | .B7_SR;                                                                
;----------------------------------------------------------------------
        ldiu      128,r0
        ldiu      *+fp(ir0),ar1
	...
	.line	142
;----------------------------------------------------------------------
; 2126 | pTmsTxData->nTest07_08.nTextCode = pTmsRxData->nTest10_11.nTextCode;   
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar1(12),r0
	...
	.line	144
;----------------------------------------------------------------------
; 2128 | if((nTr_St_Info.nTestCheckRemem != pTmsRxData->nTest10_11.nTextCode) &&
...
;*      Branch Occurs to L564 
	.line	147
;----------------------------------------------------------------------
; 2131 | nTr_St_Info.nSelf_Test_Flag = TRUE;                                    
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nTr_St_Info+368
L564:        
	.line	150
;----------------------------------------------------------------------
; 2134 | nTr_St_Info.nTestCheckRemem =  pTmsRxData->nTest10_11.nTextCode;       
;----------------------------------------------------------------------
        ldiu      621,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      *+ar0(12),r0
...
L565:        
	.line	154
;----------------------------------------------------------------------
; 2138 | else if(pTmsRxData->nS_DIS_ORDER_UCHAR.nS_DIS_ORDER_BIT.nTest_7) // TCM
;     | S °¡ ÆÐÅÏ Å×½ºÆ® ¸í·ÉÀ» ³»¸°´Ù.                                        
;----------------------------------------------------------------------
        ldiu      621,ir0
        ldiu      *+fp(ir0),ar0
...
L567:        
	.line	160
;----------------------------------------------------------------------
; 2144 | pTmsTxData->nTest07_08.TEST_BIT.B7_SR = 0x00;                          
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar0(8),r0
        andn      128,r0
	...
	.line	161
;----------------------------------------------------------------------
; 2145 | pTmsTxData->nTest07_08.TEST_BIT.B0_TR = 0x00;                          
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar0(8),r0
        andn      1,r0
	...
	.line	162
;----------------------------------------------------------------------
; 2146 | pTmsTxData->nTest07_08.nTextCode = 0x00;                               
; 2149 | else                                                                   
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      0,r0
...
L568:        
	.line	167
;----------------------------------------------------------------------
; 2151 | nTr_St_Info.nMasterFlag = FALSE;                                       
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nTr_St_Info+372
L569:        
	.line	170
;----------------------------------------------------------------------
; 2154 | if(WORD_L(nTr_St_Info.nAutoManuFlag)){} //¼öµ¿ ¼³Á¤ÀÌ¸é~~~             
; 2155 | else                                                                   
; 2157 |         //ÇöÀç¿ª, ´ÙÀ½¿ªÀÌ "0" ÀÌ¸é ÀÚµ¿ ¿îÀüÀ» ÇØÁ¦ ÇÑ´Ù.             
;----------------------------------------------------------------------
        ldiu      255,r0
        tstb      @_nTr_St_Info+374,r0
        bne       L591
;*      Branch Occurs to L591 
	.line	174
;----------------------------------------------------------------------
; 2158 | if((!WORD_L(sNowSt) || !WORD_L(sNextSt)) ||  !DWORD_L(sTrNUM))         
;----------------------------------------------------------------------
        ldiu      618,ir0
        tstb3     *+fp(ir0),r0
        beq       L573
...
L573:        
	.line	176
;----------------------------------------------------------------------
; 2160 | nTr_St_Info.nTcmsAutoDriverFlag = FALSE;                               
; 2162 | else                                                                   
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nTr_St_Info+370
...
L574:        
	.line	180
;----------------------------------------------------------------------
; 2164 | nTr_St_Info.nTcmsAutoDriverFlag = TRUE;                                
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nTr_St_Info+370
	.line	181
;----------------------------------------------------------------------
; 2165 | nTr_St_Info.nAutoDoorFlag = FALSE; // Á¤»óÀûÀ¸·Î ¿­¹øÀÌ ÀÔ·Â µÇ¸é ½ÇÇà
...
;----------------------------------------------------------------------
	...
	.line	182
;----------------------------------------------------------------------
; 2166 | nTr_St_Info.nSimulationFlag = FALSE;                                   
;----------------------------------------------------------------------
        sti       r0,@_nTr_St_Info+67
	.line	184
;----------------------------------------------------------------------
; 2168 | if(WORD_L(sNowSt) == WORD_L(sNextSt))                                  
; 2169 | {//2011_11_10                                                          
; 2170 | //¼­¿ï¿ª ¶Ç´Â ÀÎÃµ°øÇ× ¿ª¿¡¼­ ÇöÀç¿ª,´ÙÀ½¿ªÀº ÀÌÀü ÄÚµå°¡ ¼ö½Åµµ°í ÀÖ°í
;     | , ¿­¹ø, Çà¼± ÄÚµå¸¸ º¯°æ µÇ¸é                                          
; 2171 | //³ë¼± Ç¥½Ã±â´Â ÀÌÀü¿¡ ÂüÀÌ ¿´´ø Á¶°ÇÀ» À¯Áö ÇÏ¿© ³ë¼± Ç¥½Ã±â¿¡ Ç¥Ãâ ÇÏ
;     | °Ô µÈ´Ù.                                                               
; 2172 | //µû¶ó¼­ ÃâÀÔ¹®ÀÌ OPEN µÇ¾úÀ» °æ¿ì ÇöÀç¿ª, ´ÙÀ½¿ª ÄÚµå°¡ º¯°æ µÈ°Í ±îÁö
;     |  È®ÀÎ ÇÏ°í Àü¼Û ÇÑ´Ù.                                                  
;----------------------------------------------------------------------
        ldiu      255,r1
        ldiu      618,ir1
...
;*      Branch Occurs to L576 
	.line	189
;----------------------------------------------------------------------
; 2173 | nSccSendFlag.nRNDTXFlag = FALSE;                                       
; 2179 | //¿­Â÷ ¹øÈ£¿Í Çà¼±Àº ÃâÀÔ¹® Á¤º¸¿Í °ü°è ¾øÀÌ º¯È­ ÇÏ¸é Ç¥ÃâÇÑ´Ù.       
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nSccSendFlag+15
L576:        
	.line	196
;----------------------------------------------------------------------
; 2180 | if(DWORD_L(nSelect_Button.nGPTranNumInput.nNum) != DWORD_L(sTrNUM))  //
...
;*      Branch Occurs to L579 
	.line	199
;----------------------------------------------------------------------
; 2183 | nTr_St_Info.nTcmsAutoDriverFlag = TRUE;//ÀÚµ¿ ¿îÇà ½ÃÀÛ                
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nTr_St_Info+370
	.line	200
;----------------------------------------------------------------------
; 2184 | nSelect_Button.nGPTranNumInput.nNum = DWORD_L(sTrNUM);                 
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),r0
        and       65535,r0
        sti       r0,@_nSelect_Button+1
	.line	201
;----------------------------------------------------------------------
; 2185 | nSccSendFlag.nTranNum = 2;                                             
; 2187 | //sprintf(szBuf2,"-TRNUM Chang Data Tx... : [%d]",sTrNUM); strcat(szBuf
;     | ,szBuf2);                                                              
;----------------------------------------------------------------------
        ldiu      2,r0
        sti       r0,@_nSccSendFlag+1
	.line	204
;----------------------------------------------------------------------
; 2188 | sdebugFlag = TRUE;                                                     
; 2195 | //±ä±Þ ÄÚµå°¡ ¼ö½Å µÇ°í                                                
;----------------------------------------------------------------------
        ldiu      613,ir0
        ldiu      1,r0
        sti       r0,*+fp(ir0)
L579:        
	.line	212
;----------------------------------------------------------------------
; 2196 | if(pTmsRxData->nEmergency15_16.TEST_BIT.B7_ON && WORD_L(pTmsRxData->nEm
...
;*      Branch Occurs to L583 
	.line	214
;----------------------------------------------------------------------
; 2198 | nSccSendFlag.nEmergency_Text.nFlag = TRUE;                             
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nSccSendFlag+10
	.line	216
;----------------------------------------------------------------------
; 2200 | if(nSccSendFlag.nEmergency_Text.nFlagCnt != WORD_L(pTmsRxData->nEmergen
;     | cy15_16.nTextCode))                                                    
;----------------------------------------------------------------------
...
;*      Branch Occurs to L586 
	.line	219
;----------------------------------------------------------------------
; 2203 | nSccSendFlag.nClean = 2;                                               
; 2204 | //Ç¥Ãâ ÇÏ´Â ¹®ÀåÀ» ±â¾ï ÇÏ´Â ºÎºÐ.                                     
;----------------------------------------------------------------------
        ldiu      2,r0
        sti       r0,@_nSccSendFlag+2
	.line	221
;----------------------------------------------------------------------
; 2205 | nSccSendFlag.nEmergency_Text.nFlagCnt = WORD_L(pTmsRxData->nEmergency15
;     | _16.nTextCode);                                                        
; 2206 | //IM ¹®ÀåÀ» Àü¼Û ÇÏ´Â ºÎºÐ.                                            
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar0
        ldiu      255,r0
        and       *+ar0(17),r0
	...
	.line	223
;----------------------------------------------------------------------
; 2207 | nSccSendFlag.nIM_Text.nTxNum = WORD_L(pTmsRxData->nEmergency15_16.nText
...
;----------------------------------------------------------------------
	...
	.line	224
;----------------------------------------------------------------------
; 2208 | nSccSendFlag.nIM_Text.nTxFlag = 1;                                     
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nSccSendFlag+7
	.line	227
;----------------------------------------------------------------------
; 2211 | pTmsTxData->nEmergency03_04.TEST_BIT.B7_ON = 1;                        
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      128,r0
        or        *+ar0(4),r0
	...
	.line	228
;----------------------------------------------------------------------
; 2212 | pTmsTxData->nEmergency03_04.nTextCode = WORD_L(pTmsRxData->nEmergency15
;     | _16.nTextCode);                                                        
;----------------------------------------------------------------------
        ldiu      255,r0
        ldiu      *+fp(ir0),ar0
	...
	.line	230
;----------------------------------------------------------------------
; 2214 | pTmsTxData->nS_DIS_ORDER_UCHAR.nS_DIS_ORDER_BIT.nCall = pTmsRxData->nS_
...
;----------------------------------------------------------------------
	...
	.line	231
;----------------------------------------------------------------------
; 2215 | pTmsTxData->nS_DIS_ORDER_UCHAR.nS_DIS_ORDER_BIT.nOverload = pTmsRxData-
...
;----------------------------------------------------------------------
	...
	.line	232
;----------------------------------------------------------------------
; 2216 | pTmsTxData->nS_DIS_ORDER_UCHAR.nS_DIS_ORDER_BIT.nFire = pTmsRxData->nS_
...
;----------------------------------------------------------------------
	...
	.line	233
;----------------------------------------------------------------------
; 2217 | pTmsTxData->nS_DIS_ORDER_UCHAR.nS_DIS_ORDER_BIT.nSprinkler = pTmsRxData
...
;----------------------------------------------------------------------
	...
	.line	234
;----------------------------------------------------------------------
; 2218 | pTmsTxData->nS_DIS_ORDER_UCHAR.nS_DIS_ORDER_BIT.nEmergency_Brk = pTmsRx
...
;----------------------------------------------------------------------
	...
	.line	236
;----------------------------------------------------------------------
; 2220 | pTmsTxData->nS_DIS_ORDER_UCHAR.nS_DIS_ORDER_BIT.nTest_7 = TRUE;        
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      128,r0
        or        *+ar0(6),r0
...
L583:        
	.line	241
;----------------------------------------------------------------------
; 2225 | else if(nSccSendFlag.nEmergency_Text.nFlag && !WORD_L(pTmsRxData->nEmer
...
;*      Branch Occurs to L586 
	.line	243
;----------------------------------------------------------------------
; 2227 | nSccSendFlag.nEmergency_Text.nFlag = FALSE;                            
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nSccSendFlag+10
	.line	245
;----------------------------------------------------------------------
; 2229 | pTmsTxData->nEmergency03_04.TEST_BIT.B7_ON = 0;                        
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar0(4),r0
        andn      128,r0
	...
	.line	246
;----------------------------------------------------------------------
; 2230 | pTmsTxData->nEmergency03_04.nTextCode = 0;                             
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      0,r0
        sti       r0,*+ar0(5)
	.line	248
;----------------------------------------------------------------------
; 2232 | pTmsTxData->nS_DIS_ORDER_UCHAR.nS_DIS_ORDER_BIT.nCall = FALSE;         
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar0(6),r0
        andn      1,r0
	...
	.line	249
;----------------------------------------------------------------------
; 2233 | pTmsTxData->nS_DIS_ORDER_UCHAR.nS_DIS_ORDER_BIT.nOverload = FALSE;     
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar0(6),r0
        andn      2,r0
	...
	.line	250
;----------------------------------------------------------------------
; 2234 | pTmsTxData->nS_DIS_ORDER_UCHAR.nS_DIS_ORDER_BIT.nFire = FALSE;         
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar0(6),r0
        andn      4,r0
	...
	.line	251
;----------------------------------------------------------------------
; 2235 | pTmsTxData->nS_DIS_ORDER_UCHAR.nS_DIS_ORDER_BIT.nSprinkler = FALSE;    
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar0(6),r0
        andn      8,r0
	...
	.line	252
;----------------------------------------------------------------------
; 2236 | pTmsTxData->nS_DIS_ORDER_UCHAR.nS_DIS_ORDER_BIT.nEmergency_Brk = FALSE;
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar0(6),r0
        andn      16,r0
	...
	.line	254
;----------------------------------------------------------------------
; 2238 | pTmsTxData->nS_DIS_ORDER_UCHAR.nS_DIS_ORDER_BIT.nTest_7 = FALSE;       
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar0(6),r0
        andn      128,r0
...
L586:        
	.line	259
;----------------------------------------------------------------------
; 2243 | if(nSccSendFlag.nEmergency_Text.nRemem != nSccSendFlag.nEmergency_Text.
;     | nFlag)                                                                 
;----------------------------------------------------------------------
...
;*      Branch Occurs to L588 
	.line	261
;----------------------------------------------------------------------
; 2245 | nSccSendFlag.nEmergency_Text.nRemem = nSccSendFlag.nEmergency_Text.nFla
;     | g;                                                                     
;----------------------------------------------------------------------
	...
	.line	263
;----------------------------------------------------------------------
; 2247 | nSccSendFlag.nClean = 2;                                               
; 2250 | ///////////////////////////////////////////////////////////////////////
;     | //////////////////////                                                 
; 2255 | //2011_11_16 nOnlyDestSendFlag                                         
;----------------------------------------------------------------------
        ldiu      2,r0
        sti       r0,@_nSccSendFlag+2
L588:        
	.line	272
;----------------------------------------------------------------------
; 2256 | if(WORD_L(nNvsram_Variable.nMatchCode[WORD_L(sDestSt)]) != WORD_L(nTr_S
...
;*      Branch Occurs to L591 
	.line	274
;----------------------------------------------------------------------
; 2258 | if(WORD_L(nNvsram_Variable.nMatchCode[WORD_L(sDestSt)]) != 0xFF)       
;----------------------------------------------------------------------
        and3      r0,*+fp(ir0),ir0
        and3      r0,*+ar0(ir0),r0
        cmpi      255,r0
...
;*      Branch Occurs to L591 
	.line	276
;----------------------------------------------------------------------
; 2260 | nTr_St_Info.nTcmsAutoDriverFlag = TRUE;//ÀÚµ¿ ¿îÇà ½ÃÀÛ                
; 2261 | //2011_11_16 nOnlyDestSendFlag                                         
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nTr_St_Info+370
	.line	278
;----------------------------------------------------------------------
; 2262 | nTr_St_Info.nOnlyDestSendFlag =  WORD_L(nNvsram_Variable.nMatchCode[WOR
;     | D_L(sDestSt)]);                                                        
;----------------------------------------------------------------------
        ldiu      620,ir0
        ldiu      255,r0
	...
	.line	279
;----------------------------------------------------------------------
; 2263 | nSccSendFlag.nDeSTCodeSet = 2;                                         
; 2265 | //2011_11_16 nOnlyDestSendFlag                                         
; 2266 | //sprintf(szBuf2,"-FDI Chang Data Tx... : [%02X]H",WORD_L(nTr_St_Info.n
...
;----------------------------------------------------------------------
	...
	.line	283
;----------------------------------------------------------------------
; 2267 | sdebugFlag = TRUE;                                                     
; 2295 | //µð¹ö±ë                                                               
; 2296 | //if(sdebugFlag) {sdebugFlag = FALSE; strcat(szBuf,"\r\n"); user_DebugP
;     | rint(szBuf); }                                                         
; 2301 | //ÃâÀÔ¹®ÀÌ ¿­·Á ÀÖÀ»¶§¹® °Ë»ç ÇÑ´Ù.                                    
;----------------------------------------------------------------------
        ldiu      613,ir0
        ldiu      1,r0
        sti       r0,*+fp(ir0)
L591:        
	.line	318
;----------------------------------------------------------------------
; 2302 | if((nDi_Check.nDoor.nFlag||nTr_St_Info.nTcmsRxDoorFlag) && !WORD_L(nTr_
...
;*      Branch Occurs to L607 
	.line	321
;----------------------------------------------------------------------
; 2305 | if(WORD_L(StationCode_Definite(WORD_L(sNowSt),WORD_L(sNextSt))) >= 2 )
...
;*      Branch Occurs to L605 
	.line	324
;----------------------------------------------------------------------
; 2308 | nTmsRxData.nDoOpenIf = ((pTmsRxData->nTrain_Info18_19.TEST_BIT.nDoorOpe
;     | nA <<1) |                                                              
; 2309 |                             (pTmsRxData->nTrain_Info18_19.TEST_BIT.nDoo
;     | rOpenB))&0x03; //120820 ÃâÀÔ¹® Á¤º¸´Â µµ¾î OPEN ÀÏ °æ¿ì TCMS Á¤º¸·Î °è¼
;     | Ó °»½Å ÇÑ´Ù.                                                           
; 2310 | //¿­¹øÀÌ º¯°æ µÇ¾úÀ» °æ¿ì                                              
;----------------------------------------------------------------------
        ldiu      621,ir0
        ldiu      621,ir1
        ldiu      16,r0
	...
	.line	327
;----------------------------------------------------------------------
; 2311 | if(DWORD_L(nSelect_Button.nGPTranNumInput.nRemNum) != DWORD_L(nSelect_B
...
;*      Branch Occurs to L599 
	.line	329
;----------------------------------------------------------------------
; 2313 | if(DWORD_L(nSelect_Button.nGPTranNumInput.nNum))                       
;----------------------------------------------------------------------
        ldiu      @_nSelect_Button+1,r0
        tstb      65535,r0
        beq       L598
;*      Branch Occurs to L598 
	.line	331
;----------------------------------------------------------------------
; 2315 | nDi_Check.nDoor.nFlagCnt = 0;                                          
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nDi_Check+18
	.line	333
;----------------------------------------------------------------------
; 2317 | nSccSendFlag.nTranNum = 2; //¿­¹øÀÌ º¯°æ µÇ¸é ¿­¹øÀ» º¯°æÇÑ´Ù.         
;----------------------------------------------------------------------
        ldiu      2,r0
        sti       r0,@_nSccSendFlag+1
	.line	335
;----------------------------------------------------------------------
; 2319 | nSelect_Button.nGPTranNumInput.nRemNum = nSelect_Button.nGPTranNumInput
;     | .nNum = sTrNUM;                                                        
;----------------------------------------------------------------------
        ldiu      614,ir0
        ldiu      *+fp(ir0),r0
	...
	.line	337
;----------------------------------------------------------------------
; 2321 | nSelect_Button.nGPTranNumInput.nFlag = TRUE;  //¿­¹ø º¯°æ¿¡ µû¸¥ ¿­¹øÀ»
;     |  ´Ù½Ã °Ë»ö ÇÑ´Ù. 2010_07_31                                            
; 2323 | //sprintf(szBuf2,"-DIA FIND : [%d]",DWORD_L(nSelect_Button.nGPTranNumIn
...
;----------------------------------------------------------------------
	...
	.line	340
;----------------------------------------------------------------------
; 2324 | sdebugFlag = TRUE;                                                     
; 2326 | else{//¿­¹ø ÀÔ·ÂÀ» "0"À¸·Î ÇßÀ»¶§.                                     
;----------------------------------------------------------------------
        ldiu      613,ir0
        sti       r0,*+fp(ir0)
        bu        L599
...
L598:        
	.line	344
;----------------------------------------------------------------------
; 2328 | nTr_St_Info.nStopPatNum = FALSE;                                       
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nTr_St_Info+0
	.line	345
;----------------------------------------------------------------------
; 2329 | memset(nTr_St_Info.nGpStName.nNow,0x20,15);                            
;----------------------------------------------------------------------
        ldiu      32,r2
        ldiu      @CL8,r1
        ldiu      15,r0
	...
	.line	346
;----------------------------------------------------------------------
; 2330 | memset(nTr_St_Info.nGpStName.nNext,0x20,15);                           
;----------------------------------------------------------------------
        ldiu      15,r1
        ldiu      32,r2
        ldiu      @CL11,r0
	...
	.line	347
;----------------------------------------------------------------------
; 2331 | memset(nTr_St_Info.nGpStName.nDest,0x20,15);                           
;----------------------------------------------------------------------
        ldiu      15,r1
        push      r1
        ldiu      32,r2
...
L599:        
	.line	353
;----------------------------------------------------------------------
; 2337 | if(nTr_St_Info.nStopPatNum && //¿­Â÷ ¹øÈ£°¡ À¯È¿ ÇÏ´Ù¸é...             
; 2338 |         (WORD_L(nSelect_Button.nGPTranNumInput.nNowSt_Re) != WORD_L(sNo
;     | wSt) ||                                                                
; 2339 |         WORD_L(nSelect_Button.nGPTranNumInput.nNextSt_Re) != WORD_L(sNe
;     | xtSt)))                                                                
;----------------------------------------------------------------------
        ldi       @_nTr_St_Info+0,r0
        beq       L606
...
L602:        
	.line	357
;----------------------------------------------------------------------
; 2341 | d_CarNum1 = nSelect_Button.nGPTranNumInput.nNowSt_Re = WORD_L(sNowSt); 
;----------------------------------------------------------------------
        ldiu      618,ir0
        ldiu      255,r0
        and3      r0,*+fp(ir0),r0
	...
	.line	358
;----------------------------------------------------------------------
; 2342 | d_CarNumRem1 = nSelect_Button.nGPTranNumInput.nNextSt_Re = WORD_L(sNext
;     | St);                                                                   
;----------------------------------------------------------------------
        ldiu      619,ir0
        ldiu      255,r0
	...
	.line	360
;----------------------------------------------------------------------
; 2344 | if(WORD_L(nSelect_Button.nGPTranNumInput.nNowSt_Re) != WORD_L(nSelect_B
...
;----------------------------------------------------------------------
	...
	.line	364
;----------------------------------------------------------------------
; 2348 | nTr_St_Info.nAutoDoorFlag = FALSE; // Á¤»óÀûÀ¸·Î ¿­¹øÀÌ ÀÔ·Â µÇ¸é ½ÇÇà
;     | ÁßÀÌ´ø ½Ã¹Ä·¹ÀÌ¼Ç ÁßÁö.                                                
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nTr_St_Info+70
	.line	365
;----------------------------------------------------------------------
; 2349 | nTr_St_Info.nSimulationFlag = FALSE;                                   
;----------------------------------------------------------------------
        sti       r0,@_nTr_St_Info+67
	.line	367
;----------------------------------------------------------------------
; 2351 | nDi_Check.nDoor.nFlagCnt = 0;                                          
;----------------------------------------------------------------------
        sti       r0,@_nDi_Check+18
	.line	369
;----------------------------------------------------------------------
; 2353 | nTmsRxData.nNow = WORD_L(nNvsram_Variable.nMatchCode[WORD_L(sNowSt)]); 
;----------------------------------------------------------------------
        ldiu      618,ir0
        ldiu      @CL3,ar0
        ldiu      255,r0
	...
	.line	370
;----------------------------------------------------------------------
; 2354 | nTmsRxData.nNext = WORD_L(nNvsram_Variable.nMatchCode[WORD_L(sNextSt)])
;     | ;                                                                      
;----------------------------------------------------------------------
        ldiu      619,ir0
        ldiu      255,r0
	...
	.line	372
;----------------------------------------------------------------------
; 2356 | nTr_St_Info.nDoOp_StCh = TRUE;                                         
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nTr_St_Info+371
	.line	373
;----------------------------------------------------------------------
; 2357 | nTr_St_Info.nRxCode_NoNe_Flag = TRUE;                                  
; 2359 | //sprintf(szBuf2,"-Door Open St-St Code RX OK : Now[%02X]H : Next[%02X]
;     | H ",WORD_L(nTmsRxData.nNow),WORD_L(nTmsRxData.nNext));                 
; 2360 | //strcat(szBuf,szBuf2);                                                
;----------------------------------------------------------------------
        sti       r0,@_nTr_St_Info+373
	.line	377
;----------------------------------------------------------------------
; 2361 | sdebugFlag = TRUE;                                                     
; 2367 | else                                                                   
;----------------------------------------------------------------------
        ldiu      613,ir0
        sti       r0,*+fp(ir0)
...
;*      Branch Occurs to L606 
	.line	385
;----------------------------------------------------------------------
; 2369 | nTr_St_Info.nRxCode_NoNe_Flag = FALSE;                                 
; 2371 | //sprintf(szBuf2,"-Door Open St-St Code RX NG-NG : Now[%02X]H : Next[%0
;     | 2X]H ",WORD_L(sNowSt),WORD_L(sNextSt));                                
; 2372 | //strcat(szBuf,szBuf2);                                                
;----------------------------------------------------------------------
L605:        
        sti       r0,@_nTr_St_Info+373
	.line	389
;----------------------------------------------------------------------
; 2373 | sdebugFlag = TRUE;                                                     
; 2376 | //if(sdebugFlag) {sdebugFlag = FALSE; strcat(szBuf,"\r\n"); user_DebugP
;     | rint(szBuf); }                                                         
;----------------------------------------------------------------------
        ldiu      613,ir0
        ldiu      1,r0
...
L606:        
	.line	394
;----------------------------------------------------------------------
; 2378 | nTr_St_Info.nDoOp_StChFlag = TRUE; //120820 ÃâÀÔ¹®ÀÌ ¿­¸®°í Á¤»óÀûÀ¸·Î
;     | Åë½ÅÀÌ ÀÌ·ç¾î Á³´Ù.                                                    
; 2379 | }// Â÷·® ÀÌµ¿ °Å¸® °ªÀ» °è»ê ÇÑ´Ù.                                     
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nTr_St_Info+377
        bu        L623
...
L607:        
	.line	396
;----------------------------------------------------------------------
; 2380 | else if(!(nDi_Check.nDoor.nFlag||nTr_St_Info.nTcmsRxDoorFlag) && !WORD_
...
;*      Branch Occurs to L623 
	.line	399
;----------------------------------------------------------------------
; 2383 | sDist = MAKE_WORD(pTmsRxData->nDistance[0],pTmsRxData->nDistance[1]);  
; 2385 | // Â÷·® ¼ö½Å °Å¸® °ª Ã³¸® ¹æ¹ý º¯°æ.                                   
;----------------------------------------------------------------------
        ldiu      621,ir0
        ldiu      621,ir1
        ldiu      *+fp(ir0),ar0
	...
	.line	402
;----------------------------------------------------------------------
; 2386 | if(DWORD_L(sDist) < nDi_Check.nDist.nRemem)                            
; 2388 |         //sDist = 0xFFFF - sDist;                                      
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),r0
        and       65535,r0
...
;*      Branch Occurs to L613 
	.line	405
;----------------------------------------------------------------------
; 2389 | d_dist = nDi_Check.nDist.nFlagCnt = DWORD_L(nDi_Check.nDist.nFlagCnt) +
...
;*      Branch Occurs to L614 
	.line	409
;----------------------------------------------------------------------
; 2393 | d_dist = nDi_Check.nDist.nFlagCnt = DWORD_L(nDi_Check.nDist.nFlagCnt) +
...
L614:        
	.line	413
;----------------------------------------------------------------------
; 2397 | nDi_Check.nDist.nRemem = DWORD_L(sDist);                               
; 2403 | else                                                                   
;----------------------------------------------------------------------
        ldiu      617,ir0
        ldiu      *+fp(ir0),r0
...
L615:        
	.line	421
;----------------------------------------------------------------------
; 2405 | if(WORD_L(nTr_St_Info.nMasterFlag) || WORD_L(nTr_St_Info.nTcmsAutoDrive
;     | rFlag))                                                                
;----------------------------------------------------------------------
        ldiu      255,r0
...
L617:        
	.line	423
;----------------------------------------------------------------------
; 2407 | nTr_St_Info.nMasterOffCnt = 5;                                         
;----------------------------------------------------------------------
        ldiu      5,r0
        sti       r0,@_nTr_St_Info+376
	.line	425
;----------------------------------------------------------------------
; 2409 | memset(&nSccSendFlag,0x00,sizeof(nSccSendFlag));                       
;----------------------------------------------------------------------
        ldiu      17,r1
        ldiu      0,r2
        push      r1
	...
	.line	427
;----------------------------------------------------------------------
; 2411 | nSelect_Button.nGPTranNumInput.nNum = 0;     //¿­Â÷ ¹øÈ£¸¦ Å¬¸®¾î ÇÑ´Ù.
;     |   //2011_05_02                                                         
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nSelect_Button+1
	.line	428
;----------------------------------------------------------------------
; 2412 | nSelect_Button.nGPTranNumInput.nRemNum = 0;  //¿­Â÷ ¹øÈ£ ±â¾ï º¯¼öµµ Å¬
;     | ¸®¾î ÇÑ´Ù.//2011_05_02                                                 
;----------------------------------------------------------------------
        sti       r0,@_nSelect_Button+2
	.line	430
;----------------------------------------------------------------------
; 2414 | nTr_St_Info.nAutoManuFlag = FALSE;//¼öµ¿ ¼³Á¤                          
;----------------------------------------------------------------------
        sti       r0,@_nTr_St_Info+374
	.line	432
;----------------------------------------------------------------------
; 2416 | nTr_St_Info.nSimulationRun = FALSE;                                    
;----------------------------------------------------------------------
        sti       r0,@_nTr_St_Info+69
	.line	434
;----------------------------------------------------------------------
; 2418 | nTr_St_Info.nSimulationFlag = FALSE;                                   
;----------------------------------------------------------------------
        sti       r0,@_nTr_St_Info+67
	.line	435
;----------------------------------------------------------------------
; 2419 | nTr_St_Info.nAutoDoorFlag = FALSE;                                     
;----------------------------------------------------------------------
        sti       r0,@_nTr_St_Info+70
	.line	438
;----------------------------------------------------------------------
; 2422 | nTr_St_Info.nSpeed = 0;                                                
;----------------------------------------------------------------------
        sti       r0,@_nTr_St_Info+71
	.line	439
;----------------------------------------------------------------------
; 2423 | nTr_St_Info.nDistance = 0;                                             
;----------------------------------------------------------------------
        sti       r0,@_nTr_St_Info+74
	.line	441
;----------------------------------------------------------------------
; 2425 | nTr_St_Info.nDoor = 1;                                                 
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nTr_St_Info+72
	.line	443
;----------------------------------------------------------------------
; 2427 | nTr_St_Info.nDoorOPenCnt = 0;                                          
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nTr_St_Info+73
	.line	445
;----------------------------------------------------------------------
; 2429 | nSccSendFlag.nClean = 3; //¹«Ç¥½Ã Àü¼Û                                 
;----------------------------------------------------------------------
        ldiu      3,r0
        sti       r0,@_nSccSendFlag+2
	.line	446
;----------------------------------------------------------------------
; 2430 | nTr_St_Info.nClean_Flag = FALSE; //½Ã¹Ä·¹ÀÌ¼ÇÀÌ ¿Ï·á µÇ¸é ¹«Ç¥½Ã flage°
;     | ¡ FALSE·Î µÈ´Ù.                                                        
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nTr_St_Info+369
	.line	448
;----------------------------------------------------------------------
; 2432 | memset(mSelfTest_NG_CntBuf,0x00,sizeof(mSelfTest_NG_CntBuf));//2011_02_
;     | 17 3.2¹öÀü//                                                           
;----------------------------------------------------------------------
        ldiu      60,r2
        ldiu      @CL19,r1
	...
	.line	450
;----------------------------------------------------------------------
; 2434 | if(nTr_St_Info.nDISFPatFlag)                                           
;----------------------------------------------------------------------
...
;*      Branch Occurs to L619 
	.line	452
;----------------------------------------------------------------------
; 2436 | nTr_St_Info.nDISFPatFlag = FALSE; //³ë¼± Ç¥½Ã±â°¡ ÆÐÅÏ ÁßÀÌ ¿´´Ù¸é Á¤Áö
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nTr_St_Info+66
	.line	453
;----------------------------------------------------------------------
; 2437 | nSccSendFlag.nS_FDIPaten = 2;                                          
;----------------------------------------------------------------------
        ldiu      2,r0
        sti       r0,@_nSccSendFlag+4
L619:        
	.line	456
;----------------------------------------------------------------------
; 2440 | nTr_St_Info.nStopPatNum = FALSE;                                       
; 2442 | //sprintf(szBuf,"-Master Key Off [%02X] \r\n",nTmsRxData.nDataDefin);  
; 2443 | //user_DebugPrint(szBuf);                                              
; 2445 | //DP_SEND_PRO();       //Åë½Å TX ºÎºÐÀ» ÀÌ°÷¿¡¼­ Ã³¸® ÇÑ´Ù.            
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nTr_St_Info+0
L620:        
	.line	465
;----------------------------------------------------------------------
; 2449 | nTr_St_Info.nTcmsAutoDriverFlag = FALSE;                               
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nTr_St_Info+370
	.line	466
;----------------------------------------------------------------------
; 2450 | nTr_St_Info.nMasterFlag = FALSE;                                       
;----------------------------------------------------------------------
        sti       r0,@_nTr_St_Info+372
	.line	470
;----------------------------------------------------------------------
; 2454 | if(WORD_L(nTr_St_Info.nMasterOffCnt))                                  
;----------------------------------------------------------------------
        ldiu      255,r0
...
;*      Branch Occurs to L622 
	.line	472
;----------------------------------------------------------------------
; 2456 | nTr_St_Info.nMasterOffCnt--;                                           
;----------------------------------------------------------------------
        ldiu      1,r0
	...
	.line	473
;----------------------------------------------------------------------
; 2457 | DP_SEND_PRO();       //Åë½Å TX ºÎºÐÀ» ÀÌ°÷¿¡¼­ Ã³¸® ÇÑ´Ù.              
;----------------------------------------------------------------------
        call      _DP_SEND_PRO
                                        ; Call Occurs
L622:        
	.line	477
;----------------------------------------------------------------------
; 2461 | nIndex_Flag.nIndexCnt = 0;                                             
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nIndex_Flag+573
	.line	478
;----------------------------------------------------------------------
; 2462 | nIndex_Flag.nIndexDI_DICnt = 0;                                        
;----------------------------------------------------------------------
        sti       r0,@_nIndex_Flag+572
	.line	479
;----------------------------------------------------------------------
; 2463 | nIndex_Flag.nIndexST_STAllCnt = 0;                                     
;----------------------------------------------------------------------
        sti       r0,@_nIndex_Flag+570
	.line	480
;----------------------------------------------------------------------
; 2464 | nIndex_Flag.nAllCnt_Cnt = 0;                                           
;----------------------------------------------------------------------
        sti       r0,@_nIndex_Flag+571
L623:        
	.line	483
;----------------------------------------------------------------------
; 2467 | pTmsTxData->nETX = 0x03;                                               
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      3,r0
        sti       r0,*+ar0(42)
	.line	485
;----------------------------------------------------------------------
; 2469 | MakeBcc(&m_SCCPISTXBuff_Ach[1],42);                                    
;----------------------------------------------------------------------
        ldiu      42,r1
        push      r1
        ldiu      @CL68,r0
...
_DP_SEND_PRO:
	...
	.line	1
;----------------------------------------------------------------------
; 2482 | void DP_SEND_PRO()                                                     
; 2484 | char szBuf[512];                                                       
; 2485 | char szBuf2[100];                                                      
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      734,sp
	.line	5
;----------------------------------------------------------------------
; 2486 | int sdebugFlag = 0;                                                    
; 2488 | UCHAR sDestTxBuf[20];                                                  
; 2489 | UCHAR sClean_Tx_Buf[20];                                               
; 2490 | UCHAR sSimu_Tx_Buf[20];                                                
; 2491 | UCHAR sAddSet_Tx_Buf[20];                                              
; 2493 | UCHAR sSelf_Tx_Buf[20];                                                
; 2494 | UCHAR sRnd_Tx_Buf[20];                                                 
;----------------------------------------------------------------------
        ldiu      613,ir0
        ldiu      0,r0
        sti       r0,*+fp(ir0)
...
;----------------------------------------------------------------------
	...
	.line	18
;----------------------------------------------------------------------
; 2499 | if(WORD_L(nTr_St_Info.nSelf_Test_Flag)) //ÀÚ±â Áø´Ü ÁøÇàÁßÀÏ¶§ Àü¼ÛÇÑ´Ù
;     | .                                                                      
;----------------------------------------------------------------------
        ldiu      255,r0
        tstb      @_nTr_St_Info+368,r0
...
;*      Branch Occurs to L640 
	.line	20
;----------------------------------------------------------------------
; 2501 | if(pTmsTxData->nTest07_08.TEST_BIT.B0_TR) //½ÃÇè ½ÇÇà ¿ä±¸°¡ µé¿À ¿À¸é
;     | ½ÃÇè°á°ú¸¦ º¸³» ÁØ´Ù.                                                  
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      1,r0
...
;*      Branch Occurs to L629 
	.line	22
;----------------------------------------------------------------------
; 2503 | pTmsTxData->nTest_State = 0x01; // ½ÃÇèÁß.                             
;----------------------------------------------------------------------
        sti       r0,*+ar0(11)
L629:        
	.line	26
;----------------------------------------------------------------------
; 2507 | memcpy(nSelf_Test.nSELF_ADDDATA_BUF,DISPLAY_UNIT_ADD_NOR_PIB,DISPLAY_ST
;     | ANDARD_UNIT);                                                          
; 2508 | //memcpy(&nSelf_Test.nSELF_ADDDATA_BUF[DISPLAY_STANDARD_UNIT_A],DISPLAY
...
||      ldi       *ar0++(1),r0
	...
	.line	28
;----------------------------------------------------------------------
; 2509 | if(nSelf_Test.nSELF_All_Cnt < SELF_TEST_ALLCNT){                       
;----------------------------------------------------------------------
...
;*      Branch Occurs to L631 
	.line	29
;----------------------------------------------------------------------
; 2510 | DP_SelfTest(nSelf_Test.nSELF_ADDDATA_BUF,nSelf_Test.nSELF_RXDATA_Buf,DI
;     | SPLAY_STANDARD_UNIT);    // ÀÏ¹Ý                                       
; 2521 | //if(WORD_L(nSelf_Test.nSELF_All_Cnt) >=SELF_TEST_ALLCNT && nSelf_Test.
...
L631:        
	.line	41
;----------------------------------------------------------------------
; 2522 | if(WORD_L(nSelf_Test.nSELF_All_Cnt) >=SELF_TEST_ALLCNT) //ÀüÃ¥ ¹Ýº¹ È½¼
;     | ö°¡ 3È¸¸¦ ³ÑÀ¸¸é Á¾·á.                                                 
;----------------------------------------------------------------------
        ldiu      255,r0
        and       @_nSelf_Test+183,r0
...
;*      Branch Occurs to L708 
	.line	43
;----------------------------------------------------------------------
; 2524 | nSelf_Test.nSELF_All_Cnt = 0;                                          
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nSelf_Test+183
	.line	44
;----------------------------------------------------------------------
; 2525 | nSelf_Test.nSELF_All_Cnt_B = 0;                                        
;----------------------------------------------------------------------
        sti       r0,@_nSelf_Test+184
	.line	45
;----------------------------------------------------------------------
; 2526 | d_ICR_Select = nSelf_Test.nSELF_ICR_SELECT;                            
;----------------------------------------------------------------------
        ldiu      @_nSelf_Test+185,r0
        sti       r0,@_d_ICR_Select+0
	.line	47
;----------------------------------------------------------------------
; 2528 | nTr_St_Info.nSelf_Test_Flag = FALSE;                                   
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nTr_St_Info+368
	.line	48
;----------------------------------------------------------------------
; 2529 | nLedDataLoad.nSelfTestFlag = FALSE;                                    
;----------------------------------------------------------------------
        sti       r0,@_nLedDataLoad+7
	.line	50
;----------------------------------------------------------------------
; 2531 | DP_TCMSSD_Format(TCMSSD,&m_SCCPISTXBuff_Ach[22]); //TCMS Àü¼Û DATA ¸¦ °
;     | »½Å ÇÑ´Ù.                                                              
;----------------------------------------------------------------------
        ldiu      @CL70,r1
        push      r1
	...
	.line	52
;----------------------------------------------------------------------
; 2533 | if(pTmsTxData->nTest07_08.TEST_BIT.B0_TR) //½ÃÇè ½ÇÇà ¿ä±¸°¡ µé¿À ¿À¸é
;     | ½ÃÇè°á°ú¸¦ º¸³» ÁØ´Ù.                                                  
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      1,r0
        tstb      *+ar0(8),r0
        beq       L634
;*      Branch Occurs to L634 
	.line	54
;----------------------------------------------------------------------
; 2535 | pTmsTxData->nTest_State = 0x02; // ½ÃÇè ¿Ï·á.                          
;----------------------------------------------------------------------
        ldiu      2,r0
        sti       r0,*+ar0(11)
	.line	55
;----------------------------------------------------------------------
; 2536 | pTmsTxData->nS_TEST_RESULT.UCHAR = 0xFF; //½ÃÇè °á°ú 1 = OK / 0 = NG   
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      255,r0
        sti       r0,*+ar0(12)
L634:        
	.line	58
;----------------------------------------------------------------------
; 2539 | if(nTr_St_Info.nPowerOnFalg == 1)                                      
;----------------------------------------------------------------------
...
;----------------------------------------------------------------------
	...
	.line	62
;----------------------------------------------------------------------
; 2543 | if((TCMSSD[0].DR.BYTE &0x03) && (TCMSSD[1].DR.BYTE&0x07) && (TCMSSD[2].
...
;*      Branch Occurs to L639 
	.line	64
;----------------------------------------------------------------------
; 2545 | pTmsTxData->nS_Master_Self_union.nS_Master_Self_BIT.selfBit =  0;      
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      *+ar0(21),r0
        andn      1,r0
...
L639:        
	.line	66
;----------------------------------------------------------------------
; 2547 | else {pTmsTxData->nS_Master_Self_union.nS_Master_Self_BIT.selfBit = 1;
;     | }                                                                      
; 2550 | //sprintf(szBuf2,"#Self Test FINISH");strcat(szBuf,szBuf2);sdebugFlag =
;     |  TRUE;                                                                 
;----------------------------------------------------------------------
        ldiu      @_pTmsTxData+0,ar0
        ldiu      1,r0
...
L640:        
	.line	73
;----------------------------------------------------------------------
; 2554 | else if(WORD_L(nSccSendFlag.nManualSet)) // ¼öµ¿ ¼³Á¤½Ã ¹®Àå Àü¼Û      
;----------------------------------------------------------------------
        ldiu      255,r0
...
;*      Branch Occurs to L645 
	.line	75
;----------------------------------------------------------------------
; 2556 | if(nSccSendFlag.nManualSet == 2)                                       
;----------------------------------------------------------------------
...
;*      Branch Occurs to L643 
	.line	77
;----------------------------------------------------------------------
; 2558 | DP_IM_PR_TEXT_READ(100,1,0xF1); //Â÷³» ¼öµ¿ ¼³Á¤ Ç¥½Ã±â¿¡ 96¹ø Ç¥Ãâ ¹®À
;     | åÀ» Àü¼ÛÇÑ´Ù.                                                          
;----------------------------------------------------------------------
        ldiu      241,r2
        ldiu      1,r0
...
L643:        
	.line	79
;----------------------------------------------------------------------
; 2560 | else if(nSccSendFlag.nManualSet == 1)                                  
; 2562 |         //DP_IM_PR_TEXT_READ(99,1,0xF1);        //Â÷³» ¼öµ¿ ¼³Á¤ Ç¥½Ã±â
;     | ¿¡ 97¹ø Ç¥Ãâ ¹®ÀåÀ» Àü¼ÛÇÑ´Ù.                                          
;----------------------------------------------------------------------
L644:        
	.line	83
;----------------------------------------------------------------------
; 2564 | nSccSendFlag.nManualSet--;                                             
;----------------------------------------------------------------------
        ldiu      1,r0
...
L645:        
	.line	85
;----------------------------------------------------------------------
; 2566 | else if(WORD_L(nSccSendFlag.nDeSTCodeSet) || WORD_L(nSccSendFlag.nTranN
;     | um)) // Á¤¸é Çà¼± Ç¥½Ã±â Ãâ·Â                                          
; 2568 |         //2011_11_16 nOnlyDestSendFlag                                 
;----------------------------------------------------------------------
        ldiu      255,r0
...
L647:        
	.line	88
;----------------------------------------------------------------------
; 2569 | nDest_Num = WORD_L(nTr_St_Info.nOnlyDestSendFlag)+1;                   
;----------------------------------------------------------------------
        ldiu      255,r0
        ldiu      734,ir0
        and       @_nTr_St_Info+367,r0
	...
	.line	90
;----------------------------------------------------------------------
; 2571 | if(nSccSendFlag.nDeSTCodeSet && nSccSendFlag.nTranNum){                
; 2572 |         //SCC_TX_PROTOCOL(sDestTxBuf,0xF8,0x31,0x1C,nDest_Num,BIN_BCD(n
...
;*      Branch Occurs to L653 
	.line	92
;----------------------------------------------------------------------
; 2573 | if(nSccSendFlag.nDeSTCodeSet == 2)                                     
;----------------------------------------------------------------------
...
;*      Branch Occurs to L652 
	.line	94
;----------------------------------------------------------------------
; 2575 | SCC_TX_PROTOCOL(sClean_Tx_Buf,0xF5,0x31,0x1A,0,0,0,0,0,10);            
;----------------------------------------------------------------------
        ldiu      10,r0
        ldiu      0,r1
        push      r0
	...
	.line	95
;----------------------------------------------------------------------
; 2576 | Delay_SCC3_SendTo(sClean_Tx_Buf,20,SCC_B_CHANNEL,5);                   
; 2578 | else                                                                   
;----------------------------------------------------------------------
        ldiu      5,r2
        push      r2
        ...
        subi      4,sp
        bu        L659
;*      Branch Occurs to L659 
	.line	99
;----------------------------------------------------------------------
; 2580 | DP_IM_PR_TEXT_READ(71,1,0xF5);                                         
; 2582 | //sprintf(szBuf2,"#SEND FDI-TRNUM Tx Ok");strcat(szBuf,szBuf2);sdebugFl
;     | ag = TRUE;                                                             
;----------------------------------------------------------------------
L652:        
        push      r2
...
L653:        
	.line	103
;----------------------------------------------------------------------
; 2584 | else if(nSccSendFlag.nDeSTCodeSet)                                     
; 2586 |         //SCC_TX_PROTOCOL(sDestTxBuf,0xF5,0x31,0x1C,nDest_Num,BIN_BCD(n
;     | Select_Button.nGPTranNumInput.nNum),0,0,0,10);                         
;----------------------------------------------------------------------
        ldi       @_nSccSendFlag+0,r0
        beq       L658
;*      Branch Occurs to L658 
	.line	106
;----------------------------------------------------------------------
; 2587 | if(nSccSendFlag.nDeSTCodeSet == 2)                                     
;----------------------------------------------------------------------
        cmpi      2,r0
        bned      L657
        ldine     245,r0
...
;*      Branch Occurs to L657 
	.line	108
;----------------------------------------------------------------------
; 2589 | SCC_TX_PROTOCOL(sClean_Tx_Buf,0xF5,0x31,0x1A,0,0,0,0,0,10);            
;----------------------------------------------------------------------
        ldiu      10,r0
        ldiu      0,r1
        ldiu      0,rs
	...
	.line	109
;----------------------------------------------------------------------
; 2590 | Delay_SCC3_SendTo(sClean_Tx_Buf,20,SCC_B_CHANNEL,5);                   
; 2592 | else                                                                   
;----------------------------------------------------------------------
        ldiu      5,r2
        push      r2
...
;*      Branch Occurs to L659 
	.line	113
;----------------------------------------------------------------------
; 2594 | DP_IM_PR_TEXT_READ(71,1,0xF5);  //Â÷³» ¼öµ¿ ¼³Á¤ Ç¥½Ã±â¿¡ 96¹ø Ç¥Ãâ ¹®À
;     | åÀ» Àü¼ÛÇÑ´Ù.                                                          
; 2596 | //sprintf(szBuf2,"#SEND FDI Tx Ok");strcat(szBuf,szBuf2);sdebugFlag = T
;     | RUE;                                                                   
;----------------------------------------------------------------------
L657:        
        push      r0
...
L658:        
	.line	117
;----------------------------------------------------------------------
; 2598 | else if(nSccSendFlag.nTranNum)                                         
; 2600 |         //SCC_TX_PROTOCOL(sDestTxBuf,0xF7,0x31,0x1C,nDest_Num,BIN_BCD(n
;     | Select_Button.nGPTranNumInput.nNum),0,0,0,10);                         
; 2602 |         //sprintf(szBuf2,"#SEND TRNUM Tx Ok");strcat(szBuf,szBuf2);sdeb
;     | ugFlag = TRUE;                                                         
; 2605 | //Delay_SCC3_SendTo(sDestTxBuf,20,SCC_B_CHANNEL,5);                    
;----------------------------------------------------------------------
L659:        
	.line	126
;----------------------------------------------------------------------
; 2607 | if(nSccSendFlag.nDeSTCodeSet)nSccSendFlag.nDeSTCodeSet--;              
;----------------------------------------------------------------------
        ldi       @_nSccSendFlag+0,r0
        beq       L661
...
L661:        
	.line	127
;----------------------------------------------------------------------
; 2608 | if(nSccSendFlag.nTranNum)nSccSendFlag.nTranNum--;                      
;----------------------------------------------------------------------
        ldi       @_nSccSendFlag+1,r0
        beq       L708
;*      Branch Occurs to L708 
...
L663:        
	.line	129
;----------------------------------------------------------------------
; 2610 | else if(WORD_L(nSccSendFlag.nCarNum)) //È£Â÷ ¹øÈ£ Ç¥½Ã±â ¼ýÀÚ Ç¥Çö.    
;----------------------------------------------------------------------
        ldiu      255,r0
...
;*      Branch Occurs to L665 
	.line	131
;----------------------------------------------------------------------
; 2612 | nSccSendFlag.nCarNum = 0;                                              
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nSccSendFlag+5
	.line	133
;----------------------------------------------------------------------
; 2614 | SCC_TX_PROTOCOL(sDestTxBuf,0xFA,0x31,0x1C,nDest_Num,BIN_BCD(nSelect_But
...
;----------------------------------------------------------------------
	...
	.line	134
;----------------------------------------------------------------------
; 2615 | Delay_SCC3_SendTo(sDestTxBuf,20,SCC_B_CHANNEL,5);                      
; 2617 | //sprintf(szBuf2,"#SEND CarNum Tx Ok");strcat(szBuf,szBuf2);sdebugFlag
;     | = TRUE;                                                                
;----------------------------------------------------------------------
        ldiu      1,r2
        ldiu      5,r0
...
L665:        
	.line	139
;----------------------------------------------------------------------
; 2620 | else if(WORD_L(nSccSendFlag.nClean)) //Â÷³» Ç¥½Ã±â Å¬¸®¾î              
;----------------------------------------------------------------------
        ldiu      255,r0
...
;*      Branch Occurs to L667 
	.line	141
;----------------------------------------------------------------------
; 2622 | SCC_TX_PROTOCOL(sClean_Tx_Buf,0xF1,0x31,0x1A,0,0,0,0,0,10);            
;----------------------------------------------------------------------
        ldiu      10,r1
        ldiu      0,r0
        push      r1
	...
	.line	143
;----------------------------------------------------------------------
; 2624 | Delay_SCC3_SendTo(sClean_Tx_Buf,20,SCC_B_CHANNEL,5);                   
;----------------------------------------------------------------------
        ldiu      5,r2
        ldiu      1,r0
        push      r2
	...
	.line	145
;----------------------------------------------------------------------
; 2626 | nSccSendFlag.nClean--;                                                 
; 2628 | //sprintf(szBuf2,"#SEND Text Clean OK");strcat(szBuf,szBuf2);sdebugFlag
;     |  = TRUE;                                                               
;----------------------------------------------------------------------
        ldiu      1,r0
...
L667:        
	.line	150
;----------------------------------------------------------------------
; 2631 | else if(WORD_L(nTr_St_Info.nTcmsAutoDriverFlag) || WORD_L(nTr_St_Info.n
...
;*      Branch Occurs to L670 
        ldi       @_nTr_St_Info+374,r0
        beq       L688
;*      Branch Occurs to L688 
L670:        
	.line	153
;----------------------------------------------------------------------
; 2634 | if(WORD_L(nSccSendFlag.nIM_Text.nTxFlag) && !WORD_L(nTr_St_Info.nClean_
...
;*      Branch Occurs to L673 
	.line	156
;----------------------------------------------------------------------
; 2637 | DP_IM_PR_TEXT_READ(WORD_L(nSccSendFlag.nIM_Text.nTxNum),1,0xF1);
;     |  //¹®ÀåÀ» ÀÐ¾î¼­ Àü¼ÛÇÑ´Ù.                                             
;----------------------------------------------------------------------
        ldiu      241,r2
	...
	.line	157
;----------------------------------------------------------------------
; 2638 | nSccSendFlag.nIM_Text.nTxFlag = 0;                                     
; 2640 | //sprintf(szBuf2,"#SEND IM OK[%d] ",WORD_L(nSccSendFlag.nIM_Text.nTxNum
...
L673:        
	.line	162
;----------------------------------------------------------------------
; 2643 | else if(WORD_L(nSccSendFlag.nPR_Text.nTxFlag) && !WORD_L(nTr_St_Info.nC
...
;*      Branch Occurs to L676 
	.line	165
;----------------------------------------------------------------------
; 2646 | DP_IM_PR_TEXT_READ(WORD_L(nSccSendFlag.nPR_Text.nTxNum),4,0xF1);
;     |  //¹®ÀåÀ» ÀÐ¾î¼­ Àü¼ÛÇÑ´Ù.                                             
;----------------------------------------------------------------------
        ldiu      241,r2
	...
	.line	166
;----------------------------------------------------------------------
; 2647 | nSccSendFlag.nPR_Text.nTxFlag = 0;                                     
; 2649 | //sprintf(szBuf2,"#SEND PR OK[%d] ",WORD_L(nSccSendFlag.nPR_Text.nTxNum
;     | ));strcat(szBuf,szBuf2);sdebugFlag = TRUE;                             
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nSccSendFlag+9
...
L676:        
	.line	171
;----------------------------------------------------------------------
; 2652 | else if(WORD_L(nSccSendFlag.nRNDTXFlag) &&      !WORD_L(nTr_St_Info.nCl
...
;*      Branch Occurs to L708 
	.line	174
;----------------------------------------------------------------------
; 2655 | if((WORD_L(nTr_St_Info.nNow.nCode)+1) && (WORD_L(nTr_St_Info.nNext.nCod
...
;*      Branch Occurs to L687 
	.line	176
;----------------------------------------------------------------------
; 2657 | if(!DWORD_L(nTr_St_Info.nDistance))                                    
;----------------------------------------------------------------------
        ldiu      @_nTr_St_Info+74,r0
        tstb      65535,r0
        bne       L685
;*      Branch Occurs to L685 
	.line	178
;----------------------------------------------------------------------
; 2659 | if(dRndNow != (WORD_L(nTr_St_Info.nNow.nCode)+1) && dRndNext != (WORD_L
...
;*      Branch Occurs to L686 
	.line	180
;----------------------------------------------------------------------
; 2661 | dRndDistance = DWORD_L(nTr_St_Info.nDistance);                         
;----------------------------------------------------------------------
        ldiu      @_nTr_St_Info+74,r0
        and       65535,r0
...
L685:        
	.line	183
;----------------------------------------------------------------------
; 2664 | else {dRndDistance = DWORD_L(nTr_St_Info.nDistance); }                 
;----------------------------------------------------------------------
        ldiu      @_nTr_St_Info+74,r0
        and       65535,r0
        sti       r0,@_dRndDistance+0
L686:        
	.line	185
;----------------------------------------------------------------------
; 2666 | dStStion = WORD_L(nTr_St_Info.nStart.nCode)+1;                         
;----------------------------------------------------------------------
        ldiu      255,r0
	...
	.line	186
;----------------------------------------------------------------------
; 2667 | dRndNow  = WORD_L(nTr_St_Info.nNow.nCode)+1;                           
;----------------------------------------------------------------------
        ldiu      255,r0
        and       @_nTr_St_Info+169,r0
        addi      1,r0                  ; Unsigned
	...
	.line	187
;----------------------------------------------------------------------
; 2668 | dRndNext = WORD_L(nTr_St_Info.nNext.nCode)+1;                          
;----------------------------------------------------------------------
        ldiu      255,r0
        and       @_nTr_St_Info+215,r0
        addi      1,r0                  ; Unsigned
	...
	.line	188
;----------------------------------------------------------------------
; 2669 | dRndDest = WORD_L(nTr_St_Info.nDest.nCode)+1;                          
;----------------------------------------------------------------------
        ldiu      255,r0
        and       @_nTr_St_Info+261,r0
        addi      1,r0                  ; Unsigned
	...
	.line	190
;----------------------------------------------------------------------
; 2671 | SCC_RNDTX_PROTOCOL(sRnd_Tx_Buf,0xF9,0x31,0x28,dRndNow,dRndNext,dRndDest
...
;----------------------------------------------------------------------
	...
	.line	191
;----------------------------------------------------------------------
; 2672 | Delay_SCC3_SendTo(sRnd_Tx_Buf,20,SCC_B_CHANNEL,5);                     
;----------------------------------------------------------------------
        ldiu      5,r0
        push      r0
        ldiu      714,r0
...
;----------------------------------------------------------------------
; 2674 | d_RndTxCnt++;                                                          
; 2677 | //if(!(d_RndTxCnt%10))                                                 
; 2678 | //{                                                                    
; 2679 |         //sprintf(szBuf2,"#RND TX DATA : StartST[%02X]H-NowST[%02X]H-Ne
;     | xtST[%02X]H-DestST[%02X]H-Distan[%d]",dStStion,dRndNow,dRndNext,dRndDes
;     | t,dRndDistance);                                                       
; 2680 |         //strcat(szBuf,szBuf2);sdebugFlag = TRUE;                      
; 2681 | //}                                                                    
;----------------------------------------------------------------------
        ldiu      1,r0
...
L687:        
	.line	204
;----------------------------------------------------------------------
; 2685 | nSccSendFlag.nRNDTXFlag = 0;                                           
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_nSccSendFlag+15
        bu        L708
...
L688:        
	.line	207
;----------------------------------------------------------------------
; 2688 | else if(!WORD_L(nTr_St_Info.nTcmsAutoDriverFlag) && !WORD_L(nTr_St_Info
...
;----------------------------------------------------------------------
	...
	.line	211
;----------------------------------------------------------------------
; 2692 | if(nSccSendFlag.nPIBPaten) // Â÷³» ÆÐÅÏ TEST                           
;----------------------------------------------------------------------
...
;*      Branch Occurs to L696 
	.line	213
;----------------------------------------------------------------------
; 2694 | if(nTr_St_Info.nPIBPatFlag)                                            
;----------------------------------------------------------------------
...
;*      Branch Occurs to L694 
	.line	215
;----------------------------------------------------------------------
; 2696 | SCC_TX_PROTOCOL(sSimu_Tx_Buf,0xF1,0x31,0x0B,0,0,0,0,0,10);             
; 2698 | else                                                                   
;----------------------------------------------------------------------
        ldiu      10,r0
        ldiu      0,r1
...
;*      Branch Occurs to L695 
	.line	219
;----------------------------------------------------------------------
; 2700 | SCC_TX_PROTOCOL(sSimu_Tx_Buf,0xF1,0x31,0x1B,0,0,0,0,0,10);             
;----------------------------------------------------------------------
L694:        
        ldiu      0,r3
        ldiu      0,rs
...
L695:        
	.line	222
;----------------------------------------------------------------------
; 2703 | Delay_SCC3_SendTo(sSimu_Tx_Buf,20,SCC_B_CHANNEL,5);                    
;----------------------------------------------------------------------
        ldiu      5,r2
        ldiu      1,r0
        ldiu      20,r1
...
L696:        
	.line	225
;----------------------------------------------------------------------
; 2706 | else if(nSccSendFlag.nS_FDIPaten) // Á¤¸é,¿­¹ø,³ë¼± ÆÐÅÏ TEST          
;----------------------------------------------------------------------
...
;*      Branch Occurs to L708 
	.line	227
;----------------------------------------------------------------------
; 2708 | if(nSccSendFlag.nS_FDIPaten == 0x01)                                   
;----------------------------------------------------------------------
        cmpi      1,r0
        bne       L702
;*      Branch Occurs to L702 
	.line	229
;----------------------------------------------------------------------
; 2710 | if(nTr_St_Info.nDISFPatFlag)                                           
;----------------------------------------------------------------------
        ldi       @_nTr_St_Info+66,r0
        beqd      L701
        ldieq     10,r0
...
;*      Branch Occurs to L701 
	.line	231
;----------------------------------------------------------------------
; 2712 | SCC_TX_PROTOCOL(sSimu_Tx_Buf,0xF8,0x31,0x0B,0,0,0,0,0,10);             
; 2714 | else                                                                   
;----------------------------------------------------------------------
        ldiu      10,r1
        ldiu      0,r0
...
;*      Branch Occurs to L707 
	.line	235
;----------------------------------------------------------------------
; 2716 | SCC_TX_PROTOCOL(sSimu_Tx_Buf,0xF8,0x31,0x1B,0,0,0,0,0,10);             
;----------------------------------------------------------------------
L701:        
        ldiu      0,rs
        ldiu      0,rc
...
L702:        
	.line	238
;----------------------------------------------------------------------
; 2719 | else if(nSccSendFlag.nS_FDIPaten == 0x02) // È£Â÷ ¹øÈ£ Ç¥½Ã±â ÆÐÅÏ TEST
;     |  ??? È®ÀÎ ÇÊ¿ä                                                         
;----------------------------------------------------------------------
...
;*      Branch Occurs to L707 
	.line	240
;----------------------------------------------------------------------
; 2721 | if(nTr_St_Info.nDISFPatFlag)                                           
;----------------------------------------------------------------------
        ldi       @_nTr_St_Info+66,r0
        beqd      L706
        ldieq     10,r0
...
;*      Branch Occurs to L706 
	.line	242
;----------------------------------------------------------------------
; 2723 | SCC_TX_PROTOCOL(sSimu_Tx_Buf,0xFA,0x31,0x0B,0,0,0,0,0,10);             
; 2725 | else                                                                   
;----------------------------------------------------------------------
        ldiu      10,r0
        ldiu      0,r1
...
;*      Branch Occurs to L707 
	.line	246
;----------------------------------------------------------------------
; 2727 | SCC_TX_PROTOCOL(sSimu_Tx_Buf,0xFA,0x31,0x1B,0,0,0,0,0,10);             
;----------------------------------------------------------------------
L706:        
        ldiu      0,rs
        ldiu      0,rc
	...
	.line	248
;----------------------------------------------------------------------
; 2729 | nSccSendFlag.nCarNum = 1; //È£Â÷ ¹øÈ£ Ç¥½Ã±â´Â ÆÐÅÏÀÌ ³¡³ª¸é ¼ýÀÚ¸¦ Ç¥Ã
;     | âÇÑ´Ù.                                                                 
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nSccSendFlag+5
L707:        
	.line	252
;----------------------------------------------------------------------
; 2733 | Delay_SCC3_SendTo(sSimu_Tx_Buf,20,SCC_B_CHANNEL,5);                    
;----------------------------------------------------------------------
        ldiu      5,r1
        ldiu      1,r2
        ldiu      20,r0
	...
	.line	254
;----------------------------------------------------------------------
; 2735 | nSccSendFlag.nS_FDIPaten--;                                            
; 2739 | //µð¹ö±ë                                                               
; 2740 | //if(sdebugFlag) {sdebugFlag = FALSE; strcat(szBuf,"\r\n"); user_DebugP
;     | rint(szBuf); }                                                         
;----------------------------------------------------------------------
        ldiu      1,r0
...
_DP_SD_Format_Change:
	...
	.line	1
;----------------------------------------------------------------------
; 2747 | void DP_SD_Format_Change(UCHAR *pDData)                                
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      1,sp
...
;----------------------------------------------------------------------
	.line	5
;----------------------------------------------------------------------
; 2751 | memcpy(d_self_Buf,pDData,60);                                          
;----------------------------------------------------------------------
        ldiu      @CL72,ar1
        ldiu      *-fp(2),ar0
        ldiu      *ar0++(1),r0
...
||      ldi       *ar0++(1),r0
	...
	.line	7
;----------------------------------------------------------------------
; 2753 | for(i=0;i<18;i++) TCMSSD[i].DR.BYTE = 0x00;                            
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(1)
        ldiu      r0,r1
...
;----------------------------------------------------------------------
        ...
        cmpi      60,r0
        bge       L717
;*      Branch Occurs to L717 
L713:        
	.line	12
;----------------------------------------------------------------------
; 2758 | if( WORD_L(mSelfTest_NG_CntBuf[i]) >= SELF_TEST_ALLCNT)//2011_02_17 3.2
;     | ¹öÀü                                                                   
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0
        ldiu      @CL19,ar0
...
L717:        
	.line	23
;----------------------------------------------------------------------
; 2769 | TCMSSD[0].DR.BIT.B0 = pDData[6];//FID1                                 
;----------------------------------------------------------------------
        ldiu      @CL71,ar0
        ldiu      1,r1
        ldiu      *-fp(2),ar1
	...
	.line	24
;----------------------------------------------------------------------
; 2770 | TCMSSD[0].DR.BIT.B1 = pDData[7];//FID2                                 
;----------------------------------------------------------------------
        ldiu      *-fp(2),ar1
        ldiu      1,r0
        ldiu      *ar0,r1
...
;----------------------------------------------------------------------
	...
	.line	26
;----------------------------------------------------------------------
; 2772 | TCMSSD[0].DR.BIT.B3 = 0;                                               
;----------------------------------------------------------------------
        andn      8,r0
        sti       r0,@_TCMSSD+0
	.line	27
;----------------------------------------------------------------------
; 2773 | TCMSSD[0].DR.BIT.B4 = 0;                                               
;----------------------------------------------------------------------
        andn      16,r0
        sti       r0,@_TCMSSD+0
	.line	28
;----------------------------------------------------------------------
; 2774 | TCMSSD[0].DR.BIT.B5 = 0;                                               
;----------------------------------------------------------------------
        andn      32,r0
        sti       r0,@_TCMSSD+0
	.line	29
;----------------------------------------------------------------------
; 2775 | TCMSSD[0].DR.BIT.B6 = 0;                                               
;----------------------------------------------------------------------
        andn      64,r0
        sti       r0,@_TCMSSD+0
	.line	30
;----------------------------------------------------------------------
; 2776 | TCMSSD[0].DR.BIT.B7 = 0;                                               
; 2779 | //CAR1                                                                 
;----------------------------------------------------------------------
        andn      128,r0
        sti       r0,@_TCMSSD+0
	.line	34
;----------------------------------------------------------------------
; 2780 | TCMSSD[1].DR.BIT.B0 = pDData[0] ;       //IDU1                         
;----------------------------------------------------------------------
        ldiu      @CL73,ar0
        ldiu      *ar0,r1
        ldiu      *-fp(2),ar1
	...
	.line	35
;----------------------------------------------------------------------
; 2781 | TCMSSD[1].DR.BIT.B1 = pDData[3] ;   //IDU2                             
;----------------------------------------------------------------------
        ldiu      1,r0
        ldiu      *-fp(2),ar1
        and       *+ar1(3),r0
	...
	.line	36
;----------------------------------------------------------------------
; 2782 | TCMSSD[1].DR.BIT.B2 = pDData[1]; ;      //IDU3                         
;----------------------------------------------------------------------
        ldiu      1,r0
        ldiu      *-fp(2),ar1
        and       *+ar1(1),r0
	...
	.line	37
;----------------------------------------------------------------------
; 2783 | TCMSSD[1].DR.BIT.B3 = 0 ;       //IDU4                                 
;----------------------------------------------------------------------
        ldiu      @_TCMSSD+1,r0
        andn      8,r0
        sti       r0,@_TCMSSD+1
	.line	38
;----------------------------------------------------------------------
; 2784 | TCMSSD[1].DR.BIT.B4 = 0 ;       //FSM1   - CNSU1                       
;----------------------------------------------------------------------
        andn      16,r0
        sti       r0,@_TCMSSD+1
	.line	39
;----------------------------------------------------------------------
; 2785 | TCMSSD[1].DR.BIT.B5 = 0 ;       //FSM2                                 
;----------------------------------------------------------------------
        andn      32,r0
        sti       r0,@_TCMSSD+1
	.line	40
;----------------------------------------------------------------------
; 2786 | TCMSSD[1].DR.BIT.B6 = 0 ;       //FSM3                                 
;----------------------------------------------------------------------
        andn      64,r0
        sti       r0,@_TCMSSD+1
	.line	41
;----------------------------------------------------------------------
; 2787 | TCMSSD[1].DR.BIT.B7 = 0 ;       //FSM4   - CNSU2                       
; 2789 | //CAR1                                                                 
;----------------------------------------------------------------------
        andn      128,r0
        sti       r0,@_TCMSSD+1
	.line	44
;----------------------------------------------------------------------
; 2790 | TCMSSD[2].DR.BIT.B0 = pDData[4] ;       //IDU1                         
;----------------------------------------------------------------------
        ldiu      *-fp(2),ar1
        ldiu      1,r0
        ldiu      @CL74,ar0
	...
	.line	45
;----------------------------------------------------------------------
; 2791 | TCMSSD[2].DR.BIT.B1 = pDData[2] ;   //IDU2                             
;----------------------------------------------------------------------
        ldiu      *-fp(2),ar1
        ldiu      1,r0
        and       *+ar1(2),r0
	...
	.line	46
;----------------------------------------------------------------------
; 2792 | TCMSSD[2].DR.BIT.B2 = pDData[5] ;       //IDU3                         
;----------------------------------------------------------------------
        ldiu      *-fp(2),ar1
        ldiu      1,r0
        and       *+ar1(5),r0
	...
	.line	47
;----------------------------------------------------------------------
; 2793 | TCMSSD[2].DR.BIT.B3 = 0 ;       //IDU4                                 
;----------------------------------------------------------------------
        ldiu      @_TCMSSD+2,r0
        andn      8,r0
        sti       r0,@_TCMSSD+2
	.line	48
;----------------------------------------------------------------------
; 2794 | TCMSSD[2].DR.BIT.B4 = 0 ;       //FSM1   - CNSU1                       
;----------------------------------------------------------------------
        andn      16,r0
        sti       r0,@_TCMSSD+2
	.line	49
;----------------------------------------------------------------------
; 2795 | TCMSSD[2].DR.BIT.B5 = 0 ;       //FSM2                                 
;----------------------------------------------------------------------
        andn      32,r0
        sti       r0,@_TCMSSD+2
	.line	50
;----------------------------------------------------------------------
; 2796 | TCMSSD[2].DR.BIT.B6 = 0 ;       //FSM3                                 
;----------------------------------------------------------------------
        andn      64,r0
        sti       r0,@_TCMSSD+2
	.line	51
;----------------------------------------------------------------------
; 2797 | TCMSSD[2].DR.BIT.B7 = 0 ;       //FSM4   - CNSU2                       
;----------------------------------------------------------------------
        andn      128,r0
        sti       r0,@_TCMSSD+2
	.line	53
...
_DP_TCMSSD_Format:
	...
	.line	1
;----------------------------------------------------------------------
; 2802 | void DP_TCMSSD_Format(TMS_SEND_DATA *Type,UCHAR *pDData)
;     |                                                                        
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
...
;----------------------------------------------------------------------
	.line	5
;----------------------------------------------------------------------
; 2806 | DP_SD_Format_Change(nSelf_Test.nSELF_RXDATA_Buf);                      
; 2808 | //pDData[0] = 0x30;                                                    
;----------------------------------------------------------------------
        ldiu      @CL27,r0
        push      r0
	...
	.line	8
;----------------------------------------------------------------------
; 2809 | for(i=0;i<3;i++) pDData[(i)] = Type[i].DR.BYTE;                        
; 2810 | //pDData[11] = ROM_VER;                                                
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(1)
        cmpi      3,r0
...
_DP_IDC_SW_UPLOAD_Pro:
	...
	.line	1
;----------------------------------------------------------------------
; 2817 | void DP_IDC_SW_UPLOAD_Pro()                                            
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      12,sp
...
;----------------------------------------------------------------------
; 2819 | DWORD i=0;                                                             
; 2820 | UCHAR sDATA_IDC;                                                       
; 2821 | UCHAR sDataBuf[10];                                                    
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(1)
	.line	7
;----------------------------------------------------------------------
; 2823 | if(((SanDisk_Vari.nDiskUpChekFlag == 1) || (SanDisk_Vari.nDiskUpChekFla
;     | g == 2)||(SanDisk_Vari.nDiskUpChekFlag == 3))                          
; 2824 |   && SanDisk_Vari.nDiskInputCheckFlag)                                 
;----------------------------------------------------------------------
...
;*      Branch Occurs to L743 
	.line	10
;----------------------------------------------------------------------
; 2826 | if(!DWORD_MASKING(Idc_Load.nDataNvsrLoad))                             
;----------------------------------------------------------------------
        ldi       @_Idc_Load+2,r0
        bne       L736
;*      Branch Occurs to L736 
	.line	12
;----------------------------------------------------------------------
; 2828 | if(!WORD_L(Idc_Load.nErCnt))                                           
;----------------------------------------------------------------------
        ldiu      255,r0
        tstb      @_Idc_Load+1,r0
        bne       L731
;*      Branch Occurs to L731 
	.line	14
;----------------------------------------------------------------------
; 2830 | Idc_Load.nErCnt = NorFlashAllErase();                                  
;----------------------------------------------------------------------
...
L731:        
	.line	16
;----------------------------------------------------------------------
; 2832 | else if(WORD_L(Idc_Load.nErCnt))                                       
;----------------------------------------------------------------------
        ldiu      255,r0
        tstb      @_Idc_Load+1,r0
        beq       L733
...
L733:        
	.line	21
;----------------------------------------------------------------------
; 2837 | if(WORD_L(Idc_Load.nErCnt)==50 && !DWORD_MASKING(Idc_Load.nDataNvsrLoad
;     | ))                                                                     
;----------------------------------------------------------------------
        ldiu      255,r0
        and       @_Idc_Load+1,r0
...
;*      Branch Occurs to L736 
	.line	23
;----------------------------------------------------------------------
; 2839 | Idc_Load.nDataNvsrLoad = Nvsram_IDC_UpLoad_Da();        //IDC ÆÄÀÏ »çÀÌ
;     | Áî¸¦ ¸®ÅÏÇÑ´Ù.                                                         
;----------------------------------------------------------------------
...
L736:        
	.line	27
;----------------------------------------------------------------------
; 2843 | if(DWORD_MASKING(Idc_Load.nDataNvsrLoad))                              
;----------------------------------------------------------------------
        ldi       @_Idc_Load+2,r0
        beq       L743
;*      Branch Occurs to L743 
...
;----------------------------------------------------------------------
	...
	.line	31
;----------------------------------------------------------------------
; 2847 | if(DWORD_MASKING(Idc_Load.nDataNvsrWait)>=5)                           
;----------------------------------------------------------------------
        cmpi      5,r0
        blo       L743
;*      Branch Occurs to L743 
...
L739:        
	.line	35
;----------------------------------------------------------------------
; 2851 | if(DWORD_MASKING(Idc_Load.nRomWriteCnt)>= DWORD_MASKING(Idc_Load.nDataN
;     | vsrLoad))                                                              
;----------------------------------------------------------------------
        ldiu      @_Idc_Load+4,r0
        cmpi      @_Idc_Load+2,r0
...
;*      Branch Occurs to L742 
	.line	37
;----------------------------------------------------------------------
; 2853 | Idc_Load.nDataNvsrLoad = 0;                                            
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_Idc_Load+2
	.line	38
;----------------------------------------------------------------------
; 2854 | Idc_Load.nDataNvsrWait = 0;                                            
;----------------------------------------------------------------------
        sti       r0,@_Idc_Load+3
	.line	39
;----------------------------------------------------------------------
; 2855 | Idc_Load.nErCnt = 0;                                                   
;----------------------------------------------------------------------
        sti       r0,@_Idc_Load+1
	.line	40
;----------------------------------------------------------------------
; 2856 | Idc_Load.nRomWriteCnt = 0;                                             
;----------------------------------------------------------------------
        sti       r0,@_Idc_Load+4
	.line	41
;----------------------------------------------------------------------
; 2857 | Idc_Load.nKO_Flag = 1;                                                 
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_Idc_Load+5
	.line	42
;----------------------------------------------------------------------
; 2858 | Idc_Load.nIDC_ROM_Flag = 0;                                            
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_Idc_Load+0
	.line	44
...
;*      Branch Occurs to L743 
	.line	50
;----------------------------------------------------------------------
; 2866 | sDATA_IDC = NVSRAM(Idc_Load.nRomWriteCnt);                             
;----------------------------------------------------------------------
L742:        
        ldiu      *+ar0(ir0),r0
        sti       r0,*+fp(2)
	.line	51
;----------------------------------------------------------------------
; 2867 | NorFlashWrite(Idc_Load.nRomWriteCnt,sDATA_IDC);                        
;----------------------------------------------------------------------
        push      r0
        ldiu      @_Idc_Load+4,r0
        push      r0
...
_DP_STOP_PT_INFO_ITEM_FUNCT:
	...
	.line	1
;----------------------------------------------------------------------
; 2885 | void DP_STOP_PT_INFO_ITEM_FUNCT(STOP_PT_INFO_ITEM *pData)              
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      12,sp
	...
	.line	4
;----------------------------------------------------------------------
; 2888 | int s_St_Info_Point = 0;                                               
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(1)
	.line	5
;----------------------------------------------------------------------
; 2889 | int s_Stop_Pt_Item_Point = 0;                                          
;----------------------------------------------------------------------
        sti       r0,*+fp(2)
	.line	6
;----------------------------------------------------------------------
...
;----------------------------------------------------------------------
; 2894 | int sCnt2=0;                                                           
; 2897 | pST_INFO pSt_Info;                                                     
; 2898 | pST_INFO_B pSt_Info_b;                                                 
; 2900 | pSTOP_PT_ITEM pStop_Pt_Item;                                           
; 2901 | pSTOP_PT_ITEM_B pStop_Pt_Item_b;                                       
;----------------------------------------------------------------------
        sti       r0,*+fp(7)
	.line	19
;----------------------------------------------------------------------
; 2903 | pPISC_DATA_ADD pDest_s = (PISC_DATA_ADD *)NVSRAM_ADD(0x00);            
;----------------------------------------------------------------------
        ldiu      @CL76,r0
        sti       r0,*+fp(12)
	.line	21
;----------------------------------------------------------------------
; 2905 | s_St_Info_Point = MAKE_DWORD(pDest_s->nAdd2_St_Info_b[0],pDest_s->nAdd2
...
;----------------------------------------------------------------------
	...
	.line	22
;----------------------------------------------------------------------
; 2906 | s_Stop_Pt_Item_Point = MAKE_DWORD(pDest_s->nAdd4_Stop_Pt_I_b[0],pDest_s
...
;----------------------------------------------------------------------
	...
	.line	24
;----------------------------------------------------------------------
; 2908 | pSt_Info = (ST_INFO *) NVSRAM_ADD(s_St_Info_Point);                    
;----------------------------------------------------------------------
        ldiu      @CL77,r0
        addi      *+fp(1),r0            ; Unsigned
        sti       r0,*+fp(8)
	.line	25
;----------------------------------------------------------------------
; 2909 | sCnt = MAKE_WORD(pSt_Info->nCnt.nNum_b[0],pSt_Info->nCnt.nNum_b[1]);   
;----------------------------------------------------------------------
        ldiu      8,r1
        ldiu      r0,ar0
        ldiu      255,r0
	...
	.line	26
;----------------------------------------------------------------------
; 2910 | pSt_Info_b = (ST_INFO_B *)calloc(sCnt,sizeof(ST_INFO_B));              
;----------------------------------------------------------------------
        ldiu      6,r0
        push      r0
        ldiu      *+fp(6),r0
	...
	.line	27
;----------------------------------------------------------------------
; 2911 | pSt_Info_b = (ST_INFO_B *) NVSRAM_ADD(s_St_Info_Point+2);              
;----------------------------------------------------------------------
        ldiu      @CL78,r0
        addi      *+fp(1),r0            ; Unsigned
        sti       r0,*+fp(9)
	.line	30
;----------------------------------------------------------------------
; 2914 | pStop_Pt_Item = (STOP_PT_ITEM *)NVSRAM_ADD(s_Stop_Pt_Item_Point);      
;----------------------------------------------------------------------
        ldiu      @CL77,r0
        addi      *+fp(2),r0            ; Unsigned
        sti       r0,*+fp(10)
	.line	31
;----------------------------------------------------------------------
; 2915 | sCnt2 = MAKE_WORD(pStop_Pt_Item->nCnt.nNum_b[0],pStop_Pt_Item->nCnt.nNu
;     | m_b[1]);                                                               
;----------------------------------------------------------------------
        ldiu      r0,ar0
        ldiu      ar0,ar1
	...
	.line	32
;----------------------------------------------------------------------
; 2916 | pStop_Pt_Item_b = (STOP_PT_ITEM_B *)calloc(sCnt2,sizeof(STOP_PT_ITEM_B)
;     | );                                                                     
;----------------------------------------------------------------------
        ldiu      9,r0
        push      r0
	...
	.line	33
;----------------------------------------------------------------------
; 2917 | pStop_Pt_Item_b = (STOP_PT_ITEM_B *)NVSRAM_ADD(s_Stop_Pt_Item_Point+2);
;----------------------------------------------------------------------
        ldiu      @CL78,r0
        addi      *+fp(2),r0            ; Unsigned
        sti       r0,*+fp(11)
	.line	36
;----------------------------------------------------------------------
; 2920 | memcpy(pData->nCnt.nNum_b,pStop_Pt_Item->nCnt.nNum_b,2);               
; 2922 | //free(pData->pBody);                                                  
; 2923 | //pData->pBody = (STOP_PT_INFO_ITEM_B *)calloc(sCnt2,sizeof(STOP_PT_INF
;     | O_ITEM_B));                                                            
;----------------------------------------------------------------------
        ldiu      *+fp(10),ar0
        ldiu      *-fp(2),ar1
	...
	.line	48
;----------------------------------------------------------------------
; 2932 | sForCnt = MAKE_WORD(pStop_Pt_Item->nCnt.nNum_b[0],pStop_Pt_Item->nCnt.n
;     | Num_b[1]);                                                             
;----------------------------------------------------------------------
        ldiu      *+fp(10),ar0
        ldiu      ar0,ar1
	...
	.line	50
;----------------------------------------------------------------------
; 2934 | d_DP_BUFF[4] = sForCnt;                                                
; 2936 | //°³¼ö ¸¸Å­ ¹Ýº¹ ÀÔ·Â ÇÑ´Ù.                                            
;----------------------------------------------------------------------
        ldiu      *+fp(5),r0
        sti       r0,@_d_DP_BUFF+4
	.line	53
;----------------------------------------------------------------------
; 2937 | for(i = 0;i<sForCnt;i++)                                               
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(3)
        ldiu      8,r1
...
L746:        
	.line	55
;----------------------------------------------------------------------
; 2939 | sAdd = MAKE_WORD(pStop_Pt_Item_b[i].nSt_Info_b[0],pStop_Pt_Item_b[i].nS
;     | t_Info_b[1]);                                                          
; 2940 | //¿ª°£ Á¤º¸¸¦ ÀÔ·Â ÇÑ´Ù.                                               
;----------------------------------------------------------------------
        ldiu      *+fp(3),ar0
        ldiu      *+fp(11),ir0
        ldiu      ar0,ar1
	...
	.line	57
;----------------------------------------------------------------------
; 2941 | memcpy(pData->pBody[i].nSt_Info_flag.nSt_Index_b,pSt_Info_b[sAdd].nSt_I
;     | ndex_b,2);                                                             
;----------------------------------------------------------------------
        ldiu      *+fp(3),ar0
        mpyi      13,ar0
	...
	.line	58
;----------------------------------------------------------------------
; 2942 | memcpy(pData->pBody[i].nSt_Info_flag.nArr_Index_b,pSt_Info_b[sAdd].nArr
;     | _Index_b,2);                                                           
;----------------------------------------------------------------------
        ldiu      *+fp(3),ar1
        mpyi      13,ar1
	...
	.line	59
;----------------------------------------------------------------------
; 2943 | memcpy(pData->pBody[i].nSt_Info_flag.nSt_Dist_b,pSt_Info_b[sAdd].nSt_Di
;     | st_b,2);                                                               
; 2945 | //Á¤Â÷ ÆÐÅÏ ITEMÀ» ÀÔ·Â ÇÑ´Ù.                                          
;----------------------------------------------------------------------
        ldiu      *+fp(4),ar1
        ldiu      *+fp(3),ar0
        mpyi      6,ar1
	...
	.line	62
;----------------------------------------------------------------------
; 2946 | pData->pBody[i].nDoor_Info = pStop_Pt_Item_b[i].nDoor_Info;            
;----------------------------------------------------------------------
        ldiu      *+fp(3),ar1
        ldiu      ar1,ar0
        mpyi      9,ar1
	...
	.line	63
;----------------------------------------------------------------------
; 2947 | memcpy(pData->pBody[i].nSt_Time_b,pStop_Pt_Item_b[i].nSt_Time_b,3);    
;----------------------------------------------------------------------
        ldiu      *+fp(3),ar0
        ldiu      ar0,ar1
        mpyi      9,ar0
	...
	.line	64
;----------------------------------------------------------------------
; 2948 | memcpy(pData->pBody[i].nStop_Time_b,pStop_Pt_Item_b[i].nStop_Time_b,3);
;----------------------------------------------------------------------
        ldiu      *+fp(3),ar0
        mpyi      13,ar0
        ldiu      *+fp(3),ar1
...
_DP_STOP_PT_INFO_G_FUNCT:
	...
	.line	1
;----------------------------------------------------------------------
; 2962 | void DP_STOP_PT_INFO_G_FUNCT(STOP_PT_G_I_HAP *pData)                   
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      1308,sp
...
;----------------------------------------------------------------------
	...
	.line	5
;----------------------------------------------------------------------
; 2966 | int sStG_IndexCnt = 0;                                                 
; 2968 | STOP_PT_INFO_ITEM nStop_Pt_Info_Flag; //Á¤Â÷ ÆÐÅÏ ITEM ´õÇÏ±â ¿ª°£ Á¤º¸
;----------------------------------------------------------------------
        sti       r0,*+fp(4)
	.line	9
;----------------------------------------------------------------------
; 2970 | pSTOP_PT_G_B pStop_Pt_G = (STOP_PT_G_B *)NVSRAM_ADD(nStart_Address_Poin
;     | t.nSt_Pt_G_Address); //Á¤Â÷ ÆÐÅÏ ±×·ì ÁÖ¼Ò.                            
; 2971 | pSTOP_PT_G_B_B pStop_Pt_G_b;                                           
;----------------------------------------------------------------------
        ldiu      1307,ir0
        ldiu      @CL79,r0
	...
	.line	12
;----------------------------------------------------------------------
; 2973 | DP_STOP_PT_INFO_ITEM_FUNCT(&nStop_Pt_Info_Flag); ////Á¤Â÷ ÆÐÅÏ ITEM ´õÇ
...
;----------------------------------------------------------------------
	...
	.line	14
;----------------------------------------------------------------------
; 2975 | memcpy(pData->nHeader.nSt_Index_b,pStop_Pt_G->nHeader.nSt_Index_b,2);  
;----------------------------------------------------------------------
        ldiu      1307,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      *-fp(2),ar1
	...
	.line	15
;----------------------------------------------------------------------
; 2976 | memcpy(pData->nHeader.nEn_Index_b,pStop_Pt_G->nHeader.nEn_Index_b,2);  
;----------------------------------------------------------------------
        ldiu      2,r0
        ldiu      2,ar0
        addi3     r0,*+fp(ir0),ar1      ; Unsigned
	...
	.line	16
;----------------------------------------------------------------------
; 2977 | memcpy(pData->nHeader.nSt_P_G_Cnt_b,pStop_Pt_G->nHeader.nSt_P_G_Cnt_b,2
;     | );                                                                     
;----------------------------------------------------------------------
        ldiu      4,r0
        addi3     r0,*+fp(ir0),ar1      ; Unsigned
	...
	.line	18
;----------------------------------------------------------------------
; 2979 | d_DP_STOP_PT_INFO_G_FUNCT = sCnt = MAKE_WORD(pStop_Pt_G->nHeader.nSt_P_
...
;----------------------------------------------------------------------
	...
	.line	20
;----------------------------------------------------------------------
; 2981 | pStop_Pt_G_b = (STOP_PT_G_B_B *)calloc(sCnt,sizeof(STOP_PT_G_B_B));    
;----------------------------------------------------------------------
        ldiu      2,r0
        push      r0
        ldiu      *+fp(3),r0
	...
	.line	21
;----------------------------------------------------------------------
; 2982 | pStop_Pt_G_b = (STOP_PT_G_B_B *)NVSRAM_ADD(nStart_Address_Point.nSt_Pt_
;     | G_Address+6); //Á¤Â÷ ÆÐÅÏ ±×·ì ÁÖ¼Ò.                                   
; 2984 | //free(pData->nSt_Pt_G_I_Flage);                                       
; 2985 | //pData->nSt_Pt_G_I_Flage = (STOP_PT_INFO_ITEM_B *)calloc(sCnt,sizeof(S
;     | TOP_PT_INFO_ITEM_B));                                                  
;----------------------------------------------------------------------
        ldiu      @CL81,r0
        addi      @_nStart_Address_Point+0,r0 ; Unsigned
	...
	.line	27
;----------------------------------------------------------------------
; 2988 | memset(d_DP_STOP_Buf,0x00,20);                                         
;----------------------------------------------------------------------
        ldiu      20,r2
        push      r2
        ldiu      0,r1
...
L751:        
	.line	32
;----------------------------------------------------------------------
; 2993 | d_DP_STOP_Buf[i] =  sStG_IndexCnt =MAKE_WORD(pStop_Pt_G_b[i].nSt_P_Item
...
||      ldiu      *+fp(1),r2
	...
	.line	34
;----------------------------------------------------------------------
; 2995 | memcpy(pData->nSt_Pt_G_I_Flage[i].nSt_Info_flag.nSt_Index_b,nStop_Pt_In
...
;----------------------------------------------------------------------
	...
	.line	35
;----------------------------------------------------------------------
; 2996 | memcpy(pData->nSt_Pt_G_I_Flage[i].nSt_Info_flag.nArr_Index_b,nStop_Pt_I
...
;----------------------------------------------------------------------
	...
	.line	36
;----------------------------------------------------------------------
; 2997 | memcpy(pData->nSt_Pt_G_I_Flage[i].nSt_Info_flag.nSt_Dist_b,nStop_Pt_Inf
...
;----------------------------------------------------------------------
	...
	.line	38
;----------------------------------------------------------------------
; 2999 | pData->nSt_Pt_G_I_Flage[i].nDoor_Info = nStop_Pt_Info_Flag.pBody[sStG_I
;     | ndexCnt].nDoor_Info;                                                   
;----------------------------------------------------------------------
        ldiu      *+fp(1),ar1
        mpyi      13,ar1
	...
	.line	40
;----------------------------------------------------------------------
; 3001 | memcpy(pData->nSt_Pt_G_I_Flage[i].nSt_Time_b,nStop_Pt_Info_Flag.pBody[s
...
;----------------------------------------------------------------------
	...
	.line	41
;----------------------------------------------------------------------
; 3002 | memcpy(pData->nSt_Pt_G_I_Flage[i].nStop_Time_b,nStop_Pt_Info_Flag.pBody
...
_DP_DISP_G__HAP:
	...
	.line	1
;----------------------------------------------------------------------
; 3013 | void DP_DISP_G__HAP(DISPLAY_G_I_H *pData)                              
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      10,sp
...
;----------------------------------------------------------------------
	...
	.line	6
;----------------------------------------------------------------------
; 3018 | int sDisplayItemAdd_D = 0;                                             
;----------------------------------------------------------------------
        sti       r0,*+fp(4)
	.line	7
;----------------------------------------------------------------------
; 3019 | DWORD sDisItemAdd = 0;                                                 
;----------------------------------------------------------------------
        sti       r0,*+fp(5)
	.line	9
;----------------------------------------------------------------------
; 3021 | pPISC_DATA_ADD pDest_s = (PISC_DATA_ADD *)NVSRAM_ADD(0x00); //ÇÁ·Î±×·¥
;     | ½ÃÀÛÇÑ¶§ ÀüÃ¼ ÁÖ¼Ò ¸®½ºÆ®                                              
; 3023 | //µ¿Àû¹è¿­ ¼±¾ð                                                        
;----------------------------------------------------------------------
        ldiu      @CL76,r0
	...
	.line	12
;----------------------------------------------------------------------
; 3024 | pDISPLAY_GROUP nDisplay_G = (DISPLAY_GROUP *)NVSRAM_ADD(nStart_Address_
;     | Point.nDisp_G_Address); //Display Group address                        
; 3025 | pDISPLAY_G_B nDisplay_G_b;                                             
; 3027 | //µ¿Àû¹è¿­ ¼±¾ð                                                        
; 3028 | pDISPLAY_ITEM nDisplay_I;                                              
; 3029 | pDISPLAY_ITEM_B nDisplay_I_b;                                          
;----------------------------------------------------------------------
        ldiu      @CL79,r0
	...
	.line	19
;----------------------------------------------------------------------
; 3031 | sDisItemAdd = MAKE_DWORD(pDest_s->nAdd6_Display_I_b[0],pDest_s->nAdd6_D
...
;----------------------------------------------------------------------
	...
	.line	21
;----------------------------------------------------------------------
; 3033 | nDisplay_I = (DISPLAY_ITEM *)NVSRAM_ADD(sDisItemAdd); //Display Item ÁÖ
;     | ¼Ò.                                                                    
;----------------------------------------------------------------------
        ldiu      @CL79,r0
	...
	.line	22
;----------------------------------------------------------------------
; 3034 | sCnt2 = MAKE_WORD(nDisplay_I->nCnt.nNum_b[0],nDisplay_G->nCnt.nNum_b[1]
;     | );                                                                     
;----------------------------------------------------------------------
        ldiu      r0,ar0
        ldiu      *+fp(7),ar1
	...
	.line	23
;----------------------------------------------------------------------
; 3035 | nDisplay_I_b = (DISPLAY_ITEM_B *)calloc(sCnt2,sizeof(DISPLAY_ITEM_B));
;     | //µ¿Àû¹è¿­ ÇÒ´ç                                                        
;----------------------------------------------------------------------
        ldiu      4,r0
        push      r0
	...
	.line	24
;----------------------------------------------------------------------
; 3036 | nDisplay_I_b = (DISPLAY_ITEM_B *)NVSRAM_ADD(sDisItemAdd+2); //Display I
;     | tem ÁÖ¼Ò.                                                              
;----------------------------------------------------------------------
        ldiu      @CL83,r0
        addi      *+fp(5),r0            ; Unsigned
	...
	.line	27
;----------------------------------------------------------------------
; 3039 | d_disp_g_cnt = sCnt = MAKE_WORD(nDisplay_G->nCnt.nNum_b[0],nDisplay_G->
;     | nCnt.nNum_b[1]);                                                       
;----------------------------------------------------------------------
        ldiu      *+fp(7),ar0
        ldiu      ar0,ar1
	...
	.line	28
;----------------------------------------------------------------------
; 3040 | nDisplay_G_b = (DISPLAY_G_B *)calloc(sCnt,sizeof(DISPLAY_G_B)); //µ¿Àû¹
;     | è¿­ ÇÒ´ç                                                               
;----------------------------------------------------------------------
        ldiu      6,r0
        push      r0
	...
	.line	29
;----------------------------------------------------------------------
; 3041 | nDisplay_G_b = (DISPLAY_G_B *)NVSRAM_ADD(nStart_Address_Point.nDisp_G_A
...
;----------------------------------------------------------------------
	...
	.line	32
;----------------------------------------------------------------------
; 3044 | memcpy(pData->nCnt.nNum_b,nDisplay_G->nCnt.nNum_b,2);                  
; 3046 | //free(pData->pBody);                                                  
; 3047 | //pData->pBody = (DISPLAY_G_I_B *)calloc(sCnt,sizeof(DISPLAY_G_I_B));  
;----------------------------------------------------------------------
        ldiu      *+fp(7),ar0
        ldiu      *-fp(2),ar1
        ldiu      *ar0,r0
	...
	.line	38
;----------------------------------------------------------------------
; 3050 | for(i=0;i<sCnt;i++)                                                    
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(3)
        cmpi      *+fp(1),r0
        bge       L757
;*      Branch Occurs to L757 
L756:        
	.line	40
;----------------------------------------------------------------------
; 3052 | memcpy(pData->pBody[i].nSt_Cnt_b,nDisplay_G_b[i].nSt_Cnt_b,2);         
;----------------------------------------------------------------------
        ldiu      *+fp(3),ar0
        ldiu      ar0,ar1
        ash       3,ar0
	...
	.line	41
;----------------------------------------------------------------------
; 3053 | memcpy(pData->pBody[i].nGuri_b,nDisplay_G_b[i].nGuri_b,2);             
;----------------------------------------------------------------------
        ldiu      *+fp(3),ar1
        ldiu      ar1,ar0
        mpyi      6,ar1
	...
	.line	43
;----------------------------------------------------------------------
; 3055 | sDisplayItemAdd_D = MAKE_WORD(nDisplay_G_b[i].nItem_Index_b[0],nDisplay
...
;----------------------------------------------------------------------
	...
	.line	45
;----------------------------------------------------------------------
; 3057 | memcpy(pData->pBody[i].nItem_Index_Flag.nText_Kind_b,nDisplay_I_b[sDisp
...
;----------------------------------------------------------------------
	...
	.line	46
;----------------------------------------------------------------------
; 3058 | memcpy(pData->pBody[i].nItem_Index_Flag.nText_Index,nDisplay_I_b[sDispl
...
_DP_ConvAsc2Hex:
	...
	.line	1
;----------------------------------------------------------------------
; 3070 | UCHAR DP_ConvAsc2Hex(char chDat)                                       
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      1,sp
...
;----------------------------------------------------------------------
	...
	.line	4
;----------------------------------------------------------------------
; 3073 | if(chDat >= '0' && chDat <= '9') nBuf = chDat-'0';                     
; 3074 | else                                                                   
;----------------------------------------------------------------------
        ldiu      *-fp(2),r0
        cmpi      48,r0
...
;*      Branch Occurs to L763 
        ...
        sti       r0,*+fp(1)
        bu        L769
;*      Branch Occurs to L769 
L763:        
	.line	6
;----------------------------------------------------------------------
; 3075 | if(chDat >= 'a' && chDat <= 'f') nBuf = ((chDat-'a') + 10);
;     |                                                                        
; 3076 | else                                                                   
;----------------------------------------------------------------------
        ldiu      *-fp(2),r0
...
L766:        
	.line	8
;----------------------------------------------------------------------
; 3077 | if(chDat >= 'A' && chDat <= 'F') nBuf = ((chDat-'A') + 10);            
;----------------------------------------------------------------------
        ldiu      *-fp(2),r0
        cmpi      65,r0
        blt       L769
...
_DP_ConvHex2Asc:
	...
	.line	1
;----------------------------------------------------------------------
; 3085 | char DP_ConvHex2Asc(UCHAR btCh)                                        
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      1,sp
...
;----------------------------------------------------------------------
	...
	.line	4
;----------------------------------------------------------------------
; 3088 | if(btCh >= 0 && btCh <= 9) chBuf = btCh+'0';                           
;----------------------------------------------------------------------
        ldiu      *-fp(2),r0
        cmpi      9,r0
        bhi       L774
...
L774:        
	.line	5
;----------------------------------------------------------------------
; 3089 | else if(btCh >= 10 && btCh <= 15) chBuf = (btCh-10)+'A';               
;----------------------------------------------------------------------
        ldiu      *-fp(2),r0
        cmpi      10,r0
        blo       L777
...
_user_DebugPrint:
	...
	.line	1
;----------------------------------------------------------------------
; 3097 | void user_DebugPrint(char *pTxBuf)                                     
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
	.line	2
;----------------------------------------------------------------------
; 3099 | //Delay_SCC3_SendTo((UCHAR *)pTxBuf,strlen(pTxBuf),SCC_B_CHANNEL,1);   
;----------------------------------------------------------------------
	.line	4
        ldiu      *-fp(1),r1
        ldiu      *fp,fp
...
;----------------------------------------------------------------------
	...
	.line	3
;----------------------------------------------------------------------
; 3109 | UINT sPlusCnt = 0;                                                     
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(1)
	.line	4
;----------------------------------------------------------------------
; 3110 | sPlusCnt = MAKE_20BIT(PULS_20BIT,PULS_16BIT,PULS_12BIT,PULS_8BIT,PULS_4
;     | BIT);                                                                  
;----------------------------------------------------------------------
        ldiu      15,r3
        ldiu      @CL87,ar2
	...
	.line	5
;----------------------------------------------------------------------
; 3111 | LINE_ON; // ÆÞ½º °ªÀ» ÃÊ±âÈ­ ÇÑ´Ù.                                     
;----------------------------------------------------------------------
	.line	6
;----------------------------------------------------------------------
; 3112 | return ((3.141592*BCD_BIN(nSelect_Button.nWheel_Value))/90 * sPlusCnt);
;     | ;                                                                      
;----------------------------------------------------------------------
...
_DP_Timer_1ms_DataPro:
	.line	1
;----------------------------------------------------------------------
; 3128 | void DP_Timer_1ms_DataPro()                                            
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
	.line	3
;----------------------------------------------------------------------
; 3130 | nTimeDataPro_Cnt++;                                                    
;----------------------------------------------------------------------
        ldiu      1,r0
	...
	.line	5
;----------------------------------------------------------------------
; 3132 | DP_ONTD(~DICE&0x02,&nDi_Check.nDoor.nFlag,nDi_Check.nDoor.nTime,&nDi_Ch
;     | eck.nDoor.nTimeCnt);                                                   
;----------------------------------------------------------------------
        ldiu      @CL91,ar0
        ldiu      @CL92,r1
	...
	.line	6
;----------------------------------------------------------------------
; 3133 | DP_ONTD(~DICE&0x01,&nDi_Check.nHcr.nFlag,nDi_Check.nHcr.nTime,&nDi_Chec
;     | k.nHcr.nTimeCnt);                                                      
; 3134 | //DP_ONTD(~DI_STATUS_RD&0x01,&nDi_Check.nDist.nFlag,nDi_Check.nDist.nTi
...
;----------------------------------------------------------------------
	...
	.line	9
;----------------------------------------------------------------------
; 3136 | d_DI_Door = nDi_Check.nDist.nFlag;                                     
; 3146 | //nDi_Check.nDist.nRemem = nDi_Check.nDist.nFlag;                      
;----------------------------------------------------------------------
        ldiu      @_nDi_Check+20,r0
        sti       r0,@_d_DI_Door+0
	.line	22
;----------------------------------------------------------------------
; 3149 | if(!(nTimeDataPro_Cnt%200))                                            
;----------------------------------------------------------------------
...
;*      Branch Occurs to L789 
	.line	24
;----------------------------------------------------------------------
; 3151 | nTimeFlag.nAutoPlay_Flag = TRUE;                                       
; 3155 | //¼öµ¿ ¼³Á¤ÀÏ °æ¿ì Å¸ÄÚ ÀÔ·Â ½ÅÈ£¸¦ °Å¸® °ªÀ¸·Î »ç¿ë ÇÑ´Ù.             
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nTimeFlag+3
L789:        
	.line	29
;----------------------------------------------------------------------
; 3156 | if(nTr_St_Info.nAutoManuFlag && !(nTimeDataPro_Cnt%200))               
;----------------------------------------------------------------------
        ldi       @_nTr_St_Info+374,r0
        beqd      L795
	nop
...
;----------------------------------------------------------------------
	...
	.line	33
;----------------------------------------------------------------------
; 3160 | if((m_PLUSCNT_Rem != d_DI_Dist_cnt) && !nDi_Check.nDoor.nFlag )        
;----------------------------------------------------------------------
...
;*      Branch Occurs to L794 
	.line	35
;----------------------------------------------------------------------
; 3162 | nDi_Check.nDist.nFlagCnt = nDi_Check.nDist.nFlagCnt + (DP_PLUSCNT()-m_P
;     | LUSCNT_Rem);                                                           
;----------------------------------------------------------------------
        call      _DP_PLUSCNT
                                        ; Call Occurs
...
L794:        
	.line	38
;----------------------------------------------------------------------
; 3165 | m_PLUSCNT_Rem = d_DI_Dist_cnt;                                         
;----------------------------------------------------------------------
...
_DP_Timer_100ms_DataPro:
	.line	1
;----------------------------------------------------------------------
; 3174 | void DP_Timer_100ms_DataPro()                                          
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
	.line	3
;----------------------------------------------------------------------
; 3176 | nTimeDataPro_100Cnt++;                                                 
; 3178 | //ÀÏ¹Ý Æí¼ºÀÌ°í ¼¿ÇÁÅ×½ºÆ®°¡ ¾Æ´Ï¸é ³ë¼± Ç¥½Ã±â¸¦ ÁøÇà ÇÑ´Ù.           
;----------------------------------------------------------------------
        ldiu      1,r0
	...
	.line	6
;----------------------------------------------------------------------
; 3179 | if(!(nTimeDataPro_100Cnt%10) && (!nTr_St_Info.nSelf_Test_Flag)) //1ÃÊ  
; 3181 |         //½Ã¹Ä·¹ÀÌ¼Ç ÀÌ´øÁö. ¾Æ´Ï¸é Á¤»ó ÄÚµå ÀÔ·Â ¹Þ¾ÒÀ» °æ¿ì         
;----------------------------------------------------------------------
        ldiu      10,r1
        call      MOD_U30
                                        ; Call Occurs
        cmpi      0,r0
        bne       L803
;*      Branch Occurs to L803 
        ldi       @_nTr_St_Info+368,r0
        bne       L803
;*      Branch Occurs to L803 
	.line	9
;----------------------------------------------------------------------
; 3182 | if((nTr_St_Info.nAutoDoorFlag && nTr_St_Info.nSimulationFlag) || WORD_L
;     | (nTr_St_Info.nRxCode_NoNe_Flag))                                       
;----------------------------------------------------------------------
        ldi       @_nTr_St_Info+70,r0
        beq       L801
;*      Branch Occurs to L801 
        ldi       @_nTr_St_Info+67,r0
        bne       L802
...
;----------------------------------------------------------------------
; 3184 | d_Rnd_Cnt++;                                                           
; 3185 | //nSccSendFlag.nRNDTXFlag = TRUE;       //³ë¼± Ç¥½Ã±â µ¿ÀÛ Flag        
;----------------------------------------------------------------------
        ldiu      1,r0
...
L803:        
	.line	18
;----------------------------------------------------------------------
; 3191 | if(!(nTimeDataPro_100Cnt%5))    //500ms                                
;----------------------------------------------------------------------
...
;*      Branch Occurs to L805 
	.line	20
;----------------------------------------------------------------------
; 3193 | nTimeFlag.nFlag_DI_INPUT = TRUE;                                       
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nTimeFlag+0
L805:        
	.line	23
;----------------------------------------------------------------------
; 3196 | if(!(nTimeDataPro_100Cnt%20) && nTr_St_Info.nAutoDoorFlag && nTr_St_Inf
...
;*      Branch Occurs to L809 
	.line	25
;----------------------------------------------------------------------
; 3198 | nTimeFlag.nFlag_Simu = TRUE;                                           
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_nTimeFlag+1
L809:        
