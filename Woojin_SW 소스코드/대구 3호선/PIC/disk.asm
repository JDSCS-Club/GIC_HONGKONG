...
;******************************************************************************
;	c:\lang\tms320c3x\511\bin\ac30.exe disk.c C:\Users\JDS_RE~1\AppData\Local\Temp\disk.if 
	.file	"disk.c"
	.file	"main.h"
	.file	"ALL_DEF.h"
...
;----------------------------------------------------------------------
	...
	.line	12
;----------------------------------------------------------------------
; 241 | for(i=0;i<16;i++) // 512byte ³»¿ë °Ë»ö                                 
;----------------------------------------------------------------------
        ldiu      513,ir0
        ldiu      0,r0
        sti       r0,*+fp(ir0)
...
;----------------------------------------------------------------------
	...
	.line	20
;----------------------------------------------------------------------
; 373 | for(i=1;i<nSize/512/m_nSecPerClust+1;i++)       // Å¬·¯½ºÅÍÀÇ ¿É¼Â°ú °á
;     | °ú °ªÀÌ ´Ù¸¥°æ¿ì ¿¡·¯Ã³¸® ÇÊ¿ä                                         
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,*+fp(1)
...
L136:        
	.line	28
;----------------------------------------------------------------------
; 381 | PreRDSector= FATOffset;                 // ÀÌÀü FAT ¼½ÅÍ ÀÐ±â ¹é¾÷
;     |                                                                        
;----------------------------------------------------------------------
        ldiu      *+fp(5),r0
        sti       r0,*+fp(4)
	.line	30
;----------------------------------------------------------------------
; 383 | k = ((pNextCluster[i-1]%1024)*3)/2;     // Å¬·¯½ºÅÍ ½ÃÀÛ ¾îµå·¹½º (ÇÏÀ§
;     | )                                                                      
;----------------------------------------------------------------------
        ldiu      *+fp(1),ar0
...
L144:        
	.line	65
;----------------------------------------------------------------------
; 418 | PreRDSector= FATOffset;                 // ÀÌÀü FAT ¼½ÅÍ ÀÐ±â ¹é¾÷
;     |                                                                        
;----------------------------------------------------------------------
        ldiu      *+fp(5),r0
        sti       r0,*+fp(4)
...
;     | tor¸¦ ÀÇ¹ÌÇÑ´Ù.                                                        
; 423 | }// FATÀÇ Cluster Read Complete!                                       
;----------------------------------------------------------------------
        ldiu      -8,r0
        lsh3      r0,*+ar0(ir0),r0
        addi      @_m_nFat1Offset+0,r0  ; Unsigned
...
; 429 | //      if(nTemp!=i) i=0;  // ÆÄÀÏÀÌ µð½ºÅ© °ø°£ÀÌ ºÎÁ·ÇÑ¶§ ÀúÀåÇÒ¶§ DO
;     | S»ó¿¡¼­ÀÇ ¿¡·¯°¡ ¹ß»ýµÊ                                                
;----------------------------------------------------------------------
        bu        L150
;*      Branch Occurs to L150 
...
;----------------------------------------------------------------------
	...
	.line	15
;----------------------------------------------------------------------
; 544 | if(Seek_File_UNIT(sKIndCode) && glTotalClusterCnt)      // Match File È
;     | ®ÀÎ                                                                    
;----------------------------------------------------------------------
        ldiu      *-fp(3),r0
        push      r0
...
;*      Branch Occurs to L202 
	.line	17
;----------------------------------------------------------------------
; 546 | PreSector = 0;  //°°Àº °÷À» ÀÐ¾îµµ °¡´ÉÇÏµµ·Ï Flag ÃÊ±âÈ­ ÇÑ´Ù.        
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,@_PreSector+0
	.line	19
...
;----------------------------------------------------------------------
	...
	.line	41
;----------------------------------------------------------------------
; 570 | if(!strncmp((char *)sIdc_Ver_Ck,(char *)ROM_VER_S,5))   // °°Àº ¹öÀü.  
;----------------------------------------------------------------------
        ldiu      5,r2
        ldiu      @CL31,r1
        push      r2
...
;*      Branch Occurs to L203 
	.line	47
;----------------------------------------------------------------------
; 576 | if(sVerCf > sVerRom) {return 1;}        //NEW ¾÷·Îµå ÇÊ¿ä ÇÑ´Ù.        
;----------------------------------------------------------------------
L194:        
        cmpi      *+fp(36),r0
        ble       L197
...
L197:        
	.line	48
;----------------------------------------------------------------------
; 577 | else if(sVerCf < sVerRom) {return 3;}//¿Ãµå ¹öÀü ¹ß°ß                  
; 580 | else                                                                   
;----------------------------------------------------------------------
        ldiu      *+fp(35),r0
        cmpi      *+fp(36),r0
...
;----------------------------------------------------------------------
	...
	.line	79
;----------------------------------------------------------------------
; 730 | if(!Cnt){break;}        // ºÎÇÏ·®À» ÃÖ¼ÒÈ­ ÇÑ´Ù.                       
...
;----------------------------------------------------------------------
	...
	.line	8
;----------------------------------------------------------------------
; 884 | int sDispayCntLen = 0;           //Display ±×·ì¿¡¼­ Ä«¿îÅÍ¸¦ ¾Ë¾Æ ³½´Ù 
...
;     | )                                                                      
; 907 | {//ÇöÀç¿ª,´ÙÀ½¿ª°ú Á¤Â÷ ÆÐÅÏ ±×·ì Index -> ÀüÃ¼ ¿ª¸í ±¸Á¶¶û ºñ±³ ÇÑ´Ù. 
;----------------------------------------------------------------------
        ldiu      255,r1
        ldiu      *+fp(6),ir0
        ldiu      255,r0
...
; 912 | {//DIA ¿¡¼­ ¾Ë¾Æ³½ Display ±×·ì¿¡¼­ Ä«¿îÅÍ¸¦ ¾Ë¾Æ ³½´Ù.(Á¤Â÷ ÆÐÅÏ ±×·ìÀ
;     | Ì¶û °°Àº ³»¿ë)                                                         
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(2)
...
;     | St_Cnt_b[1]) == i)                                                     
; 915 | { //Display ±×·ì¿¡¼­ ¿ª Ä«¿îÅÍ ¿Í Á¤Â÷ÆÐÅÏ Index ¼ø¼­¿Í ºñ±³ÇÏ¿© °°Àº ¿
;     | ªÀ» ¾Ë¾Æ ³½´Ù.                                                         
;----------------------------------------------------------------------
        ldiu      *+fp(2),r2
        ldiu      255,r1
...
;----------------------------------------------------------------------
	...
	.line	6
;----------------------------------------------------------------------
; 987 | pPISC_DATA_ADD pDest_s = (PISC_DATA_ADD *)NVSRAM_ADD(0x00); //ÇÁ·Î±×·¥
;     | ½ÃÀÛÇÑ¶§ ÀüÃ¼ ÁÖ¼Ò ¸®½ºÆ®                                              
...
_NVSRAM_Find_PR:
	...
	.line	1
;----------------------------------------------------------------------
; 1015 | UCHAR NVSRAM_Find_PR(UCHAR nCnt,UCHAR *pData)           // NoÂ°ÀÇ ¿ª¸í
;     | ÀÐ±â                                                                   
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
...
;----------------------------------------------------------------------
	...
	.line	5
;----------------------------------------------------------------------
; 1019 | pPISC_DATA_ADD pDest_s = (PISC_DATA_ADD *)NVSRAM_ADD(0x00); //ÇÁ·Î±×·¥
;     | ½ÃÀÛÇÑ¶§ ÀüÃ¼ ÁÖ¼Ò ¸®½ºÆ®                                              
...
_NVSRAM_Rd_StationName:
	...
	.line	1
;----------------------------------------------------------------------
; 1095 | WORD NVSRAM_Rd_StationName(WORD nCnt,UCHAR *pData)// nCnt°³ÀÇ ¿ª¸íÀ» ¹ö
;     | ÆÛ¿¡ ÀúÀåÇÑ´Ù.                                                         
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
...
; 1160 | //¿ªÄÚµå°¡ Áßº¹ÇØ¼­ ÀÖ´Â ¿ª¿¡ ´ëÇÏ¿© ÇØ°áÇÑ´Ù.                         
;----------------------------------------------------------------------
        ldiu      @CL75,ar0
        ldiu      *+fp(7),ir0
        ldiu      *+fp(8),r0
...
_NVSRAM_Comp_TrainNo:
	...
	.line	1
;----------------------------------------------------------------------
; 1366 | WORD NVSRAM_Comp_TrainNo(UCHAR sData) // No : ¿­Â÷¹øÈ£                 
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      21,sp
...
;----------------------------------------------------------------------
	...
	.line	37
;----------------------------------------------------------------------
; 1402 | pDia_g_i = (DIA_G_ITEM *)NVSRAM_ADD(sAdd_Point_2); //DIA ±×·ì ITEM     
;----------------------------------------------------------------------
        ldiu      @CL34,r0
        addi      *+fp(2),r0            ; Unsigned
        sti       r0,*+fp(19)
...
;----------------------------------------------------------------------
	...
	.line	42
;----------------------------------------------------------------------
; 1407 | pDia_g_i_b = (DIA_G_ITEM_B *)NVSRAM_ADD(sAdd_Point_2+2); //DIA ±×·ì ITE
;     | M                                                                      
;----------------------------------------------------------------------
        ldiu      @CL74,r0
        addi      *+fp(2),r0            ; Unsigned
...
L405:        
	.line	60
;----------------------------------------------------------------------
; 1425 | if(!WORD_L(nNvsram_Variable.nStop_Patten_Num))//¿­¹øÀÌ ¾øÀ¸¸é ±×·ì¿¡¼­
...
_NVSRAM_Find_StationRoot:
	...
	.line	1
;----------------------------------------------------------------------
; 1460 | UCHAR NVSRAM_Find_StationRoot(WORD ScreenNo,WORD nLen)  // No ¿ª¸í ÀÐ±â
;     |                                                                        
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
