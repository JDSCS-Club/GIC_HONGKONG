...
void GpProface_Init()
{
	...
	nSelect_Button.nScreenSetNum = 1;// 상태 화면을 출력 한다.
	nSelect_Button.nScreen_Send_Delay_Time = 0;

	nSelect_Button.nWheel_Value = 0x0994;
...
void Gp_Pro_Main()
{
	...
	if(m_StartScreenSetTime>25 || m_StScreen)
	{
		...
		if(!(nTimeGp_Cnt%100) && !SCC3_INIT_A.TxPos && !SCC3_INIT_A.RxPos)
		{
			if(glTxA3_Len)
			{
				d_txLenCnt = sGpTxLen = glTxA3_Len;
				glTxA3_Len = 0;
		...
		if(!(nTimeGp_Cnt%300) && !(nSelect_Button.nScreenRxTime) && SCC3_INIT_A.TxOK)
		{
			nSelect_Button.nScreenRxTime = 100;
...
void Gp_Receive(UCHAR *pGpRXData)	
{
	...
	if(WORD_L(pGpRXData[1]) == 0x41)	//GP 에서 열차 변호를 입력 한다.
	{
		...
		// 차륜경 설정 화면이면 차륜경 
		if(WORD_L(nSelect_Button.nScreenSetNum) == 14) {nSelect_Button.nWheel_Value = MAKE_WORD(pGpRXData[2],pGpRXData[3]); }
		else
		{
...
void GP_SEND_SCREEN(UCHAR nScreenNum)
{
	...
	UCHAR sTrainKindCode[4][10] = {"영업","회송","시운전",""};
	...
	switch(WORD_L(nScreenNum))
	{
		case 1:	//상태 화면
			...
			{ 
				...
				if(nTmsRxData.nTrainKind == 1) {gp_WritingStr(340,4,&sTrainKindCode[0][0]);}
				else if(nTmsRxData.nTrainKind == 5) {gp_WritingStr(340,4,&sTrainKindCode[1][0]);}
				else if(nTmsRxData.nTrainKind == 6) {gp_WritingStr(340,6,&sTrainKindCode[2][0]);}
				else {gp_WritingStr(340,2,&sTrainKindCode[3][0]); }

			}

			if(!nDi_Check.nHcr.nFlag) {gpNorClr; gpExp;}  //직-일반
			else {gpExpClr; gpNor; }
			...
			if(nDi_Check.nDoor.nFlag)//출입문 처리.
			{
				...
				{
					NVSRAM_Rd_StationName_Code(nTr_St_Info.nNow.nCode,&nTr_St_Info.nNow.nEnNameBuf[0],&nTr_St_Info.nNow.nIRNameBuf[0],&nTr_St_Info.nGpStName.nNow[0]);
			...
			else 
			{
				...
				{
					NVSRAM_Rd_StationName_Code(nTr_St_Info.nNext.nCode,&nTr_St_Info.nNext.nEnNameBuf[0],&nTr_St_Info.nNext.nIRNameBuf[0],&nTr_St_Info.nGpStName.nNext[0]);
		...
		case 8:	//디버깅 화면
			...
			if(WORD_L(SCC1_INIT_A.TxOK) == TRUE && WORD_L(SCC3_INIT_A.TxOK) == TRUE  )
			{

				nSelect_Button.nScreenRxTime = 200;
		...
		case 14: //차륜경 설정 화면.
			...
			if(BCD_BIN(nSelect_Button.nWheel_Value) >= 1000 || BCD_BIN(nSelect_Button.nWheel_Value)<= 500)
			{
				nSelect_Button.nWheel_Value = 0x0860;
				gpSETERR;
			}

			glVer[0] = DP_ConvHex2Asc(BYTE_L(WORD_H(nSelect_Button.nWheel_Value)));
			glVer[1] = DP_ConvHex2Asc(BYTE_H(WORD_L(nSelect_Button.nWheel_Value)));
			glVer[2] = DP_ConvHex2Asc(BYTE_L(WORD_L(nSelect_Button.nWheel_Value)));

			gp_WritingStr(340,3,glVer);
			break;
...
void GP_STATE_SCREEN(UCHAR nScreenInf)
{
	...
	case 0x33: //상태 화면
		...
		gp_WritingStr(340,10,sClean_Buf);
...
void GP_SDR_SCREEN(UCHAR nScreenInf)
{
	...
	case 114://디버깅 화면 이동
		...
		memset(sClBuf,0x20,100);

		gp_WritingStr(400,40,sClBuf);
		gp_WritingStr(440,20,sClBuf);

		gp_WritingStr(500,40,sClBuf);
		gp_WritingStr(540,30,sClBuf);
...
void GP_SDR_PRINT(UCHAR *pDataTcms,UCHAR *pDataIcd,UCHAR nSend_ID)
{
	int i;
	UCHAR sGpSdrBuf[200];
	memset(sGpSdrBuf,0x20,sizeof(sGpSdrBuf));
	...
	if(WORD_L(nTr_St_Info.nTcmsRxNGCnt)<=5)
	{
		if(nSend_ID&0x01)//부하량 감소
		{
			...
			for(i=0;i< WORD_L(SCC1_INIT_A.RxLen);i++)
			{
			...
			gp_WritingStr(400,40,sGpSdrBuf);
			gp_WritingStr(440,15,&sGpSdrBuf[80]);
		...
		else
		{
			...
			{
				sGpSdrBuf[i*3] =   DP_ConvHex2Asc(BYTE_H(pDataIcd[i]));//BYTE_H(pDataIcd[i]) >= 0x0A ? (BYTE_H(pDataIcd[i])-10) + 0x41 :   BYTE_H(pDataIcd[i])+0x30;
				sGpSdrBuf[i*3+1] = DP_ConvHex2Asc(BYTE_L(pDataIcd[i]));//BYTE_L(pDataIcd[i]) >= 0x0A ? (BYTE_L(pDataIcd[i])-10) + 0x41 :   BYTE_L(pDataIcd[i])+0x30;

				sGpSdrBuf[i*3+2] = 0x2D;
			}
			sGpSdrBuf[i*3+2] = 0x20;

			gp_WritingStr(500,40,sGpSdrBuf);
			gp_WritingStr(540,27,&sGpSdrBuf[80]);
		}
	}
	else
	{
...
void GP_DEST_SCREEN(UCHAR nScreenInf)
{
	...
	case 0x38://행선 설정 화면 
		...
		for(i=0;i<10;i++) gp_WritingStr(340+i*10,6, nNvsram_Variable.nDestScreen[i].nStationName);	
	...
	case 0x6F:	//다음 화면 버튼.
		...
		{
			if((WORD_L(SanDisk_Vari.nTotalDestCnt)/10) > WORD_L(nNvsram_Variable.nScreenNum+1 ))
			{
				nNvsram_Variable.nScreenNum++;
				NVSRAM_Find_StationRoot(nNvsram_Variable.nScreenNum,10);	//역명을 불러 오는 부분.
				for(i=0;i<10;i++) gp_WritingStr(340+i*10,5, nNvsram_Variable.nDestScreen[i].nStationName);	
	...
	case 0x51: 
		...
		if(sDestNum >= sDestNum1)
		{
			...
			NVSRAM_Rd_StationName_Code(nTr_St_Info.nOnlyDestSendFlag,nTr_St_Info.nDest.nEnNameBuf,nTr_St_Info.nDest.nIRNameBuf,nTr_St_Info.nGpStName.nDest);//2011_11_16 nOnlyDestSendFlag
