...
UCHAR m_SCCPISTXBuff_Ach[100] ={0x02,
...
UCHAR SCC_INIT_ASYNC_EVENPARITY_19200BPS[][2]=
{
	{ 9,0xC0},		// CH A,B INITIALIZE        
	{ 4,0x47},		// x16MODE,B23:1'Stop(01),1.5'Stop(10),2'Stop(11),B1:Even(1)Odd(0),B0:ParityEnable 
	{ 3,0xC0},		// RX 8 BIT/CHAR, NO AUTO ENABLE       
	{ 5,0x60},		// TX 8 BIT/CHAR 
	{ 6,0x00},		// ABORT SDLC 
	{ 7,0x00},		// ABORT SDLC 
...
UCHAR SCC_INIT_ASYNC_NONEPARITY_38400BPS[][2]=
{
	{ 9,0xC0},		// CH A,B INITIALIZE        
	{ 4,0x44},		// x16MODE,B23:1'Stop(01),1.5'Stop(10),2'Stop(11),B1:Even(1)Odd(0),B0:ParityEnable 
	{ 3,0xC0},		// RX 8 BIT/CHAR, NO AUTO ENABLE       
	...
	// 16MHz, 19200bps, 24(18H)
	// 16MHz, 38400bps, 11( BH)
	{12,0x0B},		// LOW UCHAR B/R ( 16.000Mhz ) 
	{13,0x00},		// HIGH  " SET 9600 BAUD 	
...
void SCC_Initial()
{
	...
	for(i = 1;;i++)
	{
		if(SCC_INIT_ASYNC_EVENPARITY_19200BPS[i][0] == 0xff) break;
		SCC1_A_C = SCC_INIT_ASYNC_EVENPARITY_19200BPS[i][0];
		SCC1_A_C = SCC_INIT_ASYNC_EVENPARITY_19200BPS[i][1];	
	}
	...
	{
		if(SCC_INIT_ASYNC_EVENPARITY_19200BPS[i][0] == 0xff) break;
		SCC1_B_C = SCC_INIT_ASYNC_EVENPARITY_19200BPS[i][0];
		SCC1_B_C = SCC_INIT_ASYNC_EVENPARITY_19200BPS[i][1];	
	} 
	...
	{
		if(SCC_INIT_ASYNC_9600bps[i][0] == 0xff) break;
		SCC3_B_C = SCC_INIT_ASYNC_9600bps[i][0];
		SCC3_B_C = SCC_INIT_ASYNC_9600bps[i][1];
	}
	
	SCC1_INIT_A.RxOK = FALSE; // 옮바른 데이터가 수신되면 TRUE
	SCC1_INIT_A.RxOKCnt = 0;
	SCC1_INIT_A.RxPos = 0; // RX의 버퍼의 위치
	SCC1_INIT_A.RxLen = 35;//(21에서 열차 번호 때문에 2BYTE길어 진다.	//RX 길이
	//SCC1_INIT_A.RxLen = 16;
	SCC1_INIT_A.TxLen = 45;	//
...
////////////////////////////
	...
	memset(m_SCCPISTXBuff_Ach,0x00,sizeof(m_SCCPISTXBuff_Ach));
...
void Delay_SCC2_SendTo(UCHAR *pData,UCHAR nLen,UCHAR nChannel,UCHAR nDlyTm)
{
	...
	case SCC_A_CHANNEL:
			...
			if(nSelf_Test.nSELF_ICR_SELECT) //일반  차량
			{
...
void SCC1_AB()
{
	...
	for(i=0;i<3;i++)
	{
		...
		if(nCmdCode&0x20) // RR3 A채널 RX 일때..
		{
			...
			{
				case 0x01: if(SCC1_INIT_A.RxBuffer[0]!=0x02) SCC1_INIT_A.RxPos=0; break;
				case 0x02: if(SCC1_INIT_A.RxBuffer[1]!=0x20) SCC1_INIT_A.RxPos=0; break;
				default:
					if(SCC1_INIT_A.RxPos >= SCC1_INIT_A.RxLen) 
					{
						
						if(IsBCCOK(&SCC1_INIT_A.RxBuffer[1],32))
						{
							memcpy(m_Scc1RXBuff_Ach_Back,SCC1_INIT_A.RxBuffer,SCC1_INIT_A.RxLen);
							SCC1_INIT_A.RxOK = TRUE;
							SCC1_INIT_A.RxOKCnt++;
							SCC1_INIT_A.RxDlyTm = 5;
...
void SCC3_AB()
{
	...
	for(i=0;i<3;i++)
	{
		...
		if(nCmdCode&0x04) // RR3 B채널 RX 일때..
		{
			...
			{
			case 0x01 : if(WORD_L(SCC3_INIT_B.RxBuffer[0]) == 0xAA ||	// 설정기 Pass 카운터
							WORD_L(SCC3_INIT_B.RxBuffer[0]) == 0xAB ||
							WORD_L(SCC3_INIT_B.RxBuffer[0]) == 0xAC ||
							WORD_L(SCC3_INIT_B.RxBuffer[0]) == 0xAD ||
							WORD_L(SCC3_INIT_B.RxBuffer[0]) == 0xAE){ } 
						else SCC3_INIT_B.RxPos = 0; break;
			case 0x02 : if(WORD_L(SCC3_INIT_B.RxBuffer[1]) == 0xBB ||	//설정기 그룹 명령어
							WORD_L(SCC3_INIT_B.RxBuffer[1]) == 0xBC || 
							WORD_L(SCC3_INIT_B.RxBuffer[1]) == 0xBE){ } 
						else SCC3_INIT_B.RxPos = 0; break;
			case 0x03 : if(WORD_L(SCC3_INIT_B.RxBuffer[2]) != 0xCC) SCC3_INIT_B.RxPos = 0; break;
			case 0x07 :
				nTemp3 = MAKE_WORD(SCC3_INIT_B.RxBuffer[3],SCC3_INIT_B.RxBuffer[4]);
				nTemp4 = MAKE_WORD(SCC3_INIT_B.RxBuffer[5],SCC3_INIT_B.RxBuffer[6]);
				if(nTemp3&nTemp4 ) {SCC3_INIT_B.RxPos = 0;}
				else {SCC3_INIT_B.RxLen = nTemp3 + 10;}
				break;
			default :
				if(DWORD_L(SCC3_INIT_B.RxPos) >=DWORD_L(SCC3_INIT_B.RxLen))
				{
					memcpy(m_Scc3RXBuff_Bch_Back,SCC3_INIT_B.RxBuffer,SCC3_INIT_B.RxLen);
...
void Scc1msTime()
{
    ...
    *******************************************/
	...
	if(SCC1_INIT_A.TX_RTS_OFF_TIME == 1) {SCC1_INIT_A.TxOK = TRUE; SCC1A_RTS_OFF;}	
    ...
    *******************************************/
	...
	{
		//if(!nTr_St_Info.nSendIddTxFlag){ nTr_St_Info.nSendIddTxFlag = TRUE;}
    ...
    *******************************************/
	...
	if(SCC3_INIT_B.TX_RTS_OFF_TIME){SCC3_INIT_B.TX_RTS_OFF_TIME--;}
	if(SCC3_INIT_B.TX_RTS_OFF_TIME == 1)
	{
		//if(!nTr_St_Info.nSendIddTxFlag){ nTr_St_Info.nSendIddTxFlag = TRUE;}
