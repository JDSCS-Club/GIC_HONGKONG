...
int mScanVidioFlag = 1; //ID 번호를 찍기 위해 참으로 시작.
/******************************
	패턴 TEST 변수
******************************/
...
int Blink_Timer_Count=70;
	...
	*******************/
	...
	int REMEMBER_RING[50];	//RING_BUFF 입력시 입력 DATA의 길이를 가지도 있다.
...
void Display_main(void)
{
        ...
        if(CHACK_DATA == 1)
        {
                ...
                else if((B_FRONT!=FRONT)&&(SCAN.Video_T_F == 0))
                {
                        POP();
                }
...
int UP_Font_posi=0;
int mFont_Save_posi=0;	//0x100000 기록 위치를 Count
...
void FONT_POSITION(UCHAR *Posi)
{
     ...
     memset(DOT_VIDEO,0x00,MODULE_32_256);
     ...
     DpMem=(int*)&DCC;//DCC 설정
     for(i=0;i<14;i++){DpMem[i]=(Posi[i]&0xff);}
        
     DCC.SIZE= 224;//(DCC.SIZE)*8;	// 항상 160 이어야 하무로 160으로 고정
     DCC.STATE=((DCC.STATE)>=0x02) ? 0x00 : (DCC.STATE); // Norm 또는 S-Left 기능만 동작 하도록 한다. (14_07_09)
	...
	for(Font_DATA_Cont=14;Font_DATA_Cont<now_font_Len; )
	{
		...
		if((Posi[Font_DATA_Cont]&0xff)==0x1B)
		{
			...
			{
				Font_DATA_Cont++;
				if((Posi[Font_DATA_Cont++]&0xff) == 0x01)		{ESC.COLOR = 0x01; }
				else if((Posi[Font_DATA_Cont-0x01]&0xff) == 0x02)	{ESC.COLOR = 0x02; }
			...
			else if((Posi[Font_DATA_Cont]&0xff)==0x07) 
			{
				...
				else 	// 한국어
				{
					...
					if(WORD_L(ESC.D_SIZE) == 0x01) { ESC.SIZE = 0;}
                       ...
                       (Posi[Font_DATA_Cont]&0xff)<0xa1 && 
                       (WORD_L(ESC.nCountry_Code) == 0x00 || WORD_L(ESC.nCountry_Code) == 0x02))	// 16 영숫자
		{
			...
			{ 	//화살표
                                ...
                                if(((ADD_CHECK()&0xF1)==0x01)||(((ADD_CHECK()&0x02)==0x02)&&((ADD_CHECK()&0xF2)!=0x02)))
                                {
                                        Jump=((Posi[Font_DATA_Cont+2]&0xff)==0x52) ? (270848) : (270720);//바뀜
                                }
                                else
                                {
                                        Jump=((Posi[Font_DATA_Cont+2]&0xff)==0x52) ? (270720) : (270848);	
                                }
                                
                                //for(j=0;j<128;j++) nBB[j]=nOR[Jump+j];
                                //for(j=0;j<128;j++) nBB[j]=nOR[Jump+j];
                                
                                NORFLASH_ReadData(&norFlash,Jump,nBB,128); // 2014년 7월 9일 메모리 읽는 타임 문제로 한번도 읽는다.
                                NORFLASH_ReadData(&norFlash,Jump,nBB,128);
			...
			{	//ITEM
				...
				ITEM_DATA=ITEM_HAP(ITEM_val[0],ITEM_val[1],ITEM_val[2],ITEM_val[3]);
                                ...
                                NORFLASH_ReadData(&norFlash,Jump,nBB,32);
                                NORFLASH_ReadData(&norFlash,Jump,nBB,32);
			...
			{     //영숫자
				...
				Jump=(((Posi[Font_DATA_Cont]&0xff)-0x20)*16)+ESC.SHAPE;
                                ...
                                NORFLASH_ReadData(&norFlash,Jump,nBB,16);
                                NORFLASH_ReadData(&norFlash,Jump,nBB,16);
		       ...
		       ((Posi[Font_DATA_Cont]&0xff)>=0xA0 && (Posi[Font_DATA_Cont]&0xff)<=0xDF)&& WORD_L(ESC.nCountry_Code) == 0x01)   
		{
			...
			memcpy(&Compari_ITEM[0], &Posi[Font_DATA_Cont],6);
                        ...
                        if(((((strncmp(&Compari_Arrow_R[0],&Arrow_R[0],10))&0xff)==0x00)
				||(((strncmp(&Compari_Arrow_L[0],&Arrow_L[0],10))&0xff)==0x00)))
			{ 	//화살표
                       ...
                       (Posi[Font_DATA_Cont]&0xff)>=0x20)
		{
			...
			else if(WORD_L(ESC.nCountry_Code) == 0x02)//중국어
			{
                                ...
                                {
                                        Jump=((Posi[Font_DATA_Cont]&0xff)-0xb0)*3008+((Posi[Font_DATA_Cont+1]&0xff)-0xa1)*32+30080+ESC.SIZE+ESC.SHAPE;
                                }
        ...
        if(DCC.STATE==0x01){ memset(DOT_VIDEO,0x00,MODULE_16_256), memset(SCREEN_VIDEO,0x00,MODULE_48_256);}//LEFT 시 화면을 지운다.
...
void Imag_Screen(UCHAR *Imag)
{
        ...
        for(i=Font_16_24;i<24;i++)
        {
                ...
                for(j=7;j>=0;j--)
                {
                        pVd[mFont_Save_posi++]=(((Imag[(Lint_sort*2)]&0xff)>>j)&0x01)==0x01 ? ((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? 0x00 : ESC.COLOR) 
                                                                                             :((ESC.TEMPER==0x02) ||(ESC.TEMPER==0x03)? ESC.COLOR : 0x00);
                }
                for(j=7;j>=0;j--)
                {
                        pVd[mFont_Save_posi++]=(((Imag[((Lint_sort*2)+1)]&0xff)>>j)&0x01)==0x01 ? ((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? 0x00 : ESC.COLOR) 
                                                                                                 :((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? ESC.COLOR : 0x00);
                }
                ...
                if(ESC.TEMPER != 0x01&&ESC.TEMPER != 0x03 )	//BLINK 기능 01=이면 찍지를 않는다.
                {
                        mFont_Save_posi=(DCC.STATE==0x01) ? LEF_Font_posi :((DCC.STATE==0x03) ? STATIC_LEF_Posi : UP_Font_posi) ;
                         ...
                         for(j=7;j>=0;j--)
                         {
                                 L_pVd[mFont_Save_posi++]=(((Imag[(Lint_sort*2)]&0xff)>>j)&0x01)==0x01 ? ((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? 0x00 : ESC.COLOR) 
                                                                                                        :((ESC.TEMPER==0x02) ||(ESC.TEMPER==0x03)? ESC.COLOR : 0x00);
                         }
                         for(j=7;j>=0;j--)
                         {
                                 L_pVd[mFont_Save_posi++]=(((Imag[((Lint_sort*2)+1)]&0xff)>>j)&0x01)==0x01 ? ((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? 0x00 : ESC.COLOR) 
                                                                                                            :((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? ESC.COLOR : 0x00);
                         }
...
void Imag_Screen_door(UCHAR *Imag)
{
	...
	for(i=Font_16_24;i<24;i++)
	{
                ...
                for(k=0;k<4;k++)
                {
                        for(j=7;j>=0;j--)
                        {
                              pVd[mFont_Save_posi++]=(((Imag[(Lint_sort*2)+(k*32)]&0xff)>>j)&0x01)==0x01 ? ((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? 0x00 : ESC.COLOR) 
                                                                                                          :((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? ESC.COLOR : 0x00);
                        }
                        for(j=7;j>=0;j--)
                        {
                              pVd[mFont_Save_posi++]=(((Imag[((Lint_sort*2)+1+(k*32))]&0xff)>>j)&0x01)==0x01 ? ((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? 0x00 : ESC.COLOR) 
                                                                                                              :((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? ESC.COLOR : 0x00);
                        }
                }
                ...
                if(ESC.TEMPER!=0x01&&ESC.TEMPER!=0x03)
                {
                        ...
                        for(k=0;k<4;k++)
                        {
                                for(j=7;j>=0;j--)
                                {
                                        L_pVd[mFont_Save_posi++]=(((Imag[(Lint_sort*2)+(k*32)]&0xff)>>j)&0x01)==0x01 ? ((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? 0x00 : ESC.COLOR) 
                                                                                                                      :((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? ESC.COLOR : 0x00);
                                }
                                for(j=7;j>=0;j--)
                                {
                                        L_pVd[mFont_Save_posi++]=(((Imag[((Lint_sort*2)+1+(k*32))]&0xff)>>j)&0x01)==0x01 ? ((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? 0x00 : ESC.COLOR) 
                                                                                                                          :((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? ESC.COLOR : 0x00);
                                }
                        }
...
void Imag_Screen_8(UCHAR *Imag)
{
	...
	for(i=Font_16_24;i<24;i++)
	{
                ...
                for(j=7;j>=0;j--)
                {
                        pVd[mFont_Save_posi]= (((Imag[Lint_sort]&0xff)>>j)&0x01)==0x01 ? ((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? 0x00 : ESC.COLOR) 
                                                                                        :((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? ESC.COLOR : 0x00);
                        mFont_Save_posi++;
                }
                ...
                if(ESC.TEMPER!=0x01&&ESC.TEMPER!=0x03)
                {
                        ...
                        for(j=7;j>=0;j--)	L_pVd[mFont_Save_posi++]=(((Imag[Lint_sort]&0xff)>>j)&0x01)==0x01 ? ((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? 0x00 : ESC.COLOR) 
                                                                                                                   :((ESC.TEMPER==0x02)||(ESC.TEMPER==0x03) ? ESC.COLOR : 0x00);
                }
                Lint_sort++;
	}
...
void SEND_DATA(void)
{
	...
	SCC1_Init_AChl.pTxBuffer[8]=(ADD_CHECK()&0xff);
...
void HEX_SWITCH(void)
{
	...
	for(i=0;i<2;i++)
	{
		SW_RCV= (i==0) ? (ADD_CHECK()>>4) : (ADD_CHECK()&0x0f);
	...
	for(i=1;i<16;i++)
	{
            ...
            sw_num=((i)*DCC.SIZE)+((DCC.SIZE/2)-8);
