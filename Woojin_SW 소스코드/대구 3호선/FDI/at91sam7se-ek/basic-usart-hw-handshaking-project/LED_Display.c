...
FONT_VALUE DCC={0x01,0x01,0x00,0x00,0x01,96,0,6,16,0,4,10,0,0};
...
void FONT_POSITION(UCHAR *Posi)
{
     ...
     DCC.SIZE= 96;//(DCC.SIZE)*8;	// 항상 160 이어야 하무로 160으로 고정
...
void IMAG_POINT_VALUE(int Img)
{
        ...
        UP_Font_posi=((Img*DCC.SIZE)+UP_DATA_LEN)+(((SCAN.Font_Pr_P)/DCC.SIZE)*FONT_24_256);
...
void DOT_VIDEO_PIRNT(void)
{
        ...
        for(i=23;i>7;i--)
        {
                Font_Save_posi=(((i)*DCC.SIZE)+blink_video_normal+(SCAN.NorMal_Page_Count*FONT_24_256)); // 2014y07m10d FDI 수정.
...
{	
     ...
     if(!(DCC_T_C%DCC.STOP)||(DCC.STOP)==0x00)
     {
          ...
          else if(((SCAN.NorMal_Page_Count+1) <= (SCAN.Font_Pr_P/DCC.SIZE)+1) // 2014y07m10d FDI 수정.
			     &&(DCC.STOP!=0x00))	//4월 9일 수정 (길이 계산 착오)
          {

               SCAN.NorMal_Page_Count++;
          }
          else if(((SCAN.NorMal_Page_Count+1)>(SCAN.Font_Pr_P/DCC.SIZE)+1)
	        &&(DCC.STOP!=0x00)
