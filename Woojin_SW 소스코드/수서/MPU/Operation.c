...
void op_AllFaultCheck()
{
	...
	for(nCarHo=0;nCarHo<m_nTrainSize;nCarHo++)
	{
		...
		{
			op_AddFaultCode(100,((PCCDITCTYPE)m_CC_Info.RxInfo[nCarHo].Head.ccDi)->BYPR,nCarHo);
		}
		...
		if(ISTRTYPE(m_CC_Info.RxInfo[nCarHo].Head.chHexaID[0]) == TRTYPE_MD) 
		{
			...
			op_AddFaultCode(20,!((PCCDIMTYPE)m_CC_Info.RxInfo[nCarHo].Head.ccDi)->DCTR1,nCarHo);
	...
	for(nCarHo=0;nCarHo<m_nTrainSize;nCarHo++)
	{
		...
		if(ISTRTYPE(m_CC_Info.RxInfo[nCarHo].Head.chHexaID[0]) == TRTYPE_MD) 
		{
			...
			{
				op_AddFaultCode(48,((PCCDIMTYPE)m_CC_Info.RxInfo[nCarHo].Head.ccDi)->PANPS1,nCarHo);
				op_AddFaultCode(49,((PCCDIMTYPE)m_CC_Info.RxInfo[nCarHo].Head.ccDi)->PANPS2,nCarHo);
			}

			op_AddFaultCode(50,!((PCCDIMTYPE)m_CC_Info.RxInfo[nCarHo].Head.ccDi)->BPSUF,nCarHo);
			
			pBrakeSD = m_CC_Info.RxInfo[nCarHo].pLSysRx[LSYSID_BRAKE]->Body.chDataPack;
			op_AddFaultCode(61,pBrakeSD->RBFF && ((PCCDIMTYPE)m_CC_Info.RxInfo[nCarHo].Head.ccDi)->DCTR1 ,nCarHo);
		}
...
BYTE op_GetTrType(int nCarHo)
{
	return ISTRTYPE(m_CC_Info.RxInfo[nCarHo].Head.chHexaID[0]);
}
...
int op_GetCmNotBypassNumber()
{
	...
	for(nCarHo=0;nCarHo<m_nTrainSize;nCarHo++)
	{
		...
		case TRTYPE_TC:
		case TRTYPE_T2:
			if(((PCCDITCTYPE)m_CC_Info.RxInfo[nCarHo].Head.ccDi)->BYPR) nNumber++;
			break;
		}
	}
	return nNumber;
...
void op_MakeBrakeProtocol(BOOL bTDC)
{
	...
	BOOL bTxSend = !(tg_GetMainCommTxCounter()%3) ? TRUE : FALSE;
	...
	if(bTxSend)
	{
		...
		if(wApplyCarHo)
		{
			...
			else
			{
				...
				pSivSDR->spare[0] = 0xAA;
				pSivSDR->spare[1] = 0xAA;
				pSivSDR->spare[2] = 0xAA;
				pSivSDR->usEtx = 0x03;
...
BYTE op_GetLSysMaxTraceNumber(int nLSysID,int nCarHo)
{
	...
	case LSYSID_BRAKE:
		...
		chMaxTraceNum = op_GetTraceMaxNumber(pBrakeSD->TRACENO & 0x1F);
		break;
	case LSYSID_SIV:
		pSivSD = (PSIVSDTYPE)m_CC_Info.RxInfo[nCarHo].pLSysRx[nLSysID]->Body.chDataPack;
		chMaxTraceNum = op_GetTraceMaxNumber(pSivSD->TRACENO & 0x07);
		break;
...
int op_GetIMValue(int nCarHo)
{
	...
	if(ISTRTYPE(m_CC_Info.RxInfo[nCarHo].Head.chHexaID[0]) == TRTYPE_MD) 
	{
		...
		return 0; /*(int)( xsqrt( (CIID*CIID + CIIQ*CIIQ)/3. ) );*/
	}
	else
	{
		return 0;
...
/* 단위 [kg]											   */
/***********************************************************/
UINT op_EmptyCarWeight(int nCarHo)
{
	UINT nEmptyWeight[5] = {32800,26400,31700,38400,38400};
	BYTE chTrType = ISTRTYPE(m_CC_Info.RxInfo[nCarHo].Head.chHexaID[0]);

	if(ISRANGE(chTrType,1,5)) return nEmptyWeight[chTrType-1];
	else return nEmptyWeight[0];
}
...
/* 단위 [kg]											   */
/***********************************************************/
UINT op_GetPassengerWeight(int nCarHo)
{
	int nLSysID = LSYSID_BRAKE;
	int nPairedCarHo;
	UINT nEmptyCarWeight,nCarWeight;
	BYTE chWT;

	if(ISTRTYPE(m_CC_Info.RxInfo[nCarHo].Head.chHexaID[0]) == TRTYPE_MD) 
	{
		chWT = ((PBRAKESDTYPE)m_CC_Info.RxInfo[nCarHo].pLSysRx[nLSysID]->Body.chDataPack)->WTM;
	}
	else
	{
		nPairedCarHo = op_GetPairedCarHo(nCarHo);
		if(ISTRTYPE(m_CC_Info.RxInfo[nPairedCarHo].Head.chHexaID[0]) == TRTYPE_MD) 
		{
			chWT = ((PBRAKESDTYPE)m_CC_Info.RxInfo[nPairedCarHo].pLSysRx[nLSysID]->Body.chDataPack)->WTT;
		}
		else chWT = 0;
	}
	...
	if(chWT)
	{
		nEmptyCarWeight = op_EmptyCarWeight(nCarHo);
		nCarWeight = chWT * 200 + 25000;

		if(nCarWeight > nEmptyCarWeight) return (nCarWeight-nEmptyCarWeight);
		else return 0;
	}
	else return 0;
}
...
BOOL op_IsCiLSysCut(int nCarHo)
{
	...
	PCCDIMTYPE pCcDi = (PCCDITYPE)m_CC_Info.RxInfo[nCarHo].Head.ccDi;
	...
	/*
	if(pCcDi->DCTR1) return TRUE;
	if(pCiSD->ESTF) return TRUE;
	if(pCcDi->PANPS1 && pCcDi->PANPS2 && !((PCCDITYPE)m_TcThis.pCCInfo->Head.ccDi)->MRPS) return TRUE;
	if(pCcDi->CCOS) return TRUE;
	*/
...
BOOL op_RecHisLoad()
{
	...
	for(nCarHo=0;nCarHo<m_nTrainSize;nCarHo++)
	{
		WLoad.chLDP[nCarHo] = (BYTE)(op_GetPassengerWeight(nCarHo)/200);
	}
...
void op_CCRxDervieProcess()
{
	...
	for(nCarHo=0;nCarHo<m_nTrainSize;nCarHo++)
	{
		...
		if(op_GetTrType(nCarHo) == TRTYPE_TC || op_GetTrType(nCarHo) == TRTYPE_T2)
		{
			if(((PCCDITCTYPE)m_CC_Info.RxInfo[nCarHo].Head.ccDi)->CMGR) m_CC_Info.xDerive[nCarHo].nCMGR_Counter++;
			else m_CC_Info.xDerive[nCarHo].nCMGR_Counter = 0;
		}
	}
}
...
BOOL op_IsOnTest()
{
	if(m_TcThis.xSivTest.bTxTestStart) return TRUE;
